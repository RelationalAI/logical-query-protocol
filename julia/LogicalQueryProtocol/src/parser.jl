...
function construct_csv_config(parser::Parser, config_dict::Vector{Tuple{String, Proto.Value}})::Proto.CSVConfig
    ...
    _t972 = _extract_value_int32(parser, get(config, "csv_header_row", nothing), 1)
    header_row = _t972
    _t973 = _extract_value_int64(parser, get(config, "csv_skip", nothing), 0)
    skip = _t973
    _t974 = _extract_value_string(parser, get(config, "csv_new_line", nothing), "")
    new_line = _t974
    _t975 = _extract_value_string(parser, get(config, "csv_delimiter", nothing), ",")
    delimiter = _t975
    _t976 = _extract_value_string(parser, get(config, "csv_quotechar", nothing), "\"")
    quotechar = _t976
    _t977 = _extract_value_string(parser, get(config, "csv_escapechar", nothing), "\"")
    escapechar = _t977
    _t978 = _extract_value_string(parser, get(config, "csv_comment", nothing), "")
    comment = _t978
    _t979 = _extract_value_string_list(parser, get(config, "csv_missing_strings", nothing), String[])
    missing_strings = _t979
    _t980 = _extract_value_string(parser, get(config, "csv_decimal_separator", nothing), ".")
    decimal_separator = _t980
    _t981 = _extract_value_string(parser, get(config, "csv_encoding", nothing), "utf-8")
    encoding = _t981
    _t982 = _extract_value_string(parser, get(config, "csv_compression", nothing), "auto")
    compression = _t982
    _t983 = _extract_value_int64(parser, get(config, "partition_size", nothing), 0)
    partition_size = _t983
    _t984 = Proto.CSVConfig(header_row=header_row, skip=skip, new_line=new_line, delimiter=delimiter, quotechar=quotechar, escapechar=escapechar, comment=comment, missing_strings=missing_strings, decimal_separator=decimal_separator, encoding=encoding, compression=compression, partition_size=partition_size)
    return _t984
end
...
function construct_betree_info(parser::Parser, key_types::Vector{Proto.var"#Type"}, value_types::Vector{Proto.var"#Type"}, config_dict::Vector{Tuple{String, Proto.Value}})::Proto.BeTreeInfo
    config = Dict(config_dict)
    _t985 = _try_extract_value_float64(parser, get(config, "betree_config_epsilon", nothing))
    epsilon = _t985
    _t986 = _try_extract_value_int64(parser, get(config, "betree_config_max_pivots", nothing))
    max_pivots = _t986
    _t987 = _try_extract_value_int64(parser, get(config, "betree_config_max_deltas", nothing))
    max_deltas = _t987
    _t988 = _try_extract_value_int64(parser, get(config, "betree_config_max_leaf", nothing))
    max_leaf = _t988
    _t989 = Proto.BeTreeConfig(epsilon=epsilon, max_pivots=max_pivots, max_deltas=max_deltas, max_leaf=max_leaf)
    storage_config = _t989
    _t990 = _try_extract_value_uint128(parser, get(config, "betree_locator_root_pageid", nothing))
    root_pageid = _t990
    _t991 = _try_extract_value_bytes(parser, get(config, "betree_locator_inline_data", nothing))
    inline_data = _t991
    _t992 = _try_extract_value_int64(parser, get(config, "betree_locator_element_count", nothing))
    element_count = _t992
    _t993 = _try_extract_value_int64(parser, get(config, "betree_locator_tree_height", nothing))
    tree_height = _t993
    _t994 = Proto.BeTreeLocator(location=(!isnothing(root_pageid) ? OneOf(:root_pageid, root_pageid) : (!isnothing(inline_data) ? OneOf(:inline_data, inline_data) : nothing)), element_count=element_count, tree_height=tree_height)
    relation_locator = _t994
    _t995 = Proto.BeTreeInfo(key_types=key_types, value_types=value_types, storage_config=storage_config, relation_locator=relation_locator)
    return _t995
end

function default_configure(parser::Parser)::Proto.Configure
    _t996 = Proto.IVMConfig(level=Proto.MaintenanceLevel.MAINTENANCE_LEVEL_OFF)
    ivm_config = _t996
    _t997 = Proto.Configure(semantics_version=0, ivm_config=ivm_config)
    return _t997
end
...
function construct_configure(parser::Parser, config_dict::Vector{Tuple{String, Proto.Value}})::Proto.Configure
    ...
    _t998 = Proto.IVMConfig(level=maintenance_level)
    ivm_config = _t998
    _t999 = _extract_value_int64(parser, get(config, "semantics_version", nothing), 0)
    semantics_version = _t999
    _t1000 = Proto.Configure(semantics_version=semantics_version, ivm_config=ivm_config)
    return _t1000
end
...
function construct_export_csv_config(parser::Parser, path::String, columns::Vector{Proto.ExportCSVColumn}, config_dict::Vector{Tuple{String, Proto.Value}})::Proto.ExportCSVConfig
    config = Dict(config_dict)
    _t1001 = _extract_value_int64(parser, get(config, "partition_size", nothing), 0)
    partition_size = _t1001
    _t1002 = _extract_value_string(parser, get(config, "compression", nothing), "")
    compression = _t1002
    _t1003 = _extract_value_boolean(parser, get(config, "syntax_header_row", nothing), true)
    syntax_header_row = _t1003
    _t1004 = _extract_value_string(parser, get(config, "syntax_missing_string", nothing), "")
    syntax_missing_string = _t1004
    _t1005 = _extract_value_string(parser, get(config, "syntax_delim", nothing), ",")
    syntax_delim = _t1005
    _t1006 = _extract_value_string(parser, get(config, "syntax_quotechar", nothing), "\"")
    syntax_quotechar = _t1006
    _t1007 = _extract_value_string(parser, get(config, "syntax_escapechar", nothing), "\\")
    syntax_escapechar = _t1007
    _t1008 = Proto.ExportCSVConfig(path=path, data_columns=columns, partition_size=partition_size, compression=compression, syntax_header_row=syntax_header_row, syntax_missing_string=syntax_missing_string, syntax_delim=syntax_delim, syntax_quotechar=syntax_quotechar, syntax_escapechar=syntax_escapechar)
    return _t1008
end

function construct_export_csv_config_with_source(parser::Parser, path::String, csv_source::Any, csv_config::Union{Nothing, Proto.CSVConfig})::Proto.ExportCSVConfig
    _t1009 = Proto.ExportCSVConfig(path=path, csv_source=csv_source, csv_config=csv_config)
    return _t1009
end

function _make_value_int32(parser::Parser, v::Int32)::Proto.Value
    _t1010 = Proto.Value(value=OneOf(:int_value, Int64(v)))
    return _t1010
end

function _make_value_int64(parser::Parser, v::Int64)::Proto.Value
    _t1011 = Proto.Value(value=OneOf(:int_value, v))
    return _t1011
end

function _make_value_float64(parser::Parser, v::Float64)::Proto.Value
    _t1012 = Proto.Value(value=OneOf(:float_value, v))
    return _t1012
end

function _make_value_string(parser::Parser, v::String)::Proto.Value
    _t1013 = Proto.Value(value=OneOf(:string_value, v))
    return _t1013
end

function _make_value_boolean(parser::Parser, v::Bool)::Proto.Value
    _t1014 = Proto.Value(value=OneOf(:boolean_value, v))
    return _t1014
end

function _make_value_uint128(parser::Parser, v::Proto.UInt128Value)::Proto.Value
    _t1015 = Proto.Value(value=OneOf(:uint128_value, v))
    return _t1015
end
...
function deconstruct_configure(parser::Parser, msg::Proto.Configure)::Vector{Tuple{String, Proto.Value}}
    ...
    if msg.ivm_config.level == Proto.MaintenanceLevel.MAINTENANCE_LEVEL_AUTO
        _t1016 = _make_value_string(parser, "auto")
        push!(result, ("ivm.maintenance_level", _t1016,))
    else
        if msg.ivm_config.level == Proto.MaintenanceLevel.MAINTENANCE_LEVEL_ALL
            _t1017 = _make_value_string(parser, "all")
            push!(result, ("ivm.maintenance_level", _t1017,))
        else
            if msg.ivm_config.level == Proto.MaintenanceLevel.MAINTENANCE_LEVEL_OFF
                _t1018 = _make_value_string(parser, "off")
                push!(result, ("ivm.maintenance_level", _t1018,))
            end
        end
    end
    _t1019 = _make_value_int64(parser, msg.semantics_version)
    push!(result, ("semantics_version", _t1019,))
    return sort(result)
end

function deconstruct_csv_config(parser::Parser, msg::Proto.CSVConfig)::Vector{Tuple{String, Proto.Value}}
    result = Tuple{String, Proto.Value}[]
    _t1020 = _make_value_int32(parser, msg.header_row)
    push!(result, ("csv_header_row", _t1020,))
    _t1021 = _make_value_int64(parser, msg.skip)
    push!(result, ("csv_skip", _t1021,))
    if msg.new_line != ""
        _t1022 = _make_value_string(parser, msg.new_line)
        push!(result, ("csv_new_line", _t1022,))
    end
    _t1023 = _make_value_string(parser, msg.delimiter)
    push!(result, ("csv_delimiter", _t1023,))
    _t1024 = _make_value_string(parser, msg.quotechar)
    push!(result, ("csv_quotechar", _t1024,))
    _t1025 = _make_value_string(parser, msg.escapechar)
    push!(result, ("csv_escapechar", _t1025,))
    if msg.comment != ""
        _t1026 = _make_value_string(parser, msg.comment)
        push!(result, ("csv_comment", _t1026,))
    end
    for missing_string in msg.missing_strings
        _t1027 = _make_value_string(parser, missing_string)
        push!(result, ("csv_missing_strings", _t1027,))
    end
    _t1028 = _make_value_string(parser, msg.decimal_separator)
    push!(result, ("csv_decimal_separator", _t1028,))
    _t1029 = _make_value_string(parser, msg.encoding)
    push!(result, ("csv_encoding", _t1029,))
    _t1030 = _make_value_string(parser, msg.compression)
    push!(result, ("csv_compression", _t1030,))
    if msg.partition_size != 0
        _t1031 = _make_value_int64(parser, msg.partition_size)
        push!(result, ("csv_partition_size", _t1031,))
    end
    return sort(result)
end
...
function deconstruct_betree_info_config(parser::Parser, msg::Proto.BeTreeInfo)::Vector{Tuple{String, Proto.Value}}
    result = Tuple{String, Proto.Value}[]
    _t1032 = _make_value_float64(parser, msg.storage_config.epsilon)
    push!(result, ("betree_config_epsilon", _t1032,))
    _t1033 = _make_value_int64(parser, msg.storage_config.max_pivots)
    push!(result, ("betree_config_max_pivots", _t1033,))
    _t1034 = _make_value_int64(parser, msg.storage_config.max_deltas)
    push!(result, ("betree_config_max_deltas", _t1034,))
    _t1035 = _make_value_int64(parser, msg.storage_config.max_leaf)
    push!(result, ("betree_config_max_leaf", _t1035,))
    if _has_proto_field(msg.relation_locator, Symbol("root_pageid"))
        if !isnothing(_get_oneof_field(msg.relation_locator, :root_pageid))
            _t1036 = _make_value_uint128(parser, _get_oneof_field(msg.relation_locator, :root_pageid))
            push!(result, ("betree_locator_root_pageid", _t1036,))
        end
    end
    if _has_proto_field(msg.relation_locator, Symbol("inline_data"))
        if !isnothing(_get_oneof_field(msg.relation_locator, :inline_data))
            _t1037 = _make_value_string(parser, String(_get_oneof_field(msg.relation_locator, :inline_data)))
            push!(result, ("betree_locator_inline_data", _t1037,))
        end
    end
    _t1038 = _make_value_int64(parser, msg.relation_locator.element_count)
    push!(result, ("betree_locator_element_count", _t1038,))
    _t1039 = _make_value_int64(parser, msg.relation_locator.tree_height)
    push!(result, ("betree_locator_tree_height", _t1039,))
    return sort(result)
end
...
function deconstruct_export_csv_config(parser::Parser, msg::Proto.ExportCSVConfig)::Vector{Tuple{String, Proto.Value}}
    ...
    if !isnothing(msg.partition_size)
        _t1040 = _make_value_int64(parser, msg.partition_size)
        push!(result, ("partition_size", _t1040,))
    end
    if !isnothing(msg.compression)
        _t1041 = _make_value_string(parser, msg.compression)
        push!(result, ("compression", _t1041,))
    end
    if !isnothing(msg.syntax_header_row)
        _t1042 = _make_value_boolean(parser, msg.syntax_header_row)
        push!(result, ("syntax_header_row", _t1042,))
    end
    if !isnothing(msg.syntax_missing_string)
        _t1043 = _make_value_string(parser, msg.syntax_missing_string)
        push!(result, ("syntax_missing_string", _t1043,))
    end
    if !isnothing(msg.syntax_delim)
        _t1044 = _make_value_string(parser, msg.syntax_delim)
        push!(result, ("syntax_delim", _t1044,))
    end
    if !isnothing(msg.syntax_quotechar)
        _t1045 = _make_value_string(parser, msg.syntax_quotechar)
        push!(result, ("syntax_quotechar", _t1045,))
    end
