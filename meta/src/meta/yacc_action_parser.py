...
def _get_rhs_element_name(elem: Rhs) -> str | None:
...
def _get_rhs_param_info(rhs: Rhs) -> list[tuple[str | None, TargetType | None]]:
    ...
    if isinstance(rhs, Sequence):
        result: list[tuple[str | None, TargetType | None]] = []
...
def _get_rhs_param_types(rhs: Rhs) -> list[TargetType | None]:
...
def _infer_type(
    expr: TargetExpr, line: int | None = None, ctx: Optional["TypeContext"] = None
) -> TargetType:
    ...
    if isinstance(expr, Var):
        ...
        try:
            ...
            return BaseType("Any")
    raise YaccGrammarError(
        f"Cannot infer type of expression: {type(expr).__name__}", line
    )


def _make_list_expr(
    elements: list, line: int | None, ctx: Optional["TypeContext"]
) -> ListExpr:
...
def _unsupported_node_error(
    node: ast.AST, line: int | None, reason: str = ""
) -> YaccGrammarError:
...
def _convert_expr(
    ...
    ctx: "TypeContext",
    line: int | None,
    local_vars: dict[str, TargetType],
    param_info: list[tuple[str | None, TargetType | None]] | None = None,
    params: list[Var] | None = None,
) -> TargetExpr:
    """Convert a Python AST node to target IR.

    Handles both action expressions (with $N parameter references) and
    function body expressions. When param_info and params are provided,
    $N references are resolved against those parameters.

    Args:
        node: The AST node to convert
        ctx: Type context for looking up functions
        line: Line number for error messages
        local_vars: Variables in scope (name -> type)
        param_info: Optional list of (name, type) for each RHS element
        params: Optional parameter Vars (non-literal elements only)
    """
    from .target_builtins import is_builtin

    def convert(n: ast.AST) -> TargetExpr:
        return _convert_expr(n, ctx, line, local_vars, param_info, params)
    ...
    if isinstance(node, ast.Constant):
        ...
        if name == "True":
            ...
            if param_type is None:
                raise YaccGrammarError(
                    f"Cannot reference literal at position ${idx + 1}", line
                )
        ...
        if isinstance(node.value, ast.Attribute) and isinstance(
            ...
            possible_enum = _unescape_keyword(node.value.attr)
            ...
            if ctx.enum_lookup is not None:
                ...
                if enum_values is not None:
                    ...
                    if attr_name not in valid:
                        raise YaccGrammarError(
                            f"Unknown enum value '{attr_name}' for enum "
                            f"{module_name}.{possible_enum}. Valid values: {valid}",
                            line,
                        )
                    return EnumValue(module_name, possible_enum, attr_name)

            # Not an enum, nested field access
            obj = convert(node.value)
            return _make_get_field(obj, attr_name, ctx, line)

        # Two-level: module.Message or obj.field
        if isinstance(node.value, ast.Name):
            name = node.value.id
            if name in local_vars:
                obj = Var(name, local_vars[name])
                return _make_get_field(obj, attr_name, ctx, line)
        ...
        # Handle message constructor or method call
        if isinstance(func, ast.Attribute) and isinstance(func.value, ast.Name):
                ...
                for kw in node.keywords:
                    if kw.arg is None:
                        raise YaccGrammarError(
                            ...
                            "module.Msg(field1=val1, field2=val2)",
                            line,
                        )
                    field_name = _unescape_keyword(kw.arg)
        ...
        # Method calls on expressions: expr.method(args)
        if isinstance(func, ast.Attribute):
            obj = convert(func.value)
            method_name = func.attr
            ...
            if isinstance(obj_type, MessageType):
                ...
                return Call(method_ref, args)
            raise YaccGrammarError(
                f"Cannot call method '{method_name}' on type {obj_type}", line
            )
        ...
        if len(node.ops) == 1 and len(node.comparators) == 1:
            ...
            if isinstance(op, ast.Is):
                ...
                return Call(
                    make_builtin("not"),
                    [Call(make_builtin("string_in_list"), [left, right])],
                )
        raise YaccGrammarError("Unsupported comparison operator in action.", line)
...
def parse_action(
    ...
    ctx: "TypeContext",
    line: int | None = None,
    expected_return_type: TargetType | None = None,
) -> Lambda:
    ...
    dollar_dollar_value: ast.expr | None = None
        ...
        if isinstance(stmt, ast.Assign) and len(stmt.targets) == 1:
            ...
            if isinstance(target, ast.Name) and target.id == "_dollar_dollar":
                if dollar_dollar_value is not None:
                    raise YaccGrammarError(
                        "Multiple assignments to $$ in construct action", line
                    )
                dollar_dollar_value = stmt.value
                continue
        # Side-effect statement (e.g., builtin.start_fragment($1))
        if isinstance(stmt, ast.Expr):
            expr = _convert_expr(stmt.value, ctx, line, {}, param_info, params)
    ...
    body = _convert_expr(dollar_dollar_value, ctx, line, {}, param_info, params)
    ...
    return_type = (
        ...
        else _infer_type(body, line, ctx)
    )
    return Lambda(params, return_type, body)


def parse_deconstruct_action(
    text: str,
    lhs_type: TargetType,
    rhs: "Rhs",
    ctx: "TypeContext",
    ...
    non_literal_indices = [
        i for i, (_, t) in enumerate(rhs_param_info) if t is not None
    ]
    ...
    for stmt in tree.body:
        ...
        if isinstance(stmt, ast.Assign) and len(stmt.targets) == 1:
            ...
            if (
                ...
                and target.id.startswith("_dollar_")
                and target.id[8:].isdigit()
            ):
    ...
    for idx, declared_type in type_annotations.items():
        ...
        if rhs_idx < len(rhs_param_info):
            ...
            if expected_type is not None and not is_subtype(
    ...
    for idx in sorted(assignments.keys()):
        expr = _convert_expr(assignments[idx], ctx, line, extra_vars)
    ...
    for se_node in side_effects:
        se_expr = _convert_expr(se_node, ctx, line, extra_vars)
    ...
    if guard is not None:
        ...
        guard_expr = _convert_expr(guard_tree.body, ctx, line, extra_vars)
...
def parse_action_expr(
    text: str,
    param_info: list[tuple[str | None, TargetType | None]],
    params: list[Var],
    ctx: "TypeContext",
    line: int | None,
    extra_vars: dict[str, TargetType] | None = None,
) -> TargetExpr:
    ...
    try:
        ...
        return _convert_expr(tree.body, ctx, line, extra_vars, param_info, params)
    except SyntaxError:
        pass
    ...
    for stmt in tree.body:
        if isinstance(stmt, ast.Expr):
            exprs.append(
                _convert_expr(stmt.value, ctx, line, extra_vars, param_info, params)
            )
        else:
            raise YaccGrammarError(
                f"Unsupported statement in action block: {type(stmt).__name__}", line
            )
...
def prescan_helper_function_names(
    lines: list[str], start_line: int, ctx: "TypeContext"
) -> None:
...
def parse_helper_functions(
    ...
    try:
        ...
        raise YaccGrammarError(
            f"Syntax error in helper functions: {e}", start_line + (e.lineno or 1)
        )
...
def _convert_function_def(
    node: ast.FunctionDef, ctx: "TypeContext", base_line: int
) -> FunDef:
...
def _convert_stmt(
    stmt: ast.stmt, ctx: "TypeContext", line: int, local_vars: dict[str, TargetType]
) -> TargetExpr:
    """Convert a single Python statement to target IR."""
    if isinstance(stmt, ast.Return):
        if stmt.value is None:
            return Return(Lit(None))
        return Return(_convert_expr(stmt.value, ctx, line, local_vars))

    elif isinstance(stmt, ast.If):
        cond = _convert_expr(stmt.test, ctx, line, local_vars)
        ...
        value = _convert_expr(stmt.value, ctx, line, local_vars)
        ...
        return _convert_expr(stmt.value, ctx, line, local_vars)

    else:
        raise YaccGrammarError(
            f"Unsupported statement type: {type(stmt).__name__}", line
        )


def _make_get_field(
    obj: TargetExpr, field_name: str, ctx: "TypeContext", line: int | None = None
) -> GetField:
    ...
    if not isinstance(obj_type, MessageType):
        raise YaccGrammarError(
            f"Cannot access field '{field_name}' on non-message type: {obj_type}", line
        )
...
class TypeContext:
    ...
    field_type_lookup: Callable[[MessageType, str], TargetType | None] | None = None
