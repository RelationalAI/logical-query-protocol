        ...
        if constructor == "Sequence":
            if len(args) != 1:
                raise YaccGrammarError(
                    f"Sequence type requires exactly one argument: {text}"
                )
            return SequenceType(parse_type(args[0]))
        elif constructor == "List":
            if len(args) != 1:
                raise YaccGrammarError(
                    f"List type requires exactly one argument: {text}"
                )
            return ListType(parse_type(args[0]))
        elif constructor == "Optional":
            if len(args) != 1:
                raise YaccGrammarError(
                    f"Optional type requires exactly one argument: {text}"
                )
            return OptionType(parse_type(args[0]))
        elif constructor == "Tuple":
            if len(args) < 1:
                raise YaccGrammarError(
                    f"Tuple type requires at least one argument: {text}"
                )
...
def _parse_token_pattern(rest: str, token_name: str) -> tuple[str, str, bool]:
    ...
    if not pattern_match:
        raise YaccGrammarError(
            f"%token {token_name} requires a pattern (r'...' or '...')"
        )
    ...
    try:
        ...
        raise YaccGrammarError(
            f"%token {token_name} has invalid pattern {pattern_literal!r}: {e}"
        )

    if not isinstance(pattern, str):
        raise YaccGrammarError(
            f"%token {token_name} pattern must be a string, got {type(pattern).__name__}"
        )
    ...
    while i < len(lines):
        ...
        if line == "%%":
            ...
            if ctx.start_symbol not in ctx.nonterminals:
                raise YaccGrammarError(
                    f"Start symbol '{ctx.start_symbol}' is not a declared nonterminal",
                    i,
                )
        ...
        if line.startswith("%token"):
            ...
            ctx.terminal_info[name] = TerminalInfo(
                parse_type(type_str), pattern, is_regex
            )
            ...
            if not name:
                raise YaccGrammarError(
                    f"Invalid %validator_ignore_completeness directive: {line}", i
                )
...
def parse_rhs_element(text: str, ctx: TypeContext) -> Rhs:
    ...
    if text.endswith("*"):
        ...
        if not isinstance(inner_rhs, (Nonterminal, NamedTerminal)):
            raise YaccGrammarError(
                f"Star inner must be nonterminal or terminal: {text}"
            )
    ...
    if text.endswith("?"):
        ...
        if not isinstance(inner_rhs, (Nonterminal, NamedTerminal)):
            raise YaccGrammarError(
                f"Option inner must be nonterminal or terminal: {text}"
            )
    ...
    # Symbol not declared
    raise YaccGrammarError(
        f"Unknown symbol '{text}': must be declared with %token or %type"
    )
...
def tokenize_rhs(text: str) -> list[str]:
    ...
    while i < len(text):
        ...
        # Handle identifier (possibly with * or ?)
            ...
            raise YaccGrammarError(
                f"Unexpected character in RHS: {text[i]!r} in {text}"
            )
...
def parse_rules(
    ...
    current_lhs_type: TargetType | None = None
    ...
    current_deconstruct_guard: str | None = (
        None  # Guard expression from 'deconstruct if COND:'
    )
    current_alt_start_line: int = 0
    in_action: bool = False
    in_deconstruct: bool = False
    action_base_indent: int = (
        0  # Indentation level of the 'construct:'/'deconstruct:' line
    )
    ...
    def flush_alternative():
        ...
        nonlocal \
            ...
            current_deconstruct_guard, \
            in_action, \
            in_deconstruct
        if (
            current_rhs_lines
            and current_lhs is not None
            and current_lhs_type is not None
        ):
            ...
            rule = _parse_alternative(
                ...
                deconstruct_text,
                ctx,
                current_alt_start_line,
                deconstruct_guard=current_deconstruct_guard,
            )
            rules.append(rule)
        current_rhs_lines = []
        current_action_lines = []
        ...
        if line and (line[0].isalpha() or line[0] == "_"):
            ...
            if ":" in stripped:
                ...
                if current_lhs not in ctx.nonterminals:
                    raise YaccGrammarError(
                        f"Unknown nonterminal: {current_lhs}", line_num
                    )
            ...
            if rest:
                ...
                if " construct:" in rest or rest.endswith(" construct"):
                    raise YaccGrammarError(
                        "'construct:' must be on a separate line from the RHS", line_num
                    )
                current_rhs_lines = [rest]
                current_alt_start_line = line_num
            ...
            if deconstruct_rest.startswith("if "):
                ...
                if not deconstruct_rest.endswith(":"):
                    raise YaccGrammarError(
                        "deconstruct guard must end with ':'", line_num
                    )
                ...
                raise YaccGrammarError(
                    f"Invalid deconstruct syntax: {stripped}", line_num
                )
            ...
            raise YaccGrammarError(
                f"Unexpected line in rules section: {stripped}", line_num
            )
...
def _parse_alternative(
    ...
    ctx: TypeContext,
    line: int,
    deconstruct_guard: str | None = None,
) -> Rule:
    ...
    constructor = parse_action(
        action_text, rhs, ctx, line, expected_return_type=lhs_type
    )
    ...
    if deconstruct_text:
        deconstructor = parse_deconstruct_action(
            deconstruct_text, lhs_type, rhs, ctx, line, guard=deconstruct_guard
        )
    elif deconstruct_guard:
        deconstructor = parse_deconstruct_action(
            "$$", lhs_type, rhs, ctx, line, guard=deconstruct_guard
        )
    else:
...
) -> Callable[[MessageType, str], TargetType | None]:
    ...
    for (module, msg_name), proto_msg in proto_messages.items():
        ...
        for oneof in proto_msg.oneofs:
            for field in oneof.fields:
                field_type = _proto_type_to_target_type(
                    field.type, False, name_to_module=name_to_module
                )
                field_types[(module, msg_name, field.name)] = field_type

    def lookup(message_type: MessageType, field_name: str) -> TargetType | None:
...
def _proto_type_to_target_type(
    proto_type: str,
    is_repeated: bool,
    is_optional: bool = False,
    name_to_module: dict[str, str] | None = None,
) -> TargetType:
...
def _make_enum_lookup(
    proto_enums: dict[str, Any], proto_messages: dict[tuple[str, str], Any]
) -> Callable[[str, str], list[tuple[str, int]] | None]:
...
def load_yacc_grammar(
    ...
    proto_enums: dict[str, Any] | None = None,
) -> GrammarConfig:
    ...
    for line_num, line in enumerate(lines, 1):
        if "\t" in line:
            raise YaccGrammarError(
                "Tabs are not allowed in grammar files. Semantic actions are sensitive to indentation.",
                line_num,
            )
    ...
    if helpers_sep_idx is not None:
        ...
        prescan_helper_function_names(
            helpers_lines, rules_start + helpers_sep_idx + 2, ctx
        )
    ...
    functions = parse_helper_functions(
        helpers_lines, rules_start + helpers_start + 1, ctx
    )
