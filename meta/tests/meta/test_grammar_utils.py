...
from meta.grammar_utils import collect, get_literals, get_nonterminals, rewrite_rule
from meta.target import BaseType, Lambda, ListType, MessageType, OptionType, Var
...
class TestRewriteRhs:
    ...
    def test_rewrite_preserves_lhs_and_constructor(self):
        ...
        constructor = Lambda([param], MessageType("test", "Result"), param)
        rule = Rule(
            lhs=lhs, rhs=nt_old, constructor=constructor, source_type="test.Result"
        )
        result = rewrite_rule(rule, {nt_old: nt_new})
