...
class TestSubstitution:
    ...
    def test_subst_multiple_occurrences_simple(self):
        ...
        expr = Call(
            _test_builtin("concat"), [Var("x", STRING_TYPE), Var("x", STRING_TYPE)]
        )
        result = subst(expr, {"x": Lit("hello")})
        assert isinstance(result, Call)
        assert result.args[0] == Lit("hello")
    ...
    def test_subst_in_ifelse(self):
        ...
        expr = IfElse(Var("cond", BOOLEAN_TYPE), x_var, Lit(0))
...
class TestIsSimpleExpr:
    ...
    def test_call_builtin_equal_is_simple(self):
        ...
        expr = Call(
            _test_builtin("f"), [Var("x", BOOLEAN_TYPE), Var("x", BOOLEAN_TYPE)]
        )
...
class TestCountVarOccurrences:
    ...
    def test_occurrence_in_nested_call(self):
        ...
        expr = Call(
            _test_builtin("f"), [Call(_test_builtin("g"), [Var("x", INT64_TYPE)])]
        )
...
class TestTypeJoin:
    ...
    def test_option_covariant(self):
        assert type_join(OptionType(_NEVER), OptionType(INT64_TYPE)) == OptionType(
            INT64_TYPE
        )
    ...
    def test_dict_covariant_value(self):
        assert type_join(
            DictType(STRING_TYPE, _NEVER), DictType(STRING_TYPE, INT64_TYPE)
        ) == DictType(STRING_TYPE, INT64_TYPE)

    def test_dict_key_mismatch(self):
        with pytest.raises(TypeError, match="key types"):
            type_join(
                DictType(STRING_TYPE, INT64_TYPE), DictType(INT64_TYPE, INT64_TYPE)
            )
