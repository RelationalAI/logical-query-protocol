...
from meta.target_utils import BOOLEAN_TYPE, INT64_TYPE, STRING_TYPE, is_subtype, subst
...
class TestSubstPreservesTyping:
    ...
    def test_ifelse_preserves_branch_type(self):
        ...
        expr = IfElse(Var("cond", BOOLEAN_TYPE), Var("x", INT64_TYPE), Lit(0))
    ...
    def test_multiple_substitutions_preserve_type(self):
        ...
        expr = Call(
            builtin,
            [Var("a", INT64_TYPE), Var("b", STRING_TYPE), Var("c", BOOLEAN_TYPE)],
        )
        result = subst(expr, {"a": Lit(1), "b": Lit("hello"), "c": Lit(True)})
        assert is_subtype(result.target_type(), expr.target_type())
