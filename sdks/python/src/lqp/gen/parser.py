...
class Parser:
    ...
    def _extract_value_int64(self, value: Optional[logic_pb2.Value], default: int) -> int:
        if value is not None:
            assert value is not None
            _t1352 = value.HasField("int_value")
        else:
            _t1352 = False
        if _t1352:
            assert value is not None
    ...
    def _extract_value_string(self, value: Optional[logic_pb2.Value], default: str) -> str:
        if value is not None:
            assert value is not None
            _t1354 = value.HasField("string_value")
        else:
            _t1354 = False
        if _t1354:
            assert value is not None
    ...
    def _extract_value_boolean(self, value: Optional[logic_pb2.Value], default: bool) -> bool:
        if value is not None:
            assert value is not None
            _t1356 = value.HasField("boolean_value")
        else:
            _t1356 = False
        if _t1356:
            assert value is not None
    ...
    def _extract_value_string_list(self, value: Optional[logic_pb2.Value], default: Sequence[str]) -> Sequence[str]:
        if value is not None:
            assert value is not None
            _t1358 = value.HasField("string_value")
        else:
            _t1358 = False
        if _t1358:
            assert value is not None
    ...
    def _try_extract_value_int64(self, value: Optional[logic_pb2.Value]) -> Optional[int]:
        if value is not None:
            assert value is not None
            _t1360 = value.HasField("int_value")
        else:
            _t1360 = False
        if _t1360:
            assert value is not None
    ...
    def _try_extract_value_float64(self, value: Optional[logic_pb2.Value]) -> Optional[float]:
        if value is not None:
            assert value is not None
            _t1362 = value.HasField("float_value")
        else:
            _t1362 = False
        if _t1362:
            assert value is not None
    ...
    def _try_extract_value_bytes(self, value: Optional[logic_pb2.Value]) -> Optional[bytes]:
        if value is not None:
            assert value is not None
            _t1364 = value.HasField("string_value")
        else:
            _t1364 = False
        if _t1364:
            assert value is not None
    ...
    def _try_extract_value_uint128(self, value: Optional[logic_pb2.Value]) -> Optional[logic_pb2.UInt128Value]:
        if value is not None:
            assert value is not None
            _t1366 = value.HasField("uint128_value")
        else:
            _t1366 = False
        if _t1366:
            assert value is not None
    ...
    def construct_csv_config(self, config_dict: Sequence[tuple[str, logic_pb2.Value]]) -> logic_pb2.CSVConfig:
        ...
        _t1368 = self._extract_value_int32(config.get("csv_header_row"), 1)
        header_row = _t1368
        _t1369 = self._extract_value_int64(config.get("csv_skip"), 0)
        skip = _t1369
        _t1370 = self._extract_value_string(config.get("csv_new_line"), "")
        new_line = _t1370
        _t1371 = self._extract_value_string(config.get("csv_delimiter"), ",")
        delimiter = _t1371
        _t1372 = self._extract_value_string(config.get("csv_quotechar"), '"')
        quotechar = _t1372
        _t1373 = self._extract_value_string(config.get("csv_escapechar"), '"')
        escapechar = _t1373
        _t1374 = self._extract_value_string(config.get("csv_comment"), "")
        comment = _t1374
        _t1375 = self._extract_value_string_list(config.get("csv_missing_strings"), [])
        missing_strings = _t1375
        _t1376 = self._extract_value_string(config.get("csv_decimal_separator"), ".")
        decimal_separator = _t1376
        _t1377 = self._extract_value_string(config.get("csv_encoding"), "utf-8")
        encoding = _t1377
        _t1378 = self._extract_value_string(config.get("csv_compression"), "auto")
        compression = _t1378
        _t1379 = self._extract_value_int64(config.get("csv_partition_size_mb"), 0)
        partition_size_mb = _t1379
        _t1380 = logic_pb2.CSVConfig(header_row=header_row, skip=skip, new_line=new_line, delimiter=delimiter, quotechar=quotechar, escapechar=escapechar, comment=comment, missing_strings=missing_strings, decimal_separator=decimal_separator, encoding=encoding, compression=compression, partition_size_mb=partition_size_mb)
        return _t1380
    ...
    def construct_betree_info(self, key_types: Sequence[logic_pb2.Type], value_types: Sequence[logic_pb2.Type], config_dict: Sequence[tuple[str, logic_pb2.Value]]) -> logic_pb2.BeTreeInfo:
        config = dict(config_dict)
        _t1381 = self._try_extract_value_float64(config.get("betree_config_epsilon"))
        epsilon = _t1381
        _t1382 = self._try_extract_value_int64(config.get("betree_config_max_pivots"))
        max_pivots = _t1382
        _t1383 = self._try_extract_value_int64(config.get("betree_config_max_deltas"))
        max_deltas = _t1383
        _t1384 = self._try_extract_value_int64(config.get("betree_config_max_leaf"))
        max_leaf = _t1384
        _t1385 = logic_pb2.BeTreeConfig(epsilon=epsilon, max_pivots=max_pivots, max_deltas=max_deltas, max_leaf=max_leaf)
        storage_config = _t1385
        _t1386 = self._try_extract_value_uint128(config.get("betree_locator_root_pageid"))
        root_pageid = _t1386
        _t1387 = self._try_extract_value_bytes(config.get("betree_locator_inline_data"))
        inline_data = _t1387
        _t1388 = self._try_extract_value_int64(config.get("betree_locator_element_count"))
        element_count = _t1388
        _t1389 = self._try_extract_value_int64(config.get("betree_locator_tree_height"))
        tree_height = _t1389
        _t1390 = logic_pb2.BeTreeLocator(root_pageid=root_pageid, inline_data=inline_data, element_count=element_count, tree_height=tree_height)
        relation_locator = _t1390
        _t1391 = logic_pb2.BeTreeInfo(key_types=key_types, value_types=value_types, storage_config=storage_config, relation_locator=relation_locator)
        return _t1391

    def default_configure(self) -> transactions_pb2.Configure:
        _t1392 = transactions_pb2.IVMConfig(level=transactions_pb2.MaintenanceLevel.MAINTENANCE_LEVEL_OFF)
        ivm_config = _t1392
        _t1393 = transactions_pb2.Configure(semantics_version=0, ivm_config=ivm_config)
        return _t1393
            ...
            if maintenance_level_val.string_value == "off":
                ...
                if maintenance_level_val.string_value == "auto":
                    ...
                    if maintenance_level_val.string_value == "all":
                        ...
                        maintenance_level = transactions_pb2.MaintenanceLevel.MAINTENANCE_LEVEL_OFF
        _t1394 = transactions_pb2.IVMConfig(level=maintenance_level)
        ivm_config = _t1394
        _t1395 = self._extract_value_int64(config.get("semantics_version"), 0)
        semantics_version = _t1395
        _t1396 = transactions_pb2.Configure(semantics_version=semantics_version, ivm_config=ivm_config)
        return _t1396
    ...
    def construct_export_csv_config(self, path: str, columns: Sequence[transactions_pb2.ExportCSVColumn], config_dict: Sequence[tuple[str, logic_pb2.Value]]) -> transactions_pb2.ExportCSVConfig:
        config = dict(config_dict)
        _t1397 = self._extract_value_int64(config.get("partition_size"), 0)
        partition_size = _t1397
        _t1398 = self._extract_value_string(config.get("compression"), "")
        compression = _t1398
        _t1399 = self._extract_value_boolean(config.get("syntax_header_row"), True)
        syntax_header_row = _t1399
        _t1400 = self._extract_value_string(config.get("syntax_missing_string"), "")
        syntax_missing_string = _t1400
        _t1401 = self._extract_value_string(config.get("syntax_delim"), ",")
        syntax_delim = _t1401
        _t1402 = self._extract_value_string(config.get("syntax_quotechar"), '"')
        syntax_quotechar = _t1402
        _t1403 = self._extract_value_string(config.get("syntax_escapechar"), "\\")
        syntax_escapechar = _t1403
        _t1404 = transactions_pb2.ExportCSVConfig(path=path, data_columns=columns, partition_size=partition_size, compression=compression, syntax_header_row=syntax_header_row, syntax_missing_string=syntax_missing_string, syntax_delim=syntax_delim, syntax_quotechar=syntax_quotechar, syntax_escapechar=syntax_escapechar)
        return _t1404
    ...
    def construct_export_csv_config_with_source(self, path: str, csv_source: transactions_pb2.ExportCSVSource, csv_config: logic_pb2.CSVConfig) -> transactions_pb2.ExportCSVConfig:
        _t1405 = transactions_pb2.ExportCSVConfig(path=path, csv_source=csv_source, csv_config=csv_config)
        return _t1405
    ...
    def parse_data(self) -> logic_pb2.Data:
        ...
        if prediction636 == 2:
            ...
            if prediction636 == 1:
                ...
                if prediction636 == 0:
                    _t1249 = self.parse_rel_edb()
                    rel_edb637 = _t1249
                    _t1250 = logic_pb2.Data(rel_edb=rel_edb637)
                    _t1248 = _t1250
                else:
    ...
    def parse_rel_edb(self) -> logic_pb2.RelEDB:
        ...
        _t1252 = self.parse_rel_edb_path()
        rel_edb_path641 = _t1252
        _t1253 = self.parse_rel_edb_types()
        rel_edb_types642 = _t1253
        self.consume_literal(")")
        _t1254 = logic_pb2.RelEDB(target_id=relation_id640, path=rel_edb_path641, types=rel_edb_types642)
        return _t1254
    ...
    def parse_csv_data(self) -> logic_pb2.CSVData:
        ...
        _t1267 = self.parse_csv_columns()
        csv_columns666 = _t1267
        _t1268 = self.parse_csv_asof()
        csv_asof667 = _t1268
        self.consume_literal(")")
        _t1269 = logic_pb2.CSVData(locator=csvlocator664, config=csv_config665, columns=csv_columns666, asof=csv_asof667)
        return _t1269
    ...
    def parse_csv_column(self) -> logic_pb2.CSVColumn:
        ...
        self.consume_literal("]")
        self.consume_literal(")")
        _t1280 = logic_pb2.CSVColumn(column_name=string680, target_id=relation_id681, types=types685)
        return _t1280

    def parse_csv_asof(self) -> str:
        self.consume_literal("(")
        self.consume_literal("asof")
        string686 = self.consume_terminal("STRING")
        self.consume_literal(")")
        return string686
    ...
    def parse_undefine(self) -> transactions_pb2.Undefine:
        ...
        _t1281 = self.parse_fragment_id()
        fragment_id687 = _t1281
        self.consume_literal(")")
        _t1282 = transactions_pb2.Undefine(fragment_id=fragment_id687)
        return _t1282
    ...
    def parse_context(self) -> transactions_pb2.Context:
        ...
        xs688 = []
        cond689 = (self.match_lookahead_literal(":", 0) or self.match_lookahead_terminal("UINT128", 0))
        while cond689:
            _t1283 = self.parse_relation_id()
            item690 = _t1283
            xs688.append(item690)
            cond689 = (self.match_lookahead_literal(":", 0) or self.match_lookahead_terminal("UINT128", 0))
        relation_ids691 = xs688
        self.consume_literal(")")
        _t1284 = transactions_pb2.Context(relations=relation_ids691)
        return _t1284

    def parse_snapshot(self) -> transactions_pb2.Snapshot:
        self.consume_literal("(")
        self.consume_literal("snapshot")
        _t1285 = self.parse_rel_edb_path()
        rel_edb_path692 = _t1285
        _t1286 = self.parse_relation_id()
        relation_id693 = _t1286
        self.consume_literal(")")
        _t1287 = transactions_pb2.Snapshot(destination_path=rel_edb_path692, source_relation=relation_id693)
        return _t1287
    ...
    def parse_epoch_reads(self) -> Sequence[transactions_pb2.Read]:
        ...
        xs694 = []
        cond695 = self.match_lookahead_literal("(", 0)
        while cond695:
            _t1288 = self.parse_read()
            item696 = _t1288
    ...
    def parse_read(self) -> transactions_pb2.Read:
        ...
        if prediction698 == 4:
            _t1296 = self.parse_export()
            export703 = _t1296
            _t1297 = transactions_pb2.Read(export=export703)
            _t1295 = _t1297
        else:
            if prediction698 == 3:
                _t1299 = self.parse_abort()
                abort702 = _t1299
                _t1300 = transactions_pb2.Read(abort=abort702)
                _t1298 = _t1300
            else:
                if prediction698 == 2:
                    _t1302 = self.parse_what_if()
                    what_if701 = _t1302
                    _t1303 = transactions_pb2.Read(what_if=what_if701)
                    _t1301 = _t1303
                else:
                    if prediction698 == 1:
                        _t1305 = self.parse_output()
                        output700 = _t1305
                        _t1306 = transactions_pb2.Read(output=output700)
                        _t1304 = _t1306
                    else:
                        if prediction698 == 0:
                            _t1308 = self.parse_demand()
                            demand699 = _t1308
                            _t1309 = transactions_pb2.Read(demand=demand699)
                            _t1307 = _t1309
                        else:
    ...
    def parse_demand(self) -> transactions_pb2.Demand:
        ...
        _t1310 = self.parse_relation_id()
        relation_id704 = _t1310
        self.consume_literal(")")
        _t1311 = transactions_pb2.Demand(relation_id=relation_id704)
        return _t1311

    def parse_output(self) -> transactions_pb2.Output:
        self.consume_literal("(")
        self.consume_literal("output")
        _t1312 = self.parse_name()
        name705 = _t1312
        _t1313 = self.parse_relation_id()
        relation_id706 = _t1313
        self.consume_literal(")")
        _t1314 = transactions_pb2.Output(name=name705, relation_id=relation_id706)
        return _t1314

    def parse_what_if(self) -> transactions_pb2.WhatIf:
        self.consume_literal("(")
        self.consume_literal("what_if")
        _t1315 = self.parse_name()
        name707 = _t1315
        _t1316 = self.parse_epoch()
        epoch708 = _t1316
        self.consume_literal(")")
        _t1317 = transactions_pb2.WhatIf(branch=name707, epoch=epoch708)
        return _t1317
    ...
    def parse_abort(self) -> transactions_pb2.Abort:
        ...
        self.consume_literal(")")
        _t1321 = transactions_pb2.Abort(name=(name709 if name709 is not None else "abort"), relation_id=relation_id710)
        return _t1321

    def parse_export(self) -> transactions_pb2.Export:
        self.consume_literal("(")
        self.consume_literal("export")
        _t1322 = self.parse_export_csv_config()
        export_csv_config711 = _t1322
        self.consume_literal(")")
        _t1323 = transactions_pb2.Export(csv_config=export_csv_config711)
        return _t1323
    ...
    def parse_export_csv_config(self) -> transactions_pb2.ExportCSVConfig:
        ...
        if prediction712 == 1:
            ...
            if prediction712 == 0:
                ...
                _t1333 = self.parse_export_csv_path()
                export_csv_path713 = _t1333
                _t1334 = self.parse_export_csv_source()
                export_csv_source714 = _t1334
                _t1335 = self.parse_csv_config()
                csv_config715 = _t1335
                self.consume_literal(")")
                _t1336 = self.construct_export_csv_config_with_source(export_csv_path713, export_csv_source714, csv_config715)
                _t1332 = _t1336
            else:
    ...
    def parse_export_csv_path(self) -> str:
        ...
        string719 = self.consume_terminal("STRING")
        self.consume_literal(")")
        return string719
    ...
    def parse_export_csv_source(self) -> transactions_pb2.ExportCSVSource:
        ...
        if prediction720 == 1:
            ...
            self.consume_literal(")")
            _t1342 = transactions_pb2.ExportCSVSource(table_def=relation_id725)
            _t1340 = _t1342
        else:
            if prediction720 == 0:
                self.consume_literal("(")
                ...
                while cond722:
                    _t1344 = self.parse_export_csv_column()
                    item723 = _t1344
                    xs721.append(item723)
                    cond722 = self.match_lookahead_literal("(", 0)
                export_csv_columns724 = xs721
                self.consume_literal(")")
                _t1345 = transactions_pb2.ExportCSVColumns(columns=export_csv_columns724)
                _t1346 = transactions_pb2.ExportCSVSource(gnf_columns=_t1345)
                _t1343 = _t1346
            else:
    ...
    def parse_export_csv_column(self) -> transactions_pb2.ExportCSVColumn:
        self.consume_literal("(")
        self.consume_literal("column")
        string726 = self.consume_terminal("STRING")
        _t1347 = self.parse_relation_id()
    ...
    def parse_export_csv_columns(self) -> Sequence[transactions_pb2.ExportCSVColumn]:
        ...
        self.consume_literal("columns")
        xs728 = []
        cond729 = self.match_lookahead_literal("(", 0)
        while cond729:
            _t1349 = self.parse_export_csv_column()
            item730 = _t1349
            xs728.append(item730)
            cond729 = self.match_lookahead_literal("(", 0)
