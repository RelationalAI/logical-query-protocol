...
class Parser:
            ...
            if maintenance_level_val.string_value == 'off':
                ...
                if maintenance_level_val.string_value == 'auto':
                    ...
                    if maintenance_level_val.string_value == 'all':
                        ...
                        maintenance_level = transactions_pb2.MaintenanceLevel.MAINTENANCE_LEVEL_OFF
        _t959 = transactions_pb2.IVMConfig(level=maintenance_level)
        ivm_config = _t959
        _t960 = self._extract_value_int64(config.get('semantics_version'), 0)
        semantics_version = _t960
        _t961 = transactions_pb2.Configure(semantics_version=semantics_version, ivm_config=ivm_config)
        return _t961
    ...
    def default_configure(self) -> transactions_pb2.Configure:
        _t964 = transactions_pb2.IVMConfig(level=transactions_pb2.MaintenanceLevel.MAINTENANCE_LEVEL_OFF)
        ivm_config = _t964
        _t965 = transactions_pb2.Configure(semantics_version=0, ivm_config=ivm_config)
        return _t965
    ...
    def _extract_value_boolean(self, value: Optional[logic_pb2.Value], default: bool) -> bool:
        ...
        if value is not None:
            assert value is not None
            _t966 = value.HasField('boolean_value')
        else:
            _t966 = False
        if _t966:
            assert value is not None
    ...
    def _try_extract_value_float64(self, value: Optional[logic_pb2.Value]) -> Optional[float]:
        ...
        if value is not None:
            assert value is not None
            _t977 = value.HasField('float_value')
        else:
            _t977 = False
        if _t977:
            assert value is not None
