...
class Parser:
    ...
    def construct_csv_config(self, config_dict: Sequence[tuple[str, logic_pb2.Value]]) -> logic_pb2.CSVConfig:
        ...
        skip = _t946
        _t947 = self._extract_value_string(config.get('csv_new_line'), '')
        new_line = _t947
        _t948 = self._extract_value_string(config.get('csv_delimiter'), ',')
        delimiter = _t948
        _t949 = self._extract_value_string(config.get('csv_quotechar'), '"')
        quotechar = _t949
        _t950 = self._extract_value_string(config.get('csv_escapechar'), '"')
        escapechar = _t950
        _t951 = self._extract_value_string(config.get('csv_comment'), '')
        comment = _t951
        _t952 = self._extract_value_string_list(config.get('csv_missing_strings'), [])
        missing_strings = _t952
        _t953 = self._extract_value_string(config.get('csv_decimal_separator'), '.')
        decimal_separator = _t953
        _t954 = self._extract_value_string(config.get('csv_encoding'), 'utf-8')
        encoding = _t954
    ...
    def construct_configure(self, config_dict: Sequence[tuple[str, logic_pb2.Value]]) -> transactions_pb2.Configure:
        ...
        _t960 = transactions_pb2.Configure(semantics_version=semantics_version, ivm_config=ivm_config)
        return _t960
    ...
    def _extract_value_int32(self, value: Optional[logic_pb2.Value], default: int) -> int:
        ...
        if value is not None:
            assert value is not None
            _t962 = value.HasField('int_value')
        else:
            _t962 = False
        if _t962:
            assert value is not None
    ...
    def _extract_value_string_list(self, value: Optional[logic_pb2.Value], default: Sequence[str]) -> Sequence[str]:
        ...
        if value is not None:
            assert value is not None
            _t963 = value.HasField('string_value')
        else:
            _t963 = False
        if _t963:
            assert value is not None
    ...
    def export_csv_config(self, path: str, columns: Sequence[transactions_pb2.ExportCSVColumn], config_dict: Sequence[tuple[str, logic_pb2.Value]]) -> transactions_pb2.ExportCSVConfig:
        config = dict(config_dict)
        _t964 = self._extract_value_int64(config.get('partition_size'), 0)
        partition_size = _t964
        _t965 = self._extract_value_string(config.get('compression'), '')
        compression = _t965
        _t966 = self._extract_value_boolean(config.get('syntax_header_row'), True)
        syntax_header_row = _t966
        _t967 = self._extract_value_string(config.get('syntax_missing_string'), '')
        syntax_missing_string = _t967
        _t968 = self._extract_value_string(config.get('syntax_delim'), ',')
        syntax_delim = _t968
        _t969 = self._extract_value_string(config.get('syntax_quotechar'), '"')
        syntax_quotechar = _t969
        _t970 = self._extract_value_string(config.get('syntax_escapechar'), '\\')
        syntax_escapechar = _t970
        _t971 = transactions_pb2.ExportCSVConfig(path=path, data_columns=columns, partition_size=partition_size, compression=compression, syntax_header_row=syntax_header_row, syntax_missing_string=syntax_missing_string, syntax_delim=syntax_delim, syntax_quotechar=syntax_quotechar, syntax_escapechar=syntax_escapechar)
        return _t971
    ...
    def construct_betree_info(self, key_types: Sequence[logic_pb2.Type], value_types: Sequence[logic_pb2.Type], config_dict: Sequence[tuple[str, logic_pb2.Value]]) -> logic_pb2.BeTreeInfo:
        config = dict(config_dict)
        _t973 = self._try_extract_value_float64(config.get('betree_config_epsilon'))
        epsilon = _t973
        _t974 = self._try_extract_value_int64(config.get('betree_config_max_pivots'))
        max_pivots = _t974
        _t975 = self._try_extract_value_int64(config.get('betree_config_max_deltas'))
        max_deltas = _t975
        _t976 = self._try_extract_value_int64(config.get('betree_config_max_leaf'))
        max_leaf = _t976
        _t977 = logic_pb2.BeTreeConfig(epsilon=epsilon, max_pivots=max_pivots, max_deltas=max_deltas, max_leaf=max_leaf)
        storage_config = _t977
        _t978 = self._try_extract_value_uint128(config.get('betree_locator_root_pageid'))
        root_pageid = _t978
        _t979 = self._try_extract_value_bytes(config.get('betree_locator_inline_data'))
        inline_data = _t979
        _t980 = self._try_extract_value_int64(config.get('betree_locator_element_count'))
        element_count = _t980
        _t981 = self._try_extract_value_int64(config.get('betree_locator_tree_height'))
        tree_height = _t981
        _t982 = logic_pb2.BeTreeLocator(root_pageid=root_pageid, inline_data=inline_data, element_count=element_count, tree_height=tree_height)
        relation_locator = _t982
        _t983 = logic_pb2.BeTreeInfo(key_types=key_types, value_types=value_types, storage_config=storage_config, relation_locator=relation_locator)
        return _t983
    ...
    def _try_extract_value_uint128(self, value: Optional[logic_pb2.Value]) -> Optional[logic_pb2.UInt128Value]:
        ...
        if value is not None:
            assert value is not None
            _t985 = value.HasField('uint128_value')
        else:
            _t985 = False
        if _t985:
            assert value is not None
    ...
    def default_configure(self) -> transactions_pb2.Configure:
        _t987 = transactions_pb2.IVMConfig(level=transactions_pb2.MaintenanceLevel.MAINTENANCE_LEVEL_OFF)
        ivm_config = _t987
        _t988 = transactions_pb2.Configure(semantics_version=0, ivm_config=ivm_config)
        return _t988
