...
class Parser:
    ...
    def construct_betree_info(
        ...
        _t945 = self._try_extract_value_float64(config.get("betree_config_epsilon"))
        epsilon = _t945
        _t946 = self._try_extract_value_int64(config.get("betree_config_max_pivots"))
        max_pivots = _t946
        _t947 = self._try_extract_value_int64(config.get("betree_config_max_deltas"))
        max_deltas = _t947
        _t948 = self._try_extract_value_int64(config.get("betree_config_max_leaf"))
        max_leaf = _t948
        _t949 = logic_pb2.BeTreeConfig(
            epsilon=epsilon,
            max_pivots=max_pivots,
            max_deltas=max_deltas,
            max_leaf=max_leaf,
        )
        storage_config = _t949
        _t950 = self._try_extract_value_uint128(
            config.get("betree_locator_root_pageid")
        )
        root_pageid = _t950
        _t951 = self._try_extract_value_bytes(config.get("betree_locator_inline_data"))
        inline_data = _t951
        _t952 = self._try_extract_value_int64(
            config.get("betree_locator_element_count")
        )
        element_count = _t952
        _t953 = self._try_extract_value_int64(config.get("betree_locator_tree_height"))
        tree_height = _t953
        _t954 = logic_pb2.BeTreeLocator(
            root_pageid=root_pageid,
            inline_data=inline_data,
            element_count=element_count,
            tree_height=tree_height,
        )
        relation_locator = _t954
        _t955 = logic_pb2.BeTreeInfo(
            key_types=key_types,
    ...
    def construct_csv_config(
        ...
        config = dict(config_dict)
        _t956 = self._extract_value_int32(config.get("csv_header_row"), 1)
        header_row = _t956
        _t957 = self._extract_value_int64(config.get("csv_skip"), 0)
        skip = _t957
        _t958 = self._extract_value_string(config.get("csv_new_line"), "")
        new_line = _t958
        _t959 = self._extract_value_string(config.get("csv_delimiter"), ",")
        delimiter = _t959
        _t960 = self._extract_value_string(config.get("csv_quotechar"), '"')
        quotechar = _t960
        _t961 = self._extract_value_string(config.get("csv_escapechar"), '"')
        escapechar = _t961
        _t962 = self._extract_value_string(config.get("csv_comment"), "")
        comment = _t962
        _t963 = self._extract_value_string_list(config.get("csv_missing_strings"), [])
        missing_strings = _t963
        _t964 = self._extract_value_string(config.get("csv_decimal_separator"), ".")
        decimal_separator = _t964
        _t965 = self._extract_value_string(config.get("csv_encoding"), "utf-8")
        encoding = _t965
        _t966 = self._extract_value_string(config.get("csv_compression"), "auto")
        compression = _t966
        _t967 = logic_pb2.CSVConfig(
    ...
    def _extract_value_int64(
        ...
        if value is not None:
            ...
            _t968 = value.HasField("int_value")
        else:
            _t968 = False
        if _t968:
            assert value is not None
    ...
    def _try_extract_value_uint128(
        ...
        if value is not None:
            assert value is not None
            _t969 = value.HasField("uint128_value")
        else:
            _t969 = False
        if _t969:
            assert value is not None
    ...
    def _extract_value_boolean(
        ...
        if value is not None:
            assert value is not None
            _t971 = value.HasField("boolean_value")
        else:
            _t971 = False
        if _t971:
            assert value is not None
    ...
    def default_configure(self) -> transactions_pb2.Configure:
        _t973 = transactions_pb2.IVMConfig(
            level=transactions_pb2.MaintenanceLevel.MAINTENANCE_LEVEL_OFF
        )
        ivm_config = _t973
        _t974 = transactions_pb2.Configure(semantics_version=0, ivm_config=ivm_config)
        return _t974
    ...
    def _extract_value_int32(
        ...
        if value is not None:
            assert value is not None
            _t975 = value.HasField("int_value")
        else:
            _t975 = False
        if _t975:
            assert value is not None
    ...
    def _try_extract_value_int64(
        ...
        if value is not None:
            assert value is not None
            _t977 = value.HasField("int_value")
        else:
            _t977 = False
        if _t977:
            assert value is not None
    ...
    def _try_extract_value_bytes(
        ...
        if value is not None:
            assert value is not None
            _t978 = value.HasField("string_value")
        else:
            _t978 = False
        if _t978:
            assert value is not None
    ...
    def construct_configure(
        ...
        _t979 = transactions_pb2.IVMConfig(level=maintenance_level)
        ivm_config = _t979
        _t980 = self._extract_value_int64(config.get("semantics_version"), 0)
        semantics_version = _t980
        _t981 = transactions_pb2.Configure(
    ...
    def export_csv_config(
        ...
        config = dict(config_dict)
        _t982 = self._extract_value_int64(config.get("partition_size"), 0)
        partition_size = _t982
        _t983 = self._extract_value_string(config.get("compression"), "")
        compression = _t983
        _t984 = self._extract_value_boolean(config.get("syntax_header_row"), True)
        syntax_header_row = _t984
        _t985 = self._extract_value_string(config.get("syntax_missing_string"), "")
        syntax_missing_string = _t985
        _t986 = self._extract_value_string(config.get("syntax_delim"), ",")
        syntax_delim = _t986
        _t987 = self._extract_value_string(config.get("syntax_quotechar"), '"')
        syntax_quotechar = _t987
        _t988 = self._extract_value_string(config.get("syntax_escapechar"), "\\")
        syntax_escapechar = _t988
        _t989 = transactions_pb2.ExportCSVConfig(
            path=path,
