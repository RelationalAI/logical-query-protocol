...
class PrettyPrinter:
    ...
    def deconstruct_configure(self, msg: transactions_pb2.Configure) -> list[tuple[str, logic_pb2.Value]]:
        ...
        if msg.ivm_config.level == transactions_pb2.MaintenanceLevel.MAINTENANCE_LEVEL_AUTO:
            _t1259 = self._make_value_string("auto")
            result.append(("ivm.maintenance_level", _t1259,))
        else:
            if msg.ivm_config.level == transactions_pb2.MaintenanceLevel.MAINTENANCE_LEVEL_ALL:
                _t1260 = self._make_value_string("all")
                result.append(("ivm.maintenance_level", _t1260,))
            else:
                if msg.ivm_config.level == transactions_pb2.MaintenanceLevel.MAINTENANCE_LEVEL_OFF:
                    _t1261 = self._make_value_string("off")
                    result.append(("ivm.maintenance_level", _t1261,))
        _t1262 = self._make_value_int64(msg.semantics_version)
        result.append(("semantics_version", _t1262,))
        return sorted(result)

    def deconstruct_csv_config(self, msg: logic_pb2.CSVConfig) -> list[tuple[str, logic_pb2.Value]]:
        result = []
        _t1263 = self._make_value_int32(msg.header_row)
        result.append(("csv_header_row", _t1263,))
        _t1264 = self._make_value_int64(msg.skip)
        result.append(("csv_skip", _t1264,))
        if msg.new_line != "":
            _t1265 = self._make_value_string(msg.new_line)
            result.append(("csv_new_line", _t1265,))
        _t1266 = self._make_value_string(msg.delimiter)
        result.append(("csv_delimiter", _t1266,))
        _t1267 = self._make_value_string(msg.quotechar)
        result.append(("csv_quotechar", _t1267,))
        _t1268 = self._make_value_string(msg.escapechar)
        result.append(("csv_escapechar", _t1268,))
        if msg.comment != "":
            _t1269 = self._make_value_string(msg.comment)
            result.append(("csv_comment", _t1269,))
        for missing_string in msg.missing_strings:
            _t1270 = self._make_value_string(missing_string)
            result.append(("csv_missing_strings", _t1270,))
        _t1271 = self._make_value_string(msg.decimal_separator)
        result.append(("csv_decimal_separator", _t1271,))
        _t1272 = self._make_value_string(msg.encoding)
        result.append(("csv_encoding", _t1272,))
        _t1273 = self._make_value_string(msg.compression)
        result.append(("csv_compression", _t1273,))
        return sorted(result)

    def deconstruct_betree_info_config(self, msg: logic_pb2.BeTreeInfo) -> list[tuple[str, logic_pb2.Value]]:
        result = []
        _t1274 = self._make_value_float64(msg.storage_config.epsilon)
        result.append(("betree_config_epsilon", _t1274,))
        _t1275 = self._make_value_int64(msg.storage_config.max_pivots)
        result.append(("betree_config_max_pivots", _t1275,))
        _t1276 = self._make_value_int64(msg.storage_config.max_deltas)
        result.append(("betree_config_max_deltas", _t1276,))
        _t1277 = self._make_value_int64(msg.storage_config.max_leaf)
        result.append(("betree_config_max_leaf", _t1277,))
        if msg.relation_locator.HasField("root_pageid"):
            if msg.relation_locator.root_pageid is not None:
                ...
                _t1278 = self._make_value_uint128(msg.relation_locator.root_pageid)
                result.append(("betree_locator_root_pageid", _t1278,))
        if msg.relation_locator.HasField("inline_data"):
            if msg.relation_locator.inline_data is not None:
                ...
                _t1279 = self._make_value_string(msg.relation_locator.inline_data.decode('utf-8'))
                result.append(("betree_locator_inline_data", _t1279,))
        _t1280 = self._make_value_int64(msg.relation_locator.element_count)
        result.append(("betree_locator_element_count", _t1280,))
        _t1281 = self._make_value_int64(msg.relation_locator.tree_height)
        result.append(("betree_locator_tree_height", _t1281,))
        return sorted(result)
    ...
    def deconstruct_export_csv_config(self, msg: transactions_pb2.ExportCSVConfig) -> list[tuple[str, logic_pb2.Value]]:
        ...
        if msg.partition_size is not None:
            ...
            _t1282 = self._make_value_int64(msg.partition_size)
            result.append(("partition_size", _t1282,))
        if msg.compression is not None:
            assert msg.compression is not None
            _t1283 = self._make_value_string(msg.compression)
            result.append(("compression", _t1283,))
        if msg.syntax_header_row is not None:
            assert msg.syntax_header_row is not None
            _t1284 = self._make_value_boolean(msg.syntax_header_row)
            result.append(("syntax_header_row", _t1284,))
        if msg.syntax_missing_string is not None:
            assert msg.syntax_missing_string is not None
            _t1285 = self._make_value_string(msg.syntax_missing_string)
            result.append(("syntax_missing_string", _t1285,))
        if msg.syntax_delim is not None:
            assert msg.syntax_delim is not None
            _t1286 = self._make_value_string(msg.syntax_delim)
            result.append(("syntax_delim", _t1286,))
        if msg.syntax_quotechar is not None:
            assert msg.syntax_quotechar is not None
            _t1287 = self._make_value_string(msg.syntax_quotechar)
            result.append(("syntax_quotechar", _t1287,))
        if msg.syntax_escapechar is not None:
            assert msg.syntax_escapechar is not None
            _t1288 = self._make_value_string(msg.syntax_escapechar)
            result.append(("syntax_escapechar", _t1288,))
        return sorted(result)
