...
class PrettyPrinter:
    ...
    def deconstruct_configure(self, msg: transactions_pb2.Configure) -> list[tuple[str, logic_pb2.Value]]:
        ...
        if msg.ivm_config.level == transactions_pb2.MaintenanceLevel.MAINTENANCE_LEVEL_AUTO:
            _t1759 = self._make_value_string("auto")
            ...
            if msg.ivm_config.level == transactions_pb2.MaintenanceLevel.MAINTENANCE_LEVEL_ALL:
                _t1760 = self._make_value_string("all")
                result.append(("ivm.maintenance_level", _t1760,))
            else:
                if msg.ivm_config.level == transactions_pb2.MaintenanceLevel.MAINTENANCE_LEVEL_OFF:
                    _t1761 = self._make_value_string("off")
                    result.append(("ivm.maintenance_level", _t1761,))
        _t1762 = self._make_value_int64(msg.semantics_version)
        result.append(("semantics_version", _t1762,))
        return sorted(result)

    def deconstruct_csv_config(self, msg: logic_pb2.CSVConfig) -> list[tuple[str, logic_pb2.Value]]:
        result = []
        _t1763 = self._make_value_int32(msg.header_row)
        result.append(("csv_header_row", _t1763,))
        _t1764 = self._make_value_int64(msg.skip)
        result.append(("csv_skip", _t1764,))
        if msg.new_line != "":
            _t1765 = self._make_value_string(msg.new_line)
            result.append(("csv_new_line", _t1765,))
        _t1766 = self._make_value_string(msg.delimiter)
        result.append(("csv_delimiter", _t1766,))
        _t1767 = self._make_value_string(msg.quotechar)
        result.append(("csv_quotechar", _t1767,))
        _t1768 = self._make_value_string(msg.escapechar)
        result.append(("csv_escapechar", _t1768,))
        if msg.comment != "":
            _t1769 = self._make_value_string(msg.comment)
            result.append(("csv_comment", _t1769,))
        for missing_string in msg.missing_strings:
            _t1770 = self._make_value_string(missing_string)
            result.append(("csv_missing_strings", _t1770,))
        _t1771 = self._make_value_string(msg.decimal_separator)
        result.append(("csv_decimal_separator", _t1771,))
        _t1772 = self._make_value_string(msg.encoding)
        result.append(("csv_encoding", _t1772,))
        _t1773 = self._make_value_string(msg.compression)
        result.append(("csv_compression", _t1773,))
        if msg.partition_size_mb != 0:
            _t1774 = self._make_value_int64(msg.partition_size_mb)
            result.append(("csv_partition_size_mb", _t1774,))
        return sorted(result)

    def deconstruct_betree_info_config(self, msg: logic_pb2.BeTreeInfo) -> list[tuple[str, logic_pb2.Value]]:
        result = []
        _t1775 = self._make_value_float64(msg.storage_config.epsilon)
        result.append(("betree_config_epsilon", _t1775,))
        _t1776 = self._make_value_int64(msg.storage_config.max_pivots)
        result.append(("betree_config_max_pivots", _t1776,))
        _t1777 = self._make_value_int64(msg.storage_config.max_deltas)
        result.append(("betree_config_max_deltas", _t1777,))
        _t1778 = self._make_value_int64(msg.storage_config.max_leaf)
        result.append(("betree_config_max_leaf", _t1778,))
        if msg.relation_locator.HasField("root_pageid"):
            if msg.relation_locator.root_pageid is not None:
                ...
                _t1779 = self._make_value_uint128(msg.relation_locator.root_pageid)
                result.append(("betree_locator_root_pageid", _t1779,))
        if msg.relation_locator.HasField("inline_data"):
            if msg.relation_locator.inline_data is not None:
                ...
                _t1780 = self._make_value_string(msg.relation_locator.inline_data.decode('utf-8'))
                result.append(("betree_locator_inline_data", _t1780,))
        _t1781 = self._make_value_int64(msg.relation_locator.element_count)
        result.append(("betree_locator_element_count", _t1781,))
        _t1782 = self._make_value_int64(msg.relation_locator.tree_height)
        result.append(("betree_locator_tree_height", _t1782,))
        return sorted(result)
    ...
    def deconstruct_export_csv_config(self, msg: transactions_pb2.ExportCSVConfig) -> list[tuple[str, logic_pb2.Value]]:
        ...
        if msg.partition_size is not None:
            ...
            _t1783 = self._make_value_int64(msg.partition_size)
            result.append(("partition_size", _t1783,))
        if msg.compression is not None:
            assert msg.compression is not None
            _t1784 = self._make_value_string(msg.compression)
            result.append(("compression", _t1784,))
        if msg.syntax_header_row is not None:
            assert msg.syntax_header_row is not None
            _t1785 = self._make_value_boolean(msg.syntax_header_row)
            result.append(("syntax_header_row", _t1785,))
        if msg.syntax_missing_string is not None:
            assert msg.syntax_missing_string is not None
            _t1786 = self._make_value_string(msg.syntax_missing_string)
            result.append(("syntax_missing_string", _t1786,))
        if msg.syntax_delim is not None:
            assert msg.syntax_delim is not None
            _t1787 = self._make_value_string(msg.syntax_delim)
            result.append(("syntax_delim", _t1787,))
        if msg.syntax_quotechar is not None:
            assert msg.syntax_quotechar is not None
            _t1788 = self._make_value_string(msg.syntax_quotechar)
            result.append(("syntax_quotechar", _t1788,))
        if msg.syntax_escapechar is not None:
            assert msg.syntax_escapechar is not None
            _t1789 = self._make_value_string(msg.syntax_escapechar)
            result.append(("syntax_escapechar", _t1789,))
        return sorted(result)
    ...
    def deconstruct_export_iceberg_config_optional(self, msg: transactions_pb2.ExportIcebergConfig) -> Sequence[tuple[str, logic_pb2.Value]] | None:
        ...
        if msg.prefix != "":
            assert msg.prefix is not None
            _t1793 = self._make_value_string(msg.prefix)
            result.append(("prefix", _t1793,))
        assert msg.target_file_size_bytes is not None
        if msg.target_file_size_bytes != 0:
            assert msg.target_file_size_bytes is not None
            _t1794 = self._make_value_int64(msg.target_file_size_bytes)
            result.append(("target_file_size_bytes", _t1794,))
        if msg.compression != "":
            _t1795 = self._make_value_string(msg.compression)
            result.append(("compression", _t1795,))
        if len(result) == 0:
            return None
        else:
            _t1796 = None
    ...
    def pretty_iceberg_data(self, msg: logic_pb2.IcebergData):
        flat1449 = self._try_flat(msg, self.pretty_iceberg_data)
        if flat1449 is not None:
            ...
            _dollar_dollar = msg
            if not len(_dollar_dollar.columns) == 0:
                _t1733 = _dollar_dollar.columns
            else:
                _t1733 = None
            if _dollar_dollar.HasField("target"):
                _t1734 = _dollar_dollar.target
            else:
                _t1734 = None
            _t1735 = self.deconstruct_iceberg_data_from_snapshot_optional(_dollar_dollar)
            _t1736 = self.deconstruct_iceberg_data_to_snapshot_optional(_dollar_dollar)
            fields1436 = (_dollar_dollar.locator, _dollar_dollar.config, _t1733, _t1734, _t1735, _t1736, _dollar_dollar.returns_delta,)
            ...
            if field1440 is not None:
                ...
                self.pretty_gnf_columns(opt_val1441)
            field1442 = unwrapped_fields1437[3]
            if field1442 is not None:
                self.newline()
                assert field1442 is not None
                opt_val1443 = field1442
                self.pretty_full_table(opt_val1443)
            field1444 = unwrapped_fields1437[4]
            if field1444 is not None:
                self.newline()
                assert field1444 is not None
                opt_val1445 = field1444
                self.pretty_iceberg_from_snapshot(opt_val1445)
            field1446 = unwrapped_fields1437[5]
            if field1446 is not None:
                self.newline()
                assert field1446 is not None
                opt_val1447 = field1446
                self.pretty_iceberg_to_snapshot(opt_val1447)
            self.newline()
            field1448 = unwrapped_fields1437[6]
            self.pretty_boolean_value(field1448)
            self.dedent()
            self.write(")")

    def pretty_iceberg_locator(self, msg: logic_pb2.IcebergLocator):
        flat1455 = self._try_flat(msg, self.pretty_iceberg_locator)
        if flat1455 is not None:
            assert flat1455 is not None
            self.write(flat1455)
            return None
        else:
            _dollar_dollar = msg
            ...
            self.pretty_iceberg_locator_table_name(field1452)
            self.newline()
            field1453 = unwrapped_fields1451[1]
            self.pretty_iceberg_locator_namespace(field1453)
            self.newline()
            field1454 = unwrapped_fields1451[2]
            self.pretty_iceberg_locator_warehouse(field1454)
            self.dedent()
            self.write(")")

    def pretty_iceberg_locator_table_name(self, msg: str):
        flat1457 = self._try_flat(msg, self.pretty_iceberg_locator_table_name)
        if flat1457 is not None:
            ...
            self.indent_sexp()
            self.newline()
            self.write(self.format_string_value(fields1456))
            self.dedent()
            self.write(")")

    def pretty_iceberg_locator_namespace(self, msg: Sequence[str]):
        flat1461 = self._try_flat(msg, self.pretty_iceberg_locator_namespace)
    ...
    def pretty_iceberg_locator_warehouse(self, msg: str):
        flat1463 = self._try_flat(msg, self.pretty_iceberg_locator_warehouse)
    ...
    def pretty_iceberg_catalog_config(self, msg: logic_pb2.IcebergCatalogConfig):
        flat1471 = self._try_flat(msg, self.pretty_iceberg_catalog_config)
        if flat1471 is not None:
            ...
            _dollar_dollar = msg
            _t1737 = self.deconstruct_iceberg_catalog_config_scope_optional(_dollar_dollar)
            fields1464 = (_dollar_dollar.catalog_uri, _t1737, sorted(_dollar_dollar.properties.items()), sorted(_dollar_dollar.auth_properties.items()),)
            ...
            self.pretty_iceberg_catalog_uri(field1466)
            field1467 = unwrapped_fields1465[1]
            if field1467 is not None:
                self.newline()
                assert field1467 is not None
                opt_val1468 = field1467
                self.pretty_iceberg_catalog_config_scope(opt_val1468)
            self.newline()
            field1469 = unwrapped_fields1465[2]
            self.pretty_iceberg_properties(field1469)
            self.newline()
            field1470 = unwrapped_fields1465[3]
            self.pretty_iceberg_auth_properties(field1470)
            self.dedent()
            self.write(")")

    def pretty_iceberg_catalog_uri(self, msg: str):
        flat1473 = self._try_flat(msg, self.pretty_iceberg_catalog_uri)
    ...
    def pretty_iceberg_catalog_config_scope(self, msg: str):
        flat1475 = self._try_flat(msg, self.pretty_iceberg_catalog_config_scope)
    ...
    def pretty_iceberg_properties(self, msg: Sequence[tuple[str, str]]):
        flat1479 = self._try_flat(msg, self.pretty_iceberg_properties)
        if flat1479 is not None:
            ...
            if not len(fields1476) == 0:
                ...
                for i1478, elem1477 in enumerate(fields1476):
                    if (i1478 > 0):
                        self.newline()
                    self.pretty_iceberg_property_entry(elem1477)
            self.dedent()
            self.write(")")

    def pretty_iceberg_property_entry(self, msg: tuple[str, str]):
        flat1484 = self._try_flat(msg, self.pretty_iceberg_property_entry)
        if flat1484 is not None:
            ...
            _dollar_dollar = msg
            fields1480 = (_dollar_dollar[0], _dollar_dollar[1],)
    ...
    def pretty_iceberg_auth_properties(self, msg: Sequence[tuple[str, str]]):
        flat1488 = self._try_flat(msg, self.pretty_iceberg_auth_properties)
        if flat1488 is not None:
            ...
            if not len(fields1485) == 0:
                ...
                for i1487, elem1486 in enumerate(fields1485):
                    if (i1487 > 0):
                        self.newline()
                    self.pretty_iceberg_masked_property_entry(elem1486)
            self.dedent()
            self.write(")")

    def pretty_iceberg_masked_property_entry(self, msg: tuple[str, str]):
        flat1493 = self._try_flat(msg, self.pretty_iceberg_masked_property_entry)
        if flat1493 is not None:
            ...
            _dollar_dollar = msg
            _t1738 = self.mask_secret_value(_dollar_dollar)
            fields1489 = (_dollar_dollar[0], _t1738,)
    ...
    def pretty_full_table(self, msg: logic_pb2.IcebergTarget):
        flat1500 = self._try_flat(msg, self.pretty_full_table)
        if flat1500 is not None:
            ...
            self.write("(full_table")
            self.indent_sexp()
            self.newline()
            field1496 = unwrapped_fields1495[0]
            self.pretty_relation_id(field1496)
            self.newline()
            self.write("[")
            field1497 = unwrapped_fields1495[1]
            for i1499, elem1498 in enumerate(field1497):
                if (i1499 > 0):
                    self.newline()
                self.pretty_type(elem1498)
            self.write("]")
            self.dedent()
            self.write(")")
    ...
    def pretty_read(self, msg: transactions_pb2.Read):
        flat1539 = self._try_flat(msg, self.pretty_read)
        if flat1539 is not None:
            ...
            if deconstruct_result1537 is not None:
                ...
                self.pretty_demand(unwrapped1538)
            else:
                _dollar_dollar = msg
                if _dollar_dollar.HasField("output"):
                    _t1740 = _dollar_dollar.output
                else:
                    _t1740 = None
                ...
                if deconstruct_result1535 is not None:
                    ...
                    self.pretty_output(unwrapped1536)
                else:
                    _dollar_dollar = msg
                    ...
                    if deconstruct_result1533 is not None:
                        ...
                        self.pretty_what_if(unwrapped1534)
                    else:
                        _dollar_dollar = msg
                        if _dollar_dollar.HasField("abort"):
                            _t1742 = _dollar_dollar.abort
                        else:
                            _t1742 = None
                        ...
                        if deconstruct_result1531 is not None:
                            ...
                            self.pretty_abort(unwrapped1532)
                        else:
                            _dollar_dollar = msg
                            if _dollar_dollar.HasField("export"):
                                _t1743 = _dollar_dollar.export
                            else:
                                _t1743 = None
    ...
    def pretty_demand(self, msg: transactions_pb2.Demand):
        ...
        if flat1542 is not None:
            ...
            assert fields1540 is not None
            unwrapped_fields1541 = fields1540
            self.write("(demand")
            self.indent_sexp()
            self.newline()
            self.pretty_relation_id(unwrapped_fields1541)
            self.dedent()
            self.write(")")

    def pretty_output(self, msg: transactions_pb2.Output):
        flat1547 = self._try_flat(msg, self.pretty_output)
        if flat1547 is not None:
            ...
            self.pretty_relation_id(field1546)
            self.dedent()
            self.write(")")

    def pretty_what_if(self, msg: transactions_pb2.WhatIf):
        flat1552 = self._try_flat(msg, self.pretty_what_if)
        if flat1552 is not None:
            ...
            self.pretty_name(field1550)
            self.newline()
            field1551 = unwrapped_fields1549[1]
            self.pretty_epoch(field1551)
            self.dedent()
            self.write(")")

    def pretty_abort(self, msg: transactions_pb2.Abort):
        flat1558 = self._try_flat(msg, self.pretty_abort)
        if flat1558 is not None:
            ...
            if _dollar_dollar.name != "abort":
                _t1744 = _dollar_dollar.name
            else:
                _t1744 = None
            fields1553 = (_t1744, _dollar_dollar.relation_id,)
            assert fields1553 is not None
            unwrapped_fields1554 = fields1553
            self.write("(abort")
            self.indent_sexp()
            field1555 = unwrapped_fields1554[0]
            if field1555 is not None:
                self.newline()
                assert field1555 is not None
                opt_val1556 = field1555
                self.pretty_name(opt_val1556)
            self.newline()
            field1557 = unwrapped_fields1554[1]
            self.pretty_relation_id(field1557)
            self.dedent()
            self.write(")")

    def pretty_export(self, msg: transactions_pb2.Export):
        flat1563 = self._try_flat(msg, self.pretty_export)
        if flat1563 is not None:
            ...
            if deconstruct_result1561 is not None:
                ...
                self.indent_sexp()
                self.newline()
                self.pretty_export_csv_config(unwrapped1562)
                self.dedent()
                self.write(")")
            else:
                _dollar_dollar = msg
                if _dollar_dollar.HasField("iceberg_config"):
                    _t1746 = _dollar_dollar.iceberg_config
                else:
                    _t1746 = None
                ...
                if deconstruct_result1559 is not None:
                    ...
                    self.indent_sexp()
                    self.newline()
                    self.pretty_export_iceberg_config(unwrapped1560)
                    self.dedent()
                    self.write(")")
                else:
    ...
    def pretty_export_csv_config(self, msg: transactions_pb2.ExportCSVConfig):
        flat1574 = self._try_flat(msg, self.pretty_export_csv_config)
        if flat1574 is not None:
            ...
            if deconstruct_result1569 is not None:
                ...
                self.indent_sexp()
                self.newline()
                field1571 = unwrapped1570[0]
                self.pretty_export_csv_path(field1571)
                self.newline()
                field1572 = unwrapped1570[1]
                self.pretty_export_csv_source(field1572)
                self.newline()
                field1573 = unwrapped1570[2]
                self.pretty_csv_config(field1573)
                self.dedent()
                self.write(")")
            else:
                _dollar_dollar = msg
                ...
                if deconstruct_result1564 is not None:
                    ...
                    self.pretty_export_csv_path(field1566)
                    self.newline()
                    field1567 = unwrapped1565[1]
                    self.pretty_export_csv_columns_list(field1567)
                    self.newline()
                    field1568 = unwrapped1565[2]
                    self.pretty_config_dict(field1568)
                    self.dedent()
                    self.write(")")
                else:
    ...
    def pretty_export_csv_path(self, msg: str):
        flat1576 = self._try_flat(msg, self.pretty_export_csv_path)
        if flat1576 is not None:
            ...
            self.indent_sexp()
            self.newline()
            self.write(self.format_string_value(fields1575))
            self.dedent()
            self.write(")")

    def pretty_export_csv_source(self, msg: transactions_pb2.ExportCSVSource):
        flat1583 = self._try_flat(msg, self.pretty_export_csv_source)
        if flat1583 is not None:
            ...
            if _dollar_dollar.HasField("gnf_columns"):
                _t1750 = _dollar_dollar.gnf_columns.columns
            else:
                _t1750 = None
            ...
            if deconstruct_result1579 is not None:
                ...
                self.indent_sexp()
                if not len(unwrapped1580) == 0:
                    self.newline()
                    for i1582, elem1581 in enumerate(unwrapped1580):
                        if (i1582 > 0):
                            self.newline()
                        self.pretty_export_csv_column(elem1581)
                self.dedent()
                self.write(")")
            else:
                _dollar_dollar = msg
                if _dollar_dollar.HasField("table_def"):
                    _t1751 = _dollar_dollar.table_def
                else:
                    _t1751 = None
                ...
                if deconstruct_result1577 is not None:
                    ...
                    self.indent_sexp()
                    self.newline()
                    self.pretty_relation_id(unwrapped1578)
                    self.dedent()
                    self.write(")")
                else:
    ...
    def pretty_export_csv_column(self, msg: transactions_pb2.ExportCSVColumn):
        flat1588 = self._try_flat(msg, self.pretty_export_csv_column)
        if flat1588 is not None:
            ...
            self.indent_sexp()
            self.newline()
            field1586 = unwrapped_fields1585[0]
            self.write(self.format_string_value(field1586))
            self.newline()
            field1587 = unwrapped_fields1585[1]
            self.pretty_relation_id(field1587)
            self.dedent()
            self.write(")")
    ...
    def pretty_export_iceberg_config(self, msg: transactions_pb2.ExportIcebergConfig):
        ...
        if flat1601 is not None:
            ...
            _dollar_dollar = msg
            _t1752 = self.deconstruct_export_iceberg_config_optional(_dollar_dollar)
            fields1593 = (_dollar_dollar.locator, _dollar_dollar.config, _dollar_dollar.table_def, sorted(_dollar_dollar.table_properties.items()), _t1752,)
    ...
    def pretty_debug_info(self, msg: fragments_pb2.DebugInfo):
        ...
        for _idx, _rid in enumerate(msg.ids):
            self.newline()
            self.write("(")
            _t1798 = logic_pb2.UInt128Value(low=_rid.id_low, high=_rid.id_high)
            self.pprint_dispatch(_t1798)
            self.write(" ")
    ...
    def pprint_dispatch(self, msg):
        ...
        elif isinstance(msg, logic_pb2.IcebergTarget):
            self.pretty_full_table(msg)
