...
class PrettyPrinter:
    ...
    def _make_value_string(self, v: str) -> logic_pb2.Value:
        _t1252 = logic_pb2.Value(string_value=v)
        return _t1252
    ...
    def _make_value_int32(self, v: int) -> logic_pb2.Value:
        _t1255 = logic_pb2.Value(int_value=int(v))
        return _t1255

    def _make_value_int64(self, v: int) -> logic_pb2.Value:
        _t1256 = logic_pb2.Value(int_value=v)
        return _t1256

    def deconstruct_csv_config(self, msg: logic_pb2.CSVConfig) -> list[tuple[str, logic_pb2.Value]]:
        result = []
        _t1257 = self._make_value_int32(msg.header_row)
        result.append(('csv_header_row', _t1257,))
        _t1258 = self._make_value_int64(msg.skip)
        result.append(('csv_skip', _t1258,))
        if msg.new_line != '':
            _t1259 = self._make_value_string(msg.new_line)
            result.append(('csv_new_line', _t1259,))
        _t1260 = self._make_value_string(msg.delimiter)
        result.append(('csv_delimiter', _t1260,))
        _t1261 = self._make_value_string(msg.quotechar)
        result.append(('csv_quotechar', _t1261,))
        _t1262 = self._make_value_string(msg.escapechar)
        result.append(('csv_escapechar', _t1262,))
        if msg.comment != '':
            _t1263 = self._make_value_string(msg.comment)
            result.append(('csv_comment', _t1263,))
        for missing_string in msg.missing_strings:
            _t1264 = self._make_value_string(missing_string)
            result.append(('csv_missing_strings', _t1264,))
        _t1265 = self._make_value_string(msg.decimal_separator)
        result.append(('csv_decimal_separator', _t1265,))
        _t1266 = self._make_value_string(msg.encoding)
        result.append(('csv_encoding', _t1266,))
        _t1267 = self._make_value_string(msg.compression)
        result.append(('csv_compression', _t1267,))
    ...
    def _make_value_uint128(self, v: logic_pb2.UInt128Value) -> logic_pb2.Value:
        _t1276 = logic_pb2.Value(uint128_value=v)
        return _t1276
