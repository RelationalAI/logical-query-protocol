...
class PrettyPrinter:
    ...
    def _make_value_boolean(self, v: bool) -> logic_pb2.Value:
        _t1253 = logic_pb2.Value(boolean_value=v)
        return _t1253
    ...
    def deconstruct_betree_info_config(self, msg: logic_pb2.BeTreeInfo) -> list[tuple[str, logic_pb2.Value]]:
        result = []
        _t1254 = self._make_value_float64(msg.storage_config.epsilon)
        result.append(('betree_config_epsilon', _t1254,))
        _t1255 = self._make_value_int64(msg.storage_config.max_pivots)
        result.append(('betree_config_max_pivots', _t1255,))
        _t1256 = self._make_value_int64(msg.storage_config.max_deltas)
        result.append(('betree_config_max_deltas', _t1256,))
        _t1257 = self._make_value_int64(msg.storage_config.max_leaf)
        result.append(('betree_config_max_leaf', _t1257,))
        if msg.relation_locator.HasField('root_pageid'):
            if msg.relation_locator.root_pageid is not None:
                ...
                _t1258 = self._make_value_uint128(msg.relation_locator.root_pageid)
                result.append(('betree_locator_root_pageid', _t1258,))
        if msg.relation_locator.HasField('inline_data'):
            if msg.relation_locator.inline_data is not None:
                ...
                _t1259 = self._make_value_string(msg.relation_locator.inline_data.decode('utf-8'))
                result.append(('betree_locator_inline_data', _t1259,))
        _t1260 = self._make_value_int64(msg.relation_locator.element_count)
        result.append(('betree_locator_element_count', _t1260,))
        _t1261 = self._make_value_int64(msg.relation_locator.tree_height)
        result.append(('betree_locator_tree_height', _t1261,))
        return sorted(result)

    def _make_value_uint128(self, v: logic_pb2.UInt128Value) -> logic_pb2.Value:
        _t1262 = logic_pb2.Value(uint128_value=v)
        return _t1262
    ...
    def deconstruct_configure(self, msg: transactions_pb2.Configure) -> list[tuple[str, logic_pb2.Value]]:
        ...
        if msg.ivm_config.level == transactions_pb2.MaintenanceLevel.MAINTENANCE_LEVEL_AUTO:
            _t1264 = self._make_value_string('auto')
            ...
            if msg.ivm_config.level == transactions_pb2.MaintenanceLevel.MAINTENANCE_LEVEL_ALL:
                _t1265 = self._make_value_string('all')
                ...
                if msg.ivm_config.level == transactions_pb2.MaintenanceLevel.MAINTENANCE_LEVEL_OFF:
                    _t1266 = self._make_value_string('off')
                    result.append(('ivm.maintenance_level', _t1266,))
        _t1267 = self._make_value_int64(msg.semantics_version)
        result.append(('semantics_version', _t1267,))
        return sorted(result)

    def _make_value_int32(self, v: int) -> logic_pb2.Value:
        _t1268 = logic_pb2.Value(int_value=int(v))
        return _t1268
    ...
    def deconstruct_export_csv_config(self, msg: transactions_pb2.ExportCSVConfig) -> list[tuple[str, logic_pb2.Value]]:
        ...
        if msg.partition_size is not None:
            ...
            _t1280 = self._make_value_int64(msg.partition_size)
            result.append(('partition_size', _t1280,))
        if msg.compression is not None:
            assert msg.compression is not None
            _t1281 = self._make_value_string(msg.compression)
            result.append(('compression', _t1281,))
        if msg.syntax_header_row is not None:
            assert msg.syntax_header_row is not None
            _t1282 = self._make_value_boolean(msg.syntax_header_row)
            result.append(('syntax_header_row', _t1282,))
        if msg.syntax_missing_string is not None:
            assert msg.syntax_missing_string is not None
            _t1283 = self._make_value_string(msg.syntax_missing_string)
            result.append(('syntax_missing_string', _t1283,))
        if msg.syntax_delim is not None:
            assert msg.syntax_delim is not None
            _t1284 = self._make_value_string(msg.syntax_delim)
            result.append(('syntax_delim', _t1284,))
        if msg.syntax_quotechar is not None:
            assert msg.syntax_quotechar is not None
            _t1285 = self._make_value_string(msg.syntax_quotechar)
            result.append(('syntax_quotechar', _t1285,))
        if msg.syntax_escapechar is not None:
            assert msg.syntax_escapechar is not None
            _t1286 = self._make_value_string(msg.syntax_escapechar)
            result.append(('syntax_escapechar', _t1286,))
        return sorted(result)
    ...
    def _make_value_int64(self, v: int) -> logic_pb2.Value:
        _t1288 = logic_pb2.Value(int_value=v)
        return _t1288
