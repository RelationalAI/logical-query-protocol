...
class PrettyPrinter:
    ...
    def _make_value_int32(self, v: int) -> logic_pb2.Value:
        _t1267 = logic_pb2.Value(int_value=int(v))
        return _t1267

    def _make_value_float64(self, v: float) -> logic_pb2.Value:
        _t1268 = logic_pb2.Value(float_value=v)
        return _t1268

    def deconstruct_betree_info_config(self, msg: logic_pb2.BeTreeInfo) -> list[tuple[str, logic_pb2.Value]]:
        result = []
        _t1269 = self._make_value_float64(msg.storage_config.epsilon)
        result.append(('betree_config_epsilon', _t1269,))
        _t1270 = self._make_value_int64(msg.storage_config.max_pivots)
        result.append(('betree_config_max_pivots', _t1270,))
        _t1271 = self._make_value_int64(msg.storage_config.max_deltas)
        result.append(('betree_config_max_deltas', _t1271,))
        _t1272 = self._make_value_int64(msg.storage_config.max_leaf)
        result.append(('betree_config_max_leaf', _t1272,))
        if msg.relation_locator.HasField('root_pageid'):
            if msg.relation_locator.root_pageid is not None:
                ...
                _t1273 = self._make_value_uint128(msg.relation_locator.root_pageid)
                result.append(('betree_locator_root_pageid', _t1273,))
        if msg.relation_locator.HasField('inline_data'):
            if msg.relation_locator.inline_data is not None:
                ...
                _t1274 = self._make_value_string(msg.relation_locator.inline_data.decode('utf-8'))
                result.append(('betree_locator_inline_data', _t1274,))
        _t1275 = self._make_value_int64(msg.relation_locator.element_count)
        result.append(('betree_locator_element_count', _t1275,))
        _t1276 = self._make_value_int64(msg.relation_locator.tree_height)
        result.append(('betree_locator_tree_height', _t1276,))
    ...
    def deconstruct_configure(self, msg: transactions_pb2.Configure) -> list[tuple[str, logic_pb2.Value]]:
        ...
        if msg.ivm_config.level == transactions_pb2.MaintenanceLevel.MAINTENANCE_LEVEL_AUTO:
            _t1277 = self._make_value_string('auto')
            result.append(('ivm.maintenance_level', _t1277,))
        else:
            if msg.ivm_config.level == transactions_pb2.MaintenanceLevel.MAINTENANCE_LEVEL_ALL:
                _t1278 = self._make_value_string('all')
                result.append(('ivm.maintenance_level', _t1278,))
            else:
                if msg.ivm_config.level == transactions_pb2.MaintenanceLevel.MAINTENANCE_LEVEL_OFF:
                    _t1279 = self._make_value_string('off')
                    result.append(('ivm.maintenance_level', _t1279,))
        _t1280 = self._make_value_int64(msg.semantics_version)
        result.append(('semantics_version', _t1280,))
        return sorted(result)
    ...
    def deconstruct_csv_config(self, msg: logic_pb2.CSVConfig) -> list[tuple[str, logic_pb2.Value]]:
        result = []
        _t1281 = self._make_value_int32(msg.header_row)
        result.append(('csv_header_row', _t1281,))
        _t1282 = self._make_value_int64(msg.skip)
        result.append(('csv_skip', _t1282,))
        if msg.new_line != '':
            _t1283 = self._make_value_string(msg.new_line)
            result.append(('csv_new_line', _t1283,))
        _t1284 = self._make_value_string(msg.delimiter)
        result.append(('csv_delimiter', _t1284,))
        _t1285 = self._make_value_string(msg.quotechar)
        result.append(('csv_quotechar', _t1285,))
        _t1286 = self._make_value_string(msg.escapechar)
        result.append(('csv_escapechar', _t1286,))
        if msg.comment != '':
            _t1287 = self._make_value_string(msg.comment)
            result.append(('csv_comment', _t1287,))
        for missing_string in msg.missing_strings:
            _t1288 = self._make_value_string(missing_string)
            result.append(('csv_missing_strings', _t1288,))
        _t1289 = self._make_value_string(msg.decimal_separator)
        result.append(('csv_decimal_separator', _t1289,))
        _t1290 = self._make_value_string(msg.encoding)
        result.append(('csv_encoding', _t1290,))
        _t1291 = self._make_value_string(msg.compression)
        result.append(('csv_compression', _t1291,))
        if msg.partition_size_mb != 0:
            _t1292 = self._make_value_int64(msg.partition_size_mb)
            result.append(('csv_partition_size_mb', _t1292,))
        return sorted(result)
    ...
    def pretty_export_csv_config(self, msg: transactions_pb2.ExportCSVConfig) -> Optional[NoReturn]:
        def _t1246(_dollar_dollar):
            return (_dollar_dollar.path, _dollar_dollar.csv_source, _dollar_dollar.csv_config,)
        _t1247 = _t1246(msg)
        fields482 = _t1247
        ...
        self.indent()
        self.newline()
        field484 = unwrapped_fields483[0]
        _t1248 = self.pretty_export_csv_path(field484)
        self.newline()
        field485 = unwrapped_fields483[1]
        _t1249 = self.pretty_export_csv_source(field485)
        self.newline()
        field486 = unwrapped_fields483[2]
        _t1250 = self.pretty_csv_config(field486)
        self.dedent()
        self.write(')')
        return None
    ...
    def pretty_export_csv_path(self, msg: str) -> Optional[NoReturn]:
        ...
        self.indent()
        self.newline()
        self.write(self.format_string_value(unwrapped_fields488))
        self.dedent()
        self.write(')')
        return None
    ...
    def pretty_export_csv_source(self, msg: transactions_pb2.ExportCSVSource) -> Optional[NoReturn]:
        def _t1253(_dollar_dollar):
            ...
            if _dollar_dollar.HasField('gnf_columns'):
                _t1254 = _dollar_dollar.gnf_columns.columns
            else:
                _t1254 = None
            return _t1254
        _t1255 = _t1253(msg)
        deconstruct_result490 = _t1255
        if deconstruct_result490 is not None:
            self.write('(')
            self.write('gnf_columns')
            self.indent()
            if not len(deconstruct_result490) == 0:
                self.newline()
                for i492, elem491 in enumerate(deconstruct_result490):
                    if (i492 > 0):
                        self.newline()
                    _t1256 = self.pretty_export_csv_column(elem491)
            self.dedent()
            self.write(')')
        else:
            def _t1257(_dollar_dollar):
                ...
                if _dollar_dollar.HasField('table_def'):
                    _t1258 = _dollar_dollar.table_def
                else:
                    _t1258 = None
                return _t1258
            _t1259 = _t1257(msg)
            deconstruct_result489 = _t1259
            if deconstruct_result489 is not None:
                self.write('(')
                self.write('table_def')
                self.indent()
                self.newline()
                _t1260 = self.pretty_relation_id(deconstruct_result489)
                self.dedent()
                self.write(')')
            else:
    ...
    def pretty_export_csv_column(self, msg: transactions_pb2.ExportCSVColumn) -> Optional[NoReturn]:
        ...
        self.indent()
        self.newline()
        field495 = unwrapped_fields494[0]
        self.write(self.format_string_value(field495))
        self.newline()
        field496 = unwrapped_fields494[1]
        _t1263 = self.pretty_relation_id(field496)
        self.dedent()
        self.write(')')
        return None
