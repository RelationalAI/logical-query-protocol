...
def test_provenance_spans_valid(input_file):
    ...
    for offset, span in provenance.items():
        ...
        assert span.start.offset <= span.stop.offset, (
            f"{os.path.basename(input_file)} offset {offset}: "
...
def test_provenance_root_spans_file(input_file):
    """The root transaction should span the beginning of the transaction."""
    with open(input_file) as f:
        content = f.read()

    _, provenance = parse(content)
    root_spans = [s for s in provenance.values() if s.type_name == "Transaction"]
    assert root_spans, f"No Transaction provenance for {input_file}"
