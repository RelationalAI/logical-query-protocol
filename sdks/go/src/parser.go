...
func (p *Parser) construct_configure(config_dict [][]interface{}) *pb.Configure {
	...
	ivm_config := _t967
	_t968 := p._extract_value_int64(dictGetValue(config, "semantics_version"), 0)
...
func (p *Parser) default_configure() *pb.Configure {
	_t970 := &pb.IVMConfig{Level: pb.MaintenanceLevel_MAINTENANCE_LEVEL_OFF}
	ivm_config := _t970
	_t971 := &pb.Configure{SemanticsVersion: 0, IvmConfig: ivm_config}
	return _t971
}
...
func (p *Parser) construct_csv_config(config_dict [][]interface{}) *pb.CSVConfig {
	config := dictFromList(config_dict)
	_t972 := p._extract_value_int32(dictGetValue(config, "csv_header_row"), 1)
	header_row := _t972
	_t973 := p._extract_value_int64(dictGetValue(config, "csv_skip"), 0)
	skip := _t973
	_t974 := p._extract_value_string(dictGetValue(config, "csv_new_line"), "")
	new_line := _t974
	_t975 := p._extract_value_string(dictGetValue(config, "csv_delimiter"), ",")
	delimiter := _t975
	_t976 := p._extract_value_string(dictGetValue(config, "csv_quotechar"), "\"")
	quotechar := _t976
	_t977 := p._extract_value_string(dictGetValue(config, "csv_escapechar"), "\"")
	escapechar := _t977
	_t978 := p._extract_value_string(dictGetValue(config, "csv_comment"), "")
	comment := _t978
	_t979 := p._extract_value_string_list(dictGetValue(config, "csv_missing_strings"), []string{})
	missing_strings := _t979
	_t980 := p._extract_value_string(dictGetValue(config, "csv_decimal_separator"), ".")
	decimal_separator := _t980
	_t981 := p._extract_value_string(dictGetValue(config, "csv_encoding"), "utf-8")
	encoding := _t981
	_t982 := p._extract_value_string(dictGetValue(config, "csv_compression"), "auto")
	compression := _t982
	_t983 := &pb.CSVConfig{HeaderRow: header_row, Skip: skip, NewLine: new_line, Delimiter: delimiter, Quotechar: quotechar, Escapechar: escapechar, Comment: comment, MissingStrings: missing_strings, DecimalSeparator: decimal_separator, Encoding: encoding, Compression: compression}
	return _t983
}
