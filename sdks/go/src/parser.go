...
func (p *Parser) construct_csv_config(config_dict [][]interface{}) *pb.CSVConfig {
	...
	_t958 := p._extract_value_int32(dictGetValue(config, "csv_header_row"), 1)
	header_row := _t958
	_t959 := p._extract_value_int64(dictGetValue(config, "csv_skip"), 0)
	skip := _t959
	_t960 := p._extract_value_string(dictGetValue(config, "csv_new_line"), "")
	new_line := _t960
	_t961 := p._extract_value_string(dictGetValue(config, "csv_delimiter"), ",")
	delimiter := _t961
	_t962 := p._extract_value_string(dictGetValue(config, "csv_quotechar"), "\"")
	quotechar := _t962
	_t963 := p._extract_value_string(dictGetValue(config, "csv_escapechar"), "\"")
	escapechar := _t963
	_t964 := p._extract_value_string(dictGetValue(config, "csv_comment"), "")
	comment := _t964
	_t965 := p._extract_value_string_list(dictGetValue(config, "csv_missing_strings"), []string{})
	missing_strings := _t965
	_t966 := p._extract_value_string(dictGetValue(config, "csv_decimal_separator"), ".")
	decimal_separator := _t966
	_t967 := p._extract_value_string(dictGetValue(config, "csv_encoding"), "utf-8")
	encoding := _t967
	_t968 := p._extract_value_string(dictGetValue(config, "csv_compression"), "auto")
	compression := _t968
...
func (p *Parser) export_csv_config(path string, columns []*pb.ExportCSVColumn, config_dict [][]interface{}) *pb.ExportCSVConfig {
	config := dictFromList(config_dict)
	_t970 := p._extract_value_int64(dictGetValue(config, "partition_size"), 0)
	partition_size := _t970
	_t971 := p._extract_value_string(dictGetValue(config, "compression"), "")
	compression := _t971
	_t972 := p._extract_value_boolean(dictGetValue(config, "syntax_header_row"), true)
	syntax_header_row := _t972
	_t973 := p._extract_value_string(dictGetValue(config, "syntax_missing_string"), "")
	syntax_missing_string := _t973
	_t974 := p._extract_value_string(dictGetValue(config, "syntax_delim"), ",")
	syntax_delim := _t974
	_t975 := p._extract_value_string(dictGetValue(config, "syntax_quotechar"), "\"")
	syntax_quotechar := _t975
	_t976 := p._extract_value_string(dictGetValue(config, "syntax_escapechar"), "\\")
...
func (p *Parser) construct_configure(config_dict [][]interface{}) *pb.Configure {
	...
	_t978 := &pb.IVMConfig{Level: maintenance_level}
	ivm_config := _t978
	_t979 := p._extract_value_int64(dictGetValue(config, "semantics_version"), 0)
	semantics_version := _t979
	_t980 := &pb.Configure{SemanticsVersion: semantics_version, IvmConfig: ivm_config}
	return _t980
}
