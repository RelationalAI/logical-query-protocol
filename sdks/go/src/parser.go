...
func (p *Parser) constructBetreeInfo(key_types []*pb.Type, value_types []*pb.Type, config_dict [][]interface{}) *pb.BeTreeInfo {
	...
	_t956 := p.TryExtractValueFloat64(dictGetValue(config, "betree_config_epsilon"))
	epsilon := _t956
	_t957 := p.TryExtractValueInt64(dictGetValue(config, "betree_config_max_pivots"))
	max_pivots := _t957
	_t958 := p.TryExtractValueInt64(dictGetValue(config, "betree_config_max_deltas"))
	max_deltas := _t958
	_t959 := p.TryExtractValueInt64(dictGetValue(config, "betree_config_max_leaf"))
	max_leaf := _t959
	...
	_t961 := p.TryExtractValueUint128(dictGetValue(config, "betree_locator_root_pageid"))
	root_pageid := _t961
	_t962 := p.TryExtractValueBytes(dictGetValue(config, "betree_locator_inline_data"))
	inline_data := _t962
	_t963 := p.TryExtractValueInt64(dictGetValue(config, "betree_locator_element_count"))
	element_count := _t963
	_t964 := p.TryExtractValueInt64(dictGetValue(config, "betree_locator_tree_height"))
	tree_height := _t964
...
func (p *Parser) constructConfigure(config_dict [][]interface{}) *pb.Configure {
	...
	_t967 := &pb.IVMConfig{Level: maintenance_level}
	ivm_config := _t967
	_t968 := p.ExtractValueInt64(dictGetValue(config, "semantics_version"), 0)
	semantics_version := _t968
	_t969 := &pb.Configure{SemanticsVersion: semantics_version, IvmConfig: ivm_config}
	return _t969
}
...
func (p *Parser) exportCsvConfig(path string, columns []*pb.ExportCSVColumn, config_dict [][]interface{}) *pb.ExportCSVConfig {
	config := dictFromList(config_dict)
	_t970 := p.ExtractValueInt64(dictGetValue(config, "partition_size"), 0)
	partition_size := _t970
	_t971 := p.ExtractValueString(dictGetValue(config, "compression"), "")
	compression := _t971
	_t972 := p.ExtractValueBoolean(dictGetValue(config, "syntax_header_row"), true)
	syntax_header_row := _t972
	_t973 := p.ExtractValueString(dictGetValue(config, "syntax_missing_string"), "")
	syntax_missing_string := _t973
	_t974 := p.ExtractValueString(dictGetValue(config, "syntax_delim"), ",")
	syntax_delim := _t974
	_t975 := p.ExtractValueString(dictGetValue(config, "syntax_quotechar"), "\"")
	syntax_quotechar := _t975
	_t976 := p.ExtractValueString(dictGetValue(config, "syntax_escapechar"), "\\")
	syntax_escapechar := _t976
	_t977 := &pb.ExportCSVConfig{Path: path, DataColumns: columns, PartitionSize: ptr(partition_size), Compression: ptr(compression), SyntaxHeaderRow: ptr(syntax_header_row), SyntaxMissingString: ptr(syntax_missing_string), SyntaxDelim: ptr(syntax_delim), SyntaxQuotechar: ptr(syntax_quotechar), SyntaxEscapechar: ptr(syntax_escapechar)}
	return _t977
}

func (p *Parser) constructCsvConfig(config_dict [][]interface{}) *pb.CSVConfig {
	config := dictFromList(config_dict)
	_t978 := p.ExtractValueInt32(dictGetValue(config, "csv_header_row"), 1)
	header_row := _t978
	_t979 := p.ExtractValueInt64(dictGetValue(config, "csv_skip"), 0)
	skip := _t979
	_t980 := p.ExtractValueString(dictGetValue(config, "csv_new_line"), "")
	new_line := _t980
	_t981 := p.ExtractValueString(dictGetValue(config, "csv_delimiter"), ",")
	delimiter := _t981
	_t982 := p.ExtractValueString(dictGetValue(config, "csv_quotechar"), "\"")
	quotechar := _t982
	_t983 := p.ExtractValueString(dictGetValue(config, "csv_escapechar"), "\"")
	escapechar := _t983
	_t984 := p.ExtractValueString(dictGetValue(config, "csv_comment"), "")
	comment := _t984
	_t985 := p.ExtractValueStringList(dictGetValue(config, "csv_missing_strings"), []string{})
	missing_strings := _t985
	_t986 := p.ExtractValueString(dictGetValue(config, "csv_decimal_separator"), ".")
	decimal_separator := _t986
	_t987 := p.ExtractValueString(dictGetValue(config, "csv_encoding"), "utf-8")
	encoding := _t987
	_t988 := p.ExtractValueString(dictGetValue(config, "csv_compression"), "auto")
	compression := _t988
	_t989 := &pb.CSVConfig{HeaderRow: header_row, Skip: skip, NewLine: new_line, Delimiter: delimiter, Quotechar: quotechar, Escapechar: escapechar, Comment: comment, MissingStrings: missing_strings, DecimalSeparator: decimal_separator, Encoding: encoding, Compression: compression}
	return _t989
}

func (p *Parser) defaultConfigure() *pb.Configure {
	_t990 := &pb.IVMConfig{Level: pb.MaintenanceLevel_MAINTENANCE_LEVEL_OFF}
	ivm_config := _t990
	_t991 := &pb.Configure{SemanticsVersion: 0, IvmConfig: ivm_config}
	return _t991
}
