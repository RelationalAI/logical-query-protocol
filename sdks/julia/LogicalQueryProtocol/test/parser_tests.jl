...
@testitem "Parser - error on empty input" begin
    using LogicalQueryProtocol: parse, ParseError
    @test_throws ParseError parse("")
    @test_throws ParseError parse("   ")
    @test_throws ParseError parse("\n\n")
end

@testitem "Parser - error on unclosed paren" begin
    using LogicalQueryProtocol: parse, ParseError
    @test_throws ParseError parse("(transaction")
    @test_throws ParseError parse("(transaction (epoch (writes) (reads))")
end

@testitem "Parser - error on missing transaction wrapper" begin
    using LogicalQueryProtocol: parse, ParseError
    @test_throws ParseError parse("(epoch (writes) (reads))")
end

@testitem "Parser - error on bad token" begin
    using LogicalQueryProtocol: parse, ParseError
    @test_throws ParseError parse("(transaction @@@)")
end

@testitem "Parser - error on trailing garbage" begin
    using LogicalQueryProtocol: parse, ParseError
    @test_throws ParseError parse("(transaction (epoch (writes) (reads))) extra")
end

@testitem "Parser - error on unterminated string" begin
    using LogicalQueryProtocol: parse, ParseError
    @test_throws ParseError parse("(transaction \"hello")
end

@testitem "Parser - scan_string" begin
    using LogicalQueryProtocol: scan_string
...
@testitem "Parser - scan_int" begin
    using LogicalQueryProtocol: scan_int
    @test scan_int("0") == 0
    @test scan_int("42") == 42
    @test scan_int("-1") == -1
...
@testitem "Parser - scan_float" begin
    using LogicalQueryProtocol: scan_float
...
@testitem "Parser - scan_int128" begin
    using LogicalQueryProtocol: scan_int128, Proto
    r = scan_int128("0i128")
...
@testitem "Parser - scan_uint128" begin
    using LogicalQueryProtocol: scan_uint128, Proto
    r = scan_uint128("0x0")
...
@testitem "Parser - scan_decimal" begin
    using LogicalQueryProtocol: scan_decimal, Proto
...
@testitem "Parser - Lexer tokenization" begin
    using LogicalQueryProtocol: Lexer
