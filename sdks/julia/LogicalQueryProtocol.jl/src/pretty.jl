...
function deconstruct_configure(pp::PrettyPrinter, msg::Proto.Configure)::Vector{Tuple{String, Proto.Value}}
    ...
    if msg.ivm_config.level == Proto.MaintenanceLevel.MAINTENANCE_LEVEL_AUTO
        _t1736 = _make_value_string(pp, "auto")
        push!(result, ("ivm.maintenance_level", _t1736,))
    else
        if msg.ivm_config.level == Proto.MaintenanceLevel.MAINTENANCE_LEVEL_ALL
            _t1737 = _make_value_string(pp, "all")
            push!(result, ("ivm.maintenance_level", _t1737,))
        else
            if msg.ivm_config.level == Proto.MaintenanceLevel.MAINTENANCE_LEVEL_OFF
                _t1738 = _make_value_string(pp, "off")
                push!(result, ("ivm.maintenance_level", _t1738,))
            end
        end
    end
    _t1739 = _make_value_int64(pp, msg.semantics_version)
    push!(result, ("semantics_version", _t1739,))
    return sort(result)
end

function deconstruct_csv_config(pp::PrettyPrinter, msg::Proto.CSVConfig)::Vector{Tuple{String, Proto.Value}}
    result = Tuple{String, Proto.Value}[]
    _t1740 = _make_value_int32(pp, msg.header_row)
    push!(result, ("csv_header_row", _t1740,))
    _t1741 = _make_value_int64(pp, msg.skip)
    push!(result, ("csv_skip", _t1741,))
    if msg.new_line != ""
        _t1742 = _make_value_string(pp, msg.new_line)
        push!(result, ("csv_new_line", _t1742,))
    end
    _t1743 = _make_value_string(pp, msg.delimiter)
    push!(result, ("csv_delimiter", _t1743,))
    _t1744 = _make_value_string(pp, msg.quotechar)
    push!(result, ("csv_quotechar", _t1744,))
    _t1745 = _make_value_string(pp, msg.escapechar)
    push!(result, ("csv_escapechar", _t1745,))
    if msg.comment != ""
        _t1746 = _make_value_string(pp, msg.comment)
        push!(result, ("csv_comment", _t1746,))
    end
    for missing_string in msg.missing_strings
        _t1747 = _make_value_string(pp, missing_string)
        push!(result, ("csv_missing_strings", _t1747,))
    end
    _t1748 = _make_value_string(pp, msg.decimal_separator)
    push!(result, ("csv_decimal_separator", _t1748,))
    _t1749 = _make_value_string(pp, msg.encoding)
    push!(result, ("csv_encoding", _t1749,))
    _t1750 = _make_value_string(pp, msg.compression)
    push!(result, ("csv_compression", _t1750,))
    if msg.partition_size_mb != 0
        _t1751 = _make_value_int64(pp, msg.partition_size_mb)
        push!(result, ("csv_partition_size_mb", _t1751,))
    end
    return sort(result)
end
...
function deconstruct_betree_info_config(pp::PrettyPrinter, msg::Proto.BeTreeInfo)::Vector{Tuple{String, Proto.Value}}
    result = Tuple{String, Proto.Value}[]
    _t1752 = _make_value_float64(pp, msg.storage_config.epsilon)
    push!(result, ("betree_config_epsilon", _t1752,))
    _t1753 = _make_value_int64(pp, msg.storage_config.max_pivots)
    push!(result, ("betree_config_max_pivots", _t1753,))
    _t1754 = _make_value_int64(pp, msg.storage_config.max_deltas)
    push!(result, ("betree_config_max_deltas", _t1754,))
    _t1755 = _make_value_int64(pp, msg.storage_config.max_leaf)
    push!(result, ("betree_config_max_leaf", _t1755,))
    if _has_proto_field(msg.relation_locator, Symbol("root_pageid"))
        if !isnothing(_get_oneof_field(msg.relation_locator, :root_pageid))
            _t1756 = _make_value_uint128(pp, _get_oneof_field(msg.relation_locator, :root_pageid))
            push!(result, ("betree_locator_root_pageid", _t1756,))
        end
    end
    if _has_proto_field(msg.relation_locator, Symbol("inline_data"))
        if !isnothing(_get_oneof_field(msg.relation_locator, :inline_data))
            _t1757 = _make_value_string(pp, String(copy(_get_oneof_field(msg.relation_locator, :inline_data))))
            push!(result, ("betree_locator_inline_data", _t1757,))
        end
    end
    _t1758 = _make_value_int64(pp, msg.relation_locator.element_count)
    push!(result, ("betree_locator_element_count", _t1758,))
    _t1759 = _make_value_int64(pp, msg.relation_locator.tree_height)
    push!(result, ("betree_locator_tree_height", _t1759,))
    return sort(result)
end
...
function deconstruct_export_csv_config(pp::PrettyPrinter, msg::Proto.ExportCSVConfig)::Vector{Tuple{String, Proto.Value}}
    ...
    if !isnothing(msg.partition_size)
        _t1760 = _make_value_int64(pp, msg.partition_size)
        push!(result, ("partition_size", _t1760,))
    end
    if !isnothing(msg.compression)
        _t1761 = _make_value_string(pp, msg.compression)
        push!(result, ("compression", _t1761,))
    end
    if !isnothing(msg.syntax_header_row)
        _t1762 = _make_value_boolean(pp, msg.syntax_header_row)
        push!(result, ("syntax_header_row", _t1762,))
    end
    if !isnothing(msg.syntax_missing_string)
        _t1763 = _make_value_string(pp, msg.syntax_missing_string)
        push!(result, ("syntax_missing_string", _t1763,))
    end
    if !isnothing(msg.syntax_delim)
        _t1764 = _make_value_string(pp, msg.syntax_delim)
        push!(result, ("syntax_delim", _t1764,))
    end
    if !isnothing(msg.syntax_quotechar)
        _t1765 = _make_value_string(pp, msg.syntax_quotechar)
        push!(result, ("syntax_quotechar", _t1765,))
    end
    if !isnothing(msg.syntax_escapechar)
        _t1766 = _make_value_string(pp, msg.syntax_escapechar)
        push!(result, ("syntax_escapechar", _t1766,))
    end
    return sort(result)
end
...
function deconstruct_export_iceberg_config_optional(pp::PrettyPrinter, msg::Proto.ExportIcebergConfig)::Union{Nothing, Vector{Tuple{String, Proto.Value}}}
    result = Tuple{String, Proto.Value}[]
    if msg.prefix != ""
        _t1770 = _make_value_string(pp, msg.prefix)
        push!(result, ("prefix", _t1770,))
    end
    if msg.target_file_size_bytes != 0
        _t1771 = _make_value_int64(pp, msg.target_file_size_bytes)
        push!(result, ("target_file_size_bytes", _t1771,))
    end
    if msg.compression != ""
...
function pretty_iceberg_data(pp::PrettyPrinter, msg::Proto.IcebergData)
    flat1409 = try_flat(pp, msg, pretty_iceberg_data)
    if !isnothing(flat1409)
        write(pp, flat1409)
        return nothing
    else
        _dollar_dollar = msg
        fields1403 = (_dollar_dollar.locator, _dollar_dollar.config, _dollar_dollar.columns, _dollar_dollar.returns_delta,)
        ...
        pretty_boolean_value(pp, field1408)
        dedent!(pp)
        write(pp, ")")
    end
    return nothing
...
function pretty_iceberg_locator(pp::PrettyPrinter, msg::Proto.IcebergLocator)
    flat1421 = try_flat(pp, msg, pretty_iceberg_locator)
    if !isnothing(flat1421)
        write(pp, flat1421)
        return nothing
    else
        _dollar_dollar = msg
        _t1703 = deconstruct_iceberg_locator_from_snapshot_optional(pp, _dollar_dollar)
        _t1704 = deconstruct_iceberg_locator_to_snapshot_optional(pp, _dollar_dollar)
        fields1410 = (_dollar_dollar.table_name, _dollar_dollar.namespace, _dollar_dollar.warehouse, _t1703, _t1704,)
        ...
        if !isempty(field1413)
            newline(pp)
            for (i1705, elem1414) in enumerate(field1413)
                i1415 = i1705 - 1
                if (i1415 > 0)
                    newline(pp)
                end
                write(pp, format_string(DEFAULT_CONSTANT_FORMATTER, pp, elem1414))
            end
        end
        dedent!(pp)
        write(pp, ")")
        ...
        write(pp, "warehouse")
        newline(pp)
        field1416 = unwrapped_fields1411[3]
        write(pp, format_string(DEFAULT_CONSTANT_FORMATTER, pp, field1416))
        dedent!(pp)
        write(pp, ")")
        field1417 = unwrapped_fields1411[4]
        if !isnothing(field1417)
            newline(pp)
            opt_val1418 = field1417
            pretty_iceberg_from_snapshot(pp, opt_val1418)
        end
        field1419 = unwrapped_fields1411[5]
        if !isnothing(field1419)
            newline(pp)
            opt_val1420 = field1419
            pretty_iceberg_to_snapshot(pp, opt_val1420)
        end
        dedent!(pp)
        write(pp, ")")
    end
...
function pretty_iceberg_from_snapshot(pp::PrettyPrinter, msg::String)
    flat1423 = try_flat(pp, msg, pretty_iceberg_from_snapshot)
    if !isnothing(flat1423)
        ...
        indent_sexp!(pp)
        newline(pp)
        write(pp, format_string(DEFAULT_CONSTANT_FORMATTER, pp, fields1422))
        dedent!(pp)
        write(pp, ")")
    end
    return nothing
...
function pretty_iceberg_to_snapshot(pp::PrettyPrinter, msg::String)
    flat1425 = try_flat(pp, msg, pretty_iceberg_to_snapshot)
...
function pretty_iceberg_catalog_config(pp::PrettyPrinter, msg::Proto.IcebergCatalogConfig)
    ...
    if !isnothing(flat1437)
        ...
        _dollar_dollar = msg
        _t1706 = deconstruct_iceberg_catalog_config_scope_optional(pp, _dollar_dollar)
        fields1426 = (_dollar_dollar.catalog_uri, _t1706, sort([(k, v) for (k, v) in _dollar_dollar.properties]), sort([(k, v) for (k, v) in _dollar_dollar.auth_properties]),)
        ...
        if !isempty(field1431)
            newline(pp)
            for (i1707, elem1432) in enumerate(field1431)
                i1433 = i1707 - 1
                if (i1433 > 0)
                    newline(pp)
                end
        ...
        if !isempty(field1434)
            newline(pp)
            for (i1708, elem1435) in enumerate(field1434)
                i1436 = i1708 - 1
                if (i1436 > 0)
                    newline(pp)
                end
...
function pretty_undefine(pp::PrettyPrinter, msg::Proto.Undefine)
    flat1447 = try_flat(pp, msg, pretty_undefine)
    if !isnothing(flat1447)
        write(pp, flat1447)
        return nothing
    else
        _dollar_dollar = msg
        fields1445 = _dollar_dollar.fragment_id
        unwrapped_fields1446 = fields1445
        write(pp, "(undefine")
        indent_sexp!(pp)
        newline(pp)
        pretty_fragment_id(pp, unwrapped_fields1446)
        dedent!(pp)
        write(pp, ")")
    end
    return nothing
...
function pretty_context(pp::PrettyPrinter, msg::Proto.Context)
    flat1452 = try_flat(pp, msg, pretty_context)
    if !isnothing(flat1452)
        write(pp, flat1452)
        return nothing
    else
        _dollar_dollar = msg
        fields1448 = _dollar_dollar.relations
        ...
        if !isempty(unwrapped_fields1449)
            ...
            for (i1709, elem1450) in enumerate(unwrapped_fields1449)
                ...
                pretty_relation_id(pp, elem1450)
            end
        end
        dedent!(pp)
        write(pp, ")")
...
function pretty_snapshot(pp::PrettyPrinter, msg::Proto.Snapshot)
    flat1457 = try_flat(pp, msg, pretty_snapshot)
    if !isnothing(flat1457)
        write(pp, flat1457)
        return nothing
    else
        _dollar_dollar = msg
        fields1453 = _dollar_dollar.mappings
        unwrapped_fields1454 = fields1453
        write(pp, "(snapshot")
        indent_sexp!(pp)
        if !isempty(unwrapped_fields1454)
            newline(pp)
            for (i1710, elem1455) in enumerate(unwrapped_fields1454)
                i1456 = i1710 - 1
                if (i1456 > 0)
                    newline(pp)
                end
                pretty_snapshot_mapping(pp, elem1455)
            end
        end
        dedent!(pp)
        write(pp, ")")
...
function pretty_snapshot_mapping(pp::PrettyPrinter, msg::Proto.SnapshotMapping)
    flat1462 = try_flat(pp, msg, pretty_snapshot_mapping)
    if !isnothing(flat1462)
        write(pp, flat1462)
        return nothing
    else
        _dollar_dollar = msg
        fields1458 = (_dollar_dollar.destination_path, _dollar_dollar.source_relation,)
        unwrapped_fields1459 = fields1458
        field1460 = unwrapped_fields1459[1]
        pretty_edb_path(pp, field1460)
        write(pp, " ")
        field1461 = unwrapped_fields1459[2]
        pretty_relation_id(pp, field1461)
    end
    return nothing
end

function pretty_epoch_reads(pp::PrettyPrinter, msg::Vector{Proto.Read})
    flat1466 = try_flat(pp, msg, pretty_epoch_reads)
    if !isnothing(flat1466)
        ...
        if !isempty(fields1463)
            newline(pp)
            for (i1711, elem1464) in enumerate(fields1463)
                i1465 = i1711 - 1
                if (i1465 > 0)
                    newline(pp)
                end
                pretty_read(pp, elem1464)
            end
        end
        dedent!(pp)
        write(pp, ")")
...
function pretty_read(pp::PrettyPrinter, msg::Proto.Read)
    flat1477 = try_flat(pp, msg, pretty_read)
    if !isnothing(flat1477)
        ...
        if _has_proto_field(_dollar_dollar, Symbol("demand"))
            _t1712 = _get_oneof_field(_dollar_dollar, :demand)
        else
            _t1712 = nothing
        end
        deconstruct_result1475 = _t1712
        if !isnothing(deconstruct_result1475)
            unwrapped1476 = deconstruct_result1475
            pretty_demand(pp, unwrapped1476)
        else
            _dollar_dollar = msg
            if _has_proto_field(_dollar_dollar, Symbol("output"))
                _t1713 = _get_oneof_field(_dollar_dollar, :output)
            else
                _t1713 = nothing
            end
            deconstruct_result1473 = _t1713
            if !isnothing(deconstruct_result1473)
                unwrapped1474 = deconstruct_result1473
                pretty_output(pp, unwrapped1474)
            else
                _dollar_dollar = msg
                if _has_proto_field(_dollar_dollar, Symbol("what_if"))
                    _t1714 = _get_oneof_field(_dollar_dollar, :what_if)
                else
                    _t1714 = nothing
                end
                deconstruct_result1471 = _t1714
                if !isnothing(deconstruct_result1471)
                    unwrapped1472 = deconstruct_result1471
                    pretty_what_if(pp, unwrapped1472)
                else
                    _dollar_dollar = msg
                    if _has_proto_field(_dollar_dollar, Symbol("abort"))
                        _t1715 = _get_oneof_field(_dollar_dollar, :abort)
                    else
                        _t1715 = nothing
                    end
                    deconstruct_result1469 = _t1715
                    if !isnothing(deconstruct_result1469)
                        unwrapped1470 = deconstruct_result1469
                        pretty_abort(pp, unwrapped1470)
                    else
                        _dollar_dollar = msg
                        if _has_proto_field(_dollar_dollar, Symbol("#export"))
                            _t1716 = _get_oneof_field(_dollar_dollar, :var"#export")
                        else
                            _t1716 = nothing
                        end
                        deconstruct_result1467 = _t1716
                        if !isnothing(deconstruct_result1467)
                            unwrapped1468 = deconstruct_result1467
                            pretty_export(pp, unwrapped1468)
                        else
...
function pretty_demand(pp::PrettyPrinter, msg::Proto.Demand)
    flat1480 = try_flat(pp, msg, pretty_demand)
    if !isnothing(flat1480)
        write(pp, flat1480)
        return nothing
    else
        _dollar_dollar = msg
        fields1478 = _dollar_dollar.relation_id
        unwrapped_fields1479 = fields1478
        write(pp, "(demand")
        indent_sexp!(pp)
        newline(pp)
        pretty_relation_id(pp, unwrapped_fields1479)
        dedent!(pp)
        write(pp, ")")
    end
    return nothing
...
function pretty_output(pp::PrettyPrinter, msg::Proto.Output)
    flat1485 = try_flat(pp, msg, pretty_output)
    if !isnothing(flat1485)
        write(pp, flat1485)
        return nothing
    else
        _dollar_dollar = msg
        fields1481 = (_dollar_dollar.name, _dollar_dollar.relation_id,)
        ...
        pretty_relation_id(pp, field1484)
        dedent!(pp)
        write(pp, ")")
    end
    return nothing
...
function pretty_what_if(pp::PrettyPrinter, msg::Proto.WhatIf)
    flat1490 = try_flat(pp, msg, pretty_what_if)
    if !isnothing(flat1490)
        write(pp, flat1490)
        return nothing
    else
        _dollar_dollar = msg
        fields1486 = (_dollar_dollar.branch, _dollar_dollar.epoch,)
        unwrapped_fields1487 = fields1486
        write(pp, "(what_if")
        indent_sexp!(pp)
        newline(pp)
        field1488 = unwrapped_fields1487[1]
        pretty_name(pp, field1488)
        newline(pp)
        field1489 = unwrapped_fields1487[2]
        pretty_epoch(pp, field1489)
        dedent!(pp)
        write(pp, ")")
    end
    return nothing
...
function pretty_abort(pp::PrettyPrinter, msg::Proto.Abort)
    flat1496 = try_flat(pp, msg, pretty_abort)
    if !isnothing(flat1496)
        ...
        if _dollar_dollar.name != "abort"
            _t1717 = _dollar_dollar.name
        else
            _t1717 = nothing
        end
        fields1491 = (_t1717, _dollar_dollar.relation_id,)
        unwrapped_fields1492 = fields1491
        write(pp, "(abort")
        indent_sexp!(pp)
        field1493 = unwrapped_fields1492[1]
        if !isnothing(field1493)
            newline(pp)
            opt_val1494 = field1493
            pretty_name(pp, opt_val1494)
        end
        newline(pp)
        field1495 = unwrapped_fields1492[2]
        pretty_relation_id(pp, field1495)
        dedent!(pp)
        write(pp, ")")
    end
    return nothing
...
function pretty_export(pp::PrettyPrinter, msg::Proto.Export)
    flat1501 = try_flat(pp, msg, pretty_export)
    if !isnothing(flat1501)
        ...
        if _has_proto_field(_dollar_dollar, Symbol("csv_config"))
            _t1718 = _get_oneof_field(_dollar_dollar, :csv_config)
        else
            _t1718 = nothing
        end
        deconstruct_result1499 = _t1718
        if !isnothing(deconstruct_result1499)
            unwrapped1500 = deconstruct_result1499
            write(pp, "(export")
            indent_sexp!(pp)
            newline(pp)
            pretty_export_csv_config(pp, unwrapped1500)
            dedent!(pp)
            write(pp, ")")
        else
            _dollar_dollar = msg
            if _has_proto_field(_dollar_dollar, Symbol("iceberg_config"))
                _t1719 = _get_oneof_field(_dollar_dollar, :iceberg_config)
            else
                _t1719 = nothing
            end
            deconstruct_result1497 = _t1719
            if !isnothing(deconstruct_result1497)
                unwrapped1498 = deconstruct_result1497
                write(pp, "(export_iceberg")
                indent_sexp!(pp)
                newline(pp)
                pretty_export_iceberg_config(pp, unwrapped1498)
                dedent!(pp)
                write(pp, ")")
            else
...
function pretty_export_csv_config(pp::PrettyPrinter, msg::Proto.ExportCSVConfig)
    flat1512 = try_flat(pp, msg, pretty_export_csv_config)
    if !isnothing(flat1512)
        ...
        if length(_dollar_dollar.data_columns) == 0
            _t1720 = (_dollar_dollar.path, _dollar_dollar.csv_source, _dollar_dollar.csv_config,)
        else
            _t1720 = nothing
        end
        deconstruct_result1507 = _t1720
        if !isnothing(deconstruct_result1507)
            unwrapped1508 = deconstruct_result1507
            write(pp, "(export_csv_config_v2")
            indent_sexp!(pp)
            newline(pp)
            field1509 = unwrapped1508[1]
            pretty_export_csv_path(pp, field1509)
            newline(pp)
            field1510 = unwrapped1508[2]
            pretty_export_csv_source(pp, field1510)
            newline(pp)
            field1511 = unwrapped1508[3]
            pretty_csv_config(pp, field1511)
            dedent!(pp)
            write(pp, ")")
        else
            _dollar_dollar = msg
            if length(_dollar_dollar.data_columns) != 0
                _t1722 = deconstruct_export_csv_config(pp, _dollar_dollar)
                _t1721 = (_dollar_dollar.path, _dollar_dollar.data_columns, _t1722,)
            else
                _t1721 = nothing
            end
            deconstruct_result1502 = _t1721
            if !isnothing(deconstruct_result1502)
                ...
                pretty_export_csv_columns_list(pp, field1505)
                newline(pp)
                field1506 = unwrapped1503[3]
                pretty_config_dict(pp, field1506)
                dedent!(pp)
                write(pp, ")")
            else
...
function pretty_export_csv_path(pp::PrettyPrinter, msg::String)
    flat1514 = try_flat(pp, msg, pretty_export_csv_path)
    if !isnothing(flat1514)
        ...
        indent_sexp!(pp)
        newline(pp)
        write(pp, format_string(DEFAULT_CONSTANT_FORMATTER, pp, fields1513))
        dedent!(pp)
        write(pp, ")")
    end
    return nothing
...
function pretty_export_csv_source(pp::PrettyPrinter, msg::Proto.ExportCSVSource)
    flat1521 = try_flat(pp, msg, pretty_export_csv_source)
    if !isnothing(flat1521)
        ...
        if _has_proto_field(_dollar_dollar, Symbol("gnf_columns"))
            _t1723 = _get_oneof_field(_dollar_dollar, :gnf_columns).columns
        else
            _t1723 = nothing
        end
        deconstruct_result1517 = _t1723
        if !isnothing(deconstruct_result1517)
            unwrapped1518 = deconstruct_result1517
            write(pp, "(gnf_columns")
            indent_sexp!(pp)
            if !isempty(unwrapped1518)
                newline(pp)
                for (i1724, elem1519) in enumerate(unwrapped1518)
                    i1520 = i1724 - 1
                    if (i1520 > 0)
                        newline(pp)
                    end
            ...
            if _has_proto_field(_dollar_dollar, Symbol("table_def"))
                _t1725 = _get_oneof_field(_dollar_dollar, :table_def)
            else
                _t1725 = nothing
            end
            deconstruct_result1515 = _t1725
            if !isnothing(deconstruct_result1515)
                unwrapped1516 = deconstruct_result1515
                write(pp, "(table_def")
                indent_sexp!(pp)
                newline(pp)
                pretty_relation_id(pp, unwrapped1516)
                dedent!(pp)
                write(pp, ")")
            else
...
function pretty_export_csv_column(pp::PrettyPrinter, msg::Proto.ExportCSVColumn)
    flat1526 = try_flat(pp, msg, pretty_export_csv_column)
    if !isnothing(flat1526)
        write(pp, flat1526)
        return nothing
    else
        _dollar_dollar = msg
        fields1522 = (_dollar_dollar.column_name, _dollar_dollar.column_data,)
        unwrapped_fields1523 = fields1522
        write(pp, "(column")
        indent_sexp!(pp)
        newline(pp)
        field1524 = unwrapped_fields1523[1]
        write(pp, format_string(DEFAULT_CONSTANT_FORMATTER, pp, field1524))
        newline(pp)
...
function pretty_export_csv_columns_list(pp::PrettyPrinter, msg::Vector{Proto.ExportCSVColumn})
    flat1530 = try_flat(pp, msg, pretty_export_csv_columns_list)
    if !isnothing(flat1530)
        ...
        if !isempty(fields1527)
            newline(pp)
            for (i1726, elem1528) in enumerate(fields1527)
                i1529 = i1726 - 1
                if (i1529 > 0)
                    newline(pp)
                end
                pretty_export_csv_column(pp, elem1528)
            end
        end
        dedent!(pp)
        write(pp, ")")
...
function pretty_export_iceberg_config(pp::PrettyPrinter, msg::Proto.ExportIcebergConfig)
    flat1544 = try_flat(pp, msg, pretty_export_iceberg_config)
    if !isnothing(flat1544)
        write(pp, flat1544)
        return nothing
    else
        _dollar_dollar = msg
        _t1727 = deconstruct_export_iceberg_config_optional(pp, _dollar_dollar)
        fields1531 = (_dollar_dollar.locator, _dollar_dollar.config, _dollar_dollar.table_def, _dollar_dollar.columns, sort([(k, v) for (k, v) in _dollar_dollar.table_properties]), _t1727,)
        unwrapped_fields1532 = fields1531
        write(pp, "(export_iceberg_config")
        indent_sexp!(pp)
        newline(pp)
        field1533 = unwrapped_fields1532[1]
        pretty_iceberg_locator(pp, field1533)
        newline(pp)
        field1534 = unwrapped_fields1532[2]
        pretty_iceberg_catalog_config(pp, field1534)
        newline(pp)
        write(pp, "(")
        newline(pp)
        write(pp, "table_def")
        newline(pp)
        field1535 = unwrapped_fields1532[3]
        ...
        newline(pp)
        write(pp, "columns")
        field1536 = unwrapped_fields1532[4]
        if !isempty(field1536)
            newline(pp)
            for (i1728, elem1537) in enumerate(field1536)
                i1538 = i1728 - 1
                if (i1538 > 0)
                    newline(pp)
                end
                pretty_export_gnf_column(pp, elem1537)
            end
        end
        dedent!(pp)
        write(pp, ")")
        ...
        field1539 = unwrapped_fields1532[5]
        if !isempty(field1539)
            newline(pp)
            for (i1729, elem1540) in enumerate(field1539)
                i1541 = i1729 - 1
                if (i1541 > 0)
                    newline(pp)
                end
                pretty_iceberg_property_entry(pp, elem1540)
            end
        end
        dedent!(pp)
        write(pp, ")")
        field1542 = unwrapped_fields1532[6]
        if !isnothing(field1542)
            newline(pp)
            opt_val1543 = field1542
            pretty_config_dict(pp, opt_val1543)
        end
        dedent!(pp)
        write(pp, ")")
    end
...
function pretty_export_gnf_column(pp::PrettyPrinter, msg::Proto.ExportGNFColumn)
    flat1549 = try_flat(pp, msg, pretty_export_gnf_column)
    if !isnothing(flat1549)
        write(pp, flat1549)
        return nothing
    else
        _dollar_dollar = msg
        fields1545 = (_dollar_dollar.name, _dollar_dollar.nullable,)
        unwrapped_fields1546 = fields1545
        write(pp, "(gnf_column")
        indent_sexp!(pp)
        newline(pp)
        field1547 = unwrapped_fields1546[1]
        write(pp, format_string(DEFAULT_CONSTANT_FORMATTER, pp, field1547))
        newline(pp)
...
function pretty_debug_info(pp::PrettyPrinter, msg::Proto.DebugInfo)
    write(pp, "(debug_info")
    indent_sexp!(pp)
    for (i1775, _rid) in enumerate(msg.ids)
        _idx = i1775 - 1
        newline(pp)
        write(pp, "(")
        _t1776 = Proto.UInt128Value(low=_rid.id_low, high=_rid.id_high)
        _pprint_dispatch(pp, _t1776)
        write(pp, " ")
...
function pretty_functional_dependency(pp::PrettyPrinter, msg::Proto.FunctionalDependency)
    ...
    newline(pp)
    write(pp, ":keys (")
    for (i1777, _elem) in enumerate(msg.keys)
        _idx = i1777 - 1
        if (_idx > 0)
            write(pp, " ")
        end
    ...
    newline(pp)
    write(pp, ":values (")
    for (i1778, _elem) in enumerate(msg.values)
        _idx = i1778 - 1
        if (_idx > 0)
            write(pp, " ")
        end
...
function pretty_export_csv_columns(pp::PrettyPrinter, msg::Proto.ExportCSVColumns)
    ...
    newline(pp)
    write(pp, ":columns (")
    for (i1779, _elem) in enumerate(msg.columns)
        _idx = i1779 - 1
        if (_idx > 0)
            write(pp, " ")
        end
