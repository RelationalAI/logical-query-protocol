...
function deconstruct_configure(pp::PrettyPrinter, msg::Proto.Configure)::Vector{Tuple{String, Proto.Value}}
    ...
    if msg.ivm_config.level == Proto.MaintenanceLevel.MAINTENANCE_LEVEL_AUTO
        _t1799 = _make_value_string(pp, "auto")
        push!(result, ("ivm.maintenance_level", _t1799,))
    else
        if msg.ivm_config.level == Proto.MaintenanceLevel.MAINTENANCE_LEVEL_ALL
            _t1800 = _make_value_string(pp, "all")
            push!(result, ("ivm.maintenance_level", _t1800,))
        else
            if msg.ivm_config.level == Proto.MaintenanceLevel.MAINTENANCE_LEVEL_OFF
                _t1801 = _make_value_string(pp, "off")
                push!(result, ("ivm.maintenance_level", _t1801,))
            end
        end
    end
    _t1802 = _make_value_int64(pp, msg.semantics_version)
    push!(result, ("semantics_version", _t1802,))
    return sort(result)
end

function deconstruct_csv_config(pp::PrettyPrinter, msg::Proto.CSVConfig)::Vector{Tuple{String, Proto.Value}}
    result = Tuple{String, Proto.Value}[]
    _t1803 = _make_value_int32(pp, msg.header_row)
    push!(result, ("csv_header_row", _t1803,))
    _t1804 = _make_value_int64(pp, msg.skip)
    push!(result, ("csv_skip", _t1804,))
    if msg.new_line != ""
        _t1805 = _make_value_string(pp, msg.new_line)
        push!(result, ("csv_new_line", _t1805,))
    end
    _t1806 = _make_value_string(pp, msg.delimiter)
    push!(result, ("csv_delimiter", _t1806,))
    _t1807 = _make_value_string(pp, msg.quotechar)
    push!(result, ("csv_quotechar", _t1807,))
    _t1808 = _make_value_string(pp, msg.escapechar)
    push!(result, ("csv_escapechar", _t1808,))
    if msg.comment != ""
        _t1809 = _make_value_string(pp, msg.comment)
        push!(result, ("csv_comment", _t1809,))
    end
    for missing_string in msg.missing_strings
        _t1810 = _make_value_string(pp, missing_string)
        push!(result, ("csv_missing_strings", _t1810,))
    end
    _t1811 = _make_value_string(pp, msg.decimal_separator)
    push!(result, ("csv_decimal_separator", _t1811,))
    _t1812 = _make_value_string(pp, msg.encoding)
    push!(result, ("csv_encoding", _t1812,))
    _t1813 = _make_value_string(pp, msg.compression)
    push!(result, ("csv_compression", _t1813,))
    if msg.partition_size_mb != 0
        _t1814 = _make_value_int64(pp, msg.partition_size_mb)
        push!(result, ("csv_partition_size_mb", _t1814,))
    end
    return sort(result)
end
...
function deconstruct_betree_info_config(pp::PrettyPrinter, msg::Proto.BeTreeInfo)::Vector{Tuple{String, Proto.Value}}
    result = Tuple{String, Proto.Value}[]
    _t1815 = _make_value_float64(pp, msg.storage_config.epsilon)
    push!(result, ("betree_config_epsilon", _t1815,))
    _t1816 = _make_value_int64(pp, msg.storage_config.max_pivots)
    push!(result, ("betree_config_max_pivots", _t1816,))
    _t1817 = _make_value_int64(pp, msg.storage_config.max_deltas)
    push!(result, ("betree_config_max_deltas", _t1817,))
    _t1818 = _make_value_int64(pp, msg.storage_config.max_leaf)
    push!(result, ("betree_config_max_leaf", _t1818,))
    if _has_proto_field(msg.relation_locator, Symbol("root_pageid"))
        if !isnothing(_get_oneof_field(msg.relation_locator, :root_pageid))
            _t1819 = _make_value_uint128(pp, _get_oneof_field(msg.relation_locator, :root_pageid))
            push!(result, ("betree_locator_root_pageid", _t1819,))
        end
    end
    if _has_proto_field(msg.relation_locator, Symbol("inline_data"))
        if !isnothing(_get_oneof_field(msg.relation_locator, :inline_data))
            _t1820 = _make_value_string(pp, String(copy(_get_oneof_field(msg.relation_locator, :inline_data))))
            push!(result, ("betree_locator_inline_data", _t1820,))
        end
    end
    _t1821 = _make_value_int64(pp, msg.relation_locator.element_count)
    push!(result, ("betree_locator_element_count", _t1821,))
    _t1822 = _make_value_int64(pp, msg.relation_locator.tree_height)
    push!(result, ("betree_locator_tree_height", _t1822,))
    return sort(result)
end
...
function deconstruct_export_csv_config(pp::PrettyPrinter, msg::Proto.ExportCSVConfig)::Vector{Tuple{String, Proto.Value}}
    ...
    if !isnothing(msg.partition_size)
        _t1823 = _make_value_int64(pp, msg.partition_size)
        push!(result, ("partition_size", _t1823,))
    end
    if !isnothing(msg.compression)
        _t1824 = _make_value_string(pp, msg.compression)
        push!(result, ("compression", _t1824,))
    end
    if !isnothing(msg.syntax_header_row)
        _t1825 = _make_value_boolean(pp, msg.syntax_header_row)
        push!(result, ("syntax_header_row", _t1825,))
    end
    if !isnothing(msg.syntax_missing_string)
        _t1826 = _make_value_string(pp, msg.syntax_missing_string)
        push!(result, ("syntax_missing_string", _t1826,))
    end
    if !isnothing(msg.syntax_delim)
        _t1827 = _make_value_string(pp, msg.syntax_delim)
        push!(result, ("syntax_delim", _t1827,))
    end
    if !isnothing(msg.syntax_quotechar)
        _t1828 = _make_value_string(pp, msg.syntax_quotechar)
        push!(result, ("syntax_quotechar", _t1828,))
    end
    if !isnothing(msg.syntax_escapechar)
        _t1829 = _make_value_string(pp, msg.syntax_escapechar)
        push!(result, ("syntax_escapechar", _t1829,))
    end
    return sort(result)
end
...
function deconstruct_export_iceberg_config_optional(pp::PrettyPrinter, msg::Proto.ExportIcebergConfig)::Union{Nothing, Vector{Tuple{String, Proto.Value}}}
    result = Tuple{String, Proto.Value}[]
    if msg.prefix != ""
        _t1833 = _make_value_string(pp, msg.prefix)
        push!(result, ("prefix", _t1833,))
    end
    if msg.target_file_size_bytes != 0
        _t1834 = _make_value_int64(pp, msg.target_file_size_bytes)
        push!(result, ("target_file_size_bytes", _t1834,))
    end
    if msg.compression != ""
        _t1835 = _make_value_string(pp, msg.compression)
        push!(result, ("compression", _t1835,))
    end
    if length(result) == 0
        return nothing
    else
...
function pretty_iceberg_data(pp::PrettyPrinter, msg::Proto.IcebergData)
    flat1449 = try_flat(pp, msg, pretty_iceberg_data)
    if !isnothing(flat1449)
        write(pp, flat1449)
        return nothing
    else
        _dollar_dollar = msg
        if !isempty(_dollar_dollar.columns)
            _t1763 = _dollar_dollar.columns
        else
            _t1763 = nothing
        end
        if _has_proto_field(_dollar_dollar, Symbol("target"))
            _t1764 = _dollar_dollar.target
        else
            _t1764 = nothing
        end
        _t1765 = deconstruct_iceberg_data_from_snapshot_optional(pp, _dollar_dollar)
        _t1766 = deconstruct_iceberg_data_to_snapshot_optional(pp, _dollar_dollar)
        fields1436 = (_dollar_dollar.locator, _dollar_dollar.config, _t1763, _t1764, _t1765, _t1766, _dollar_dollar.returns_delta,)
        ...
        if !isnothing(field1440)
            ...
            pretty_gnf_columns(pp, opt_val1441)
        end
        field1442 = unwrapped_fields1437[4]
        if !isnothing(field1442)
            newline(pp)
            opt_val1443 = field1442
            pretty_full_table(pp, opt_val1443)
        end
        field1444 = unwrapped_fields1437[5]
        if !isnothing(field1444)
            newline(pp)
            opt_val1445 = field1444
            pretty_iceberg_from_snapshot(pp, opt_val1445)
        end
        field1446 = unwrapped_fields1437[6]
        if !isnothing(field1446)
            newline(pp)
            opt_val1447 = field1446
            pretty_iceberg_to_snapshot(pp, opt_val1447)
        end
        newline(pp)
        field1448 = unwrapped_fields1437[7]
        pretty_boolean_value(pp, field1448)
        dedent!(pp)
        write(pp, ")")
    end
    return nothing
...
function pretty_iceberg_locator(pp::PrettyPrinter, msg::Proto.IcebergLocator)
    flat1455 = try_flat(pp, msg, pretty_iceberg_locator)
    if !isnothing(flat1455)
        write(pp, flat1455)
        return nothing
    else
        _dollar_dollar = msg
        fields1450 = (_dollar_dollar.table_name, _dollar_dollar.namespace, _dollar_dollar.warehouse,)
        unwrapped_fields1451 = fields1450
        write(pp, "(iceberg_locator")
        indent_sexp!(pp)
        newline(pp)
        field1452 = unwrapped_fields1451[1]
        pretty_iceberg_locator_table_name(pp, field1452)
        newline(pp)
        field1453 = unwrapped_fields1451[2]
        pretty_iceberg_locator_namespace(pp, field1453)
        newline(pp)
        field1454 = unwrapped_fields1451[3]
        pretty_iceberg_locator_warehouse(pp, field1454)
        dedent!(pp)
        write(pp, ")")
    end
    return nothing
...
function pretty_iceberg_locator_table_name(pp::PrettyPrinter, msg::String)
    flat1457 = try_flat(pp, msg, pretty_iceberg_locator_table_name)
    if !isnothing(flat1457)
        ...
        indent_sexp!(pp)
        newline(pp)
        write(pp, format_string(DEFAULT_CONSTANT_FORMATTER, pp, fields1456))
        dedent!(pp)
        write(pp, ")")
    end
    return nothing
...
function pretty_iceberg_locator_namespace(pp::PrettyPrinter, msg::Vector{String})
    flat1461 = try_flat(pp, msg, pretty_iceberg_locator_namespace)
    if !isnothing(flat1461)
        ...
        if !isempty(fields1458)
            newline(pp)
            for (i1767, elem1459) in enumerate(fields1458)
                i1460 = i1767 - 1
                if (i1460 > 0)
                    newline(pp)
                end
...
function pretty_iceberg_locator_warehouse(pp::PrettyPrinter, msg::String)
    flat1463 = try_flat(pp, msg, pretty_iceberg_locator_warehouse)
    if !isnothing(flat1463)
        ...
        indent_sexp!(pp)
        newline(pp)
        write(pp, format_string(DEFAULT_CONSTANT_FORMATTER, pp, fields1462))
        dedent!(pp)
        write(pp, ")")
    end
    return nothing
...
function pretty_iceberg_catalog_config(pp::PrettyPrinter, msg::Proto.IcebergCatalogConfig)
    flat1471 = try_flat(pp, msg, pretty_iceberg_catalog_config)
    if !isnothing(flat1471)
        write(pp, flat1471)
        return nothing
    else
        _dollar_dollar = msg
        _t1768 = deconstruct_iceberg_catalog_config_scope_optional(pp, _dollar_dollar)
        fields1464 = (_dollar_dollar.catalog_uri, _t1768, sort([(k, v) for (k, v) in _dollar_dollar.properties]), sort([(k, v) for (k, v) in _dollar_dollar.auth_properties]),)
        unwrapped_fields1465 = fields1464
        write(pp, "(iceberg_catalog_config")
        indent_sexp!(pp)
        newline(pp)
        field1466 = unwrapped_fields1465[1]
        pretty_iceberg_catalog_uri(pp, field1466)
        field1467 = unwrapped_fields1465[2]
        if !isnothing(field1467)
            newline(pp)
            opt_val1468 = field1467
            pretty_iceberg_catalog_config_scope(pp, opt_val1468)
        end
        newline(pp)
        field1469 = unwrapped_fields1465[3]
        pretty_iceberg_properties(pp, field1469)
        newline(pp)
        field1470 = unwrapped_fields1465[4]
        pretty_iceberg_auth_properties(pp, field1470)
        dedent!(pp)
        write(pp, ")")
    end
    return nothing
...
function pretty_iceberg_catalog_uri(pp::PrettyPrinter, msg::String)
    flat1473 = try_flat(pp, msg, pretty_iceberg_catalog_uri)
    if !isnothing(flat1473)
        ...
        indent_sexp!(pp)
        newline(pp)
        write(pp, format_string(DEFAULT_CONSTANT_FORMATTER, pp, fields1472))
        dedent!(pp)
        write(pp, ")")
    end
    return nothing
...
function pretty_iceberg_catalog_config_scope(pp::PrettyPrinter, msg::String)
    flat1475 = try_flat(pp, msg, pretty_iceberg_catalog_config_scope)
...
function pretty_iceberg_properties(pp::PrettyPrinter, msg::Vector{Tuple{String, String}})
    flat1479 = try_flat(pp, msg, pretty_iceberg_properties)
    if !isnothing(flat1479)
        ...
        if !isempty(fields1476)
            newline(pp)
            for (i1769, elem1477) in enumerate(fields1476)
                i1478 = i1769 - 1
                if (i1478 > 0)
                    newline(pp)
                end
                pretty_iceberg_property_entry(pp, elem1477)
            end
        end
        dedent!(pp)
        write(pp, ")")
...
function pretty_iceberg_property_entry(pp::PrettyPrinter, msg::Tuple{String, String})
    flat1484 = try_flat(pp, msg, pretty_iceberg_property_entry)
    if !isnothing(flat1484)
        write(pp, flat1484)
        return nothing
    else
        _dollar_dollar = msg
        fields1480 = (_dollar_dollar[1], _dollar_dollar[2],)
...
function pretty_iceberg_auth_properties(pp::PrettyPrinter, msg::Vector{Tuple{String, String}})
    flat1488 = try_flat(pp, msg, pretty_iceberg_auth_properties)
    if !isnothing(flat1488)
        ...
        if !isempty(fields1485)
            newline(pp)
            for (i1770, elem1486) in enumerate(fields1485)
                i1487 = i1770 - 1
                if (i1487 > 0)
                    newline(pp)
                end
                pretty_iceberg_masked_property_entry(pp, elem1486)
            end
        end
        dedent!(pp)
        write(pp, ")")
...
function pretty_iceberg_masked_property_entry(pp::PrettyPrinter, msg::Tuple{String, String})
    flat1493 = try_flat(pp, msg, pretty_iceberg_masked_property_entry)
    if !isnothing(flat1493)
        write(pp, flat1493)
        return nothing
    else
        _dollar_dollar = msg
        _t1771 = mask_secret_value(pp, _dollar_dollar)
        fields1489 = (_dollar_dollar[1], _t1771,)
...
function pretty_full_table(pp::PrettyPrinter, msg::Proto.IcebergTarget)
    flat1500 = try_flat(pp, msg, pretty_full_table)
    if !isnothing(flat1500)
        write(pp, flat1500)
        return nothing
    else
        _dollar_dollar = msg
        fields1494 = (_dollar_dollar.target_id, _dollar_dollar.types,)
        unwrapped_fields1495 = fields1494
        write(pp, "(full_table")
        indent_sexp!(pp)
        newline(pp)
        field1496 = unwrapped_fields1495[1]
        pretty_relation_id(pp, field1496)
        newline(pp)
        write(pp, "[")
        field1497 = unwrapped_fields1495[2]
        for (i1772, elem1498) in enumerate(field1497)
            i1499 = i1772 - 1
            if (i1499 > 0)
                newline(pp)
            end
            pretty_type(pp, elem1498)
        end
        write(pp, "]")
        dedent!(pp)
        write(pp, ")")
...
function pretty_context(pp::PrettyPrinter, msg::Proto.Context)
    ...
    if !isnothing(flat1512)
        ...
        if !isempty(unwrapped_fields1509)
            newline(pp)
            for (i1773, elem1510) in enumerate(unwrapped_fields1509)
                i1511 = i1773 - 1
                if (i1511 > 0)
                    newline(pp)
                end
...
function pretty_snapshot(pp::PrettyPrinter, msg::Proto.Snapshot)
    ...
    if !isnothing(flat1519)
        ...
        if !isempty(field1516)
            newline(pp)
            for (i1774, elem1517) in enumerate(field1516)
                i1518 = i1774 - 1
                if (i1518 > 0)
                    newline(pp)
                end
...
function pretty_epoch_reads(pp::PrettyPrinter, msg::Vector{Proto.Read})
    ...
    if !isnothing(flat1528)
        ...
        if !isempty(fields1525)
            newline(pp)
            for (i1775, elem1526) in enumerate(fields1525)
                i1527 = i1775 - 1
                if (i1527 > 0)
                    newline(pp)
                end
...
function pretty_read(pp::PrettyPrinter, msg::Proto.Read)
    flat1539 = try_flat(pp, msg, pretty_read)
...
function pretty_demand(pp::PrettyPrinter, msg::Proto.Demand)
    flat1542 = try_flat(pp, msg, pretty_demand)
    if !isnothing(flat1542)
        write(pp, flat1542)
        return nothing
    else
        _dollar_dollar = msg
        fields1540 = _dollar_dollar.relation_id
        unwrapped_fields1541 = fields1540
        write(pp, "(demand")
        indent_sexp!(pp)
        newline(pp)
        pretty_relation_id(pp, unwrapped_fields1541)
        dedent!(pp)
        write(pp, ")")
    end
    return nothing
...
function pretty_output(pp::PrettyPrinter, msg::Proto.Output)
    flat1547 = try_flat(pp, msg, pretty_output)
    if !isnothing(flat1547)
        write(pp, flat1547)
        return nothing
    else
        _dollar_dollar = msg
        fields1543 = (_dollar_dollar.name, _dollar_dollar.relation_id,)
        ...
        pretty_relation_id(pp, field1546)
        dedent!(pp)
        write(pp, ")")
    end
    return nothing
...
function pretty_what_if(pp::PrettyPrinter, msg::Proto.WhatIf)
    flat1552 = try_flat(pp, msg, pretty_what_if)
    if !isnothing(flat1552)
        write(pp, flat1552)
        return nothing
    else
        _dollar_dollar = msg
        fields1548 = (_dollar_dollar.branch, _dollar_dollar.epoch,)
        unwrapped_fields1549 = fields1548
        write(pp, "(what_if")
        indent_sexp!(pp)
        newline(pp)
        field1550 = unwrapped_fields1549[1]
        pretty_name(pp, field1550)
        newline(pp)
        field1551 = unwrapped_fields1549[2]
        pretty_epoch(pp, field1551)
        dedent!(pp)
        write(pp, ")")
    end
    return nothing
...
function pretty_abort(pp::PrettyPrinter, msg::Proto.Abort)
    flat1558 = try_flat(pp, msg, pretty_abort)
    if !isnothing(flat1558)
        ...
        if _dollar_dollar.name != "abort"
            _t1781 = _dollar_dollar.name
        else
            _t1781 = nothing
        end
        fields1553 = (_t1781, _dollar_dollar.relation_id,)
        unwrapped_fields1554 = fields1553
        write(pp, "(abort")
        indent_sexp!(pp)
        field1555 = unwrapped_fields1554[1]
        if !isnothing(field1555)
            newline(pp)
            opt_val1556 = field1555
            pretty_name(pp, opt_val1556)
        end
        newline(pp)
        field1557 = unwrapped_fields1554[2]
        pretty_relation_id(pp, field1557)
        dedent!(pp)
        write(pp, ")")
    end
    return nothing
...
function pretty_export(pp::PrettyPrinter, msg::Proto.Export)
    flat1563 = try_flat(pp, msg, pretty_export)
...
function pretty_export_csv_config(pp::PrettyPrinter, msg::Proto.ExportCSVConfig)
    flat1574 = try_flat(pp, msg, pretty_export_csv_config)
    if !isnothing(flat1574)
        ...
        if !isnothing(deconstruct_result1569)
            ...
            indent_sexp!(pp)
            newline(pp)
            field1571 = unwrapped1570[1]
            pretty_export_csv_path(pp, field1571)
            newline(pp)
            field1572 = unwrapped1570[2]
            pretty_export_csv_source(pp, field1572)
            newline(pp)
            field1573 = unwrapped1570[3]
            pretty_csv_config(pp, field1573)
            dedent!(pp)
            write(pp, ")")
        else
            _dollar_dollar = msg
            if length(_dollar_dollar.data_columns) != 0
                _t1786 = deconstruct_export_csv_config(pp, _dollar_dollar)
                _t1785 = (_dollar_dollar.path, _dollar_dollar.data_columns, _t1786,)
            else
                _t1785 = nothing
            end
            deconstruct_result1564 = _t1785
            if !isnothing(deconstruct_result1564)
                ...
                pretty_export_csv_path(pp, field1566)
                newline(pp)
                field1567 = unwrapped1565[2]
                pretty_export_csv_columns_list(pp, field1567)
                newline(pp)
                field1568 = unwrapped1565[3]
                pretty_config_dict(pp, field1568)
                dedent!(pp)
                write(pp, ")")
            else
...
function pretty_export_csv_path(pp::PrettyPrinter, msg::String)
    flat1576 = try_flat(pp, msg, pretty_export_csv_path)
    if !isnothing(flat1576)
        ...
        indent_sexp!(pp)
        newline(pp)
        write(pp, format_string(DEFAULT_CONSTANT_FORMATTER, pp, fields1575))
        dedent!(pp)
        write(pp, ")")
    end
    return nothing
...
function pretty_export_csv_source(pp::PrettyPrinter, msg::Proto.ExportCSVSource)
    flat1583 = try_flat(pp, msg, pretty_export_csv_source)
    if !isnothing(flat1583)
        ...
        if _has_proto_field(_dollar_dollar, Symbol("gnf_columns"))
            _t1787 = _get_oneof_field(_dollar_dollar, :gnf_columns).columns
        else
            _t1787 = nothing
        end
        deconstruct_result1579 = _t1787
        if !isnothing(deconstruct_result1579)
            unwrapped1580 = deconstruct_result1579
            write(pp, "(gnf_columns")
            indent_sexp!(pp)
            if !isempty(unwrapped1580)
                newline(pp)
                for (i1788, elem1581) in enumerate(unwrapped1580)
                    i1582 = i1788 - 1
                    if (i1582 > 0)
                        newline(pp)
                    end
                    pretty_export_csv_column(pp, elem1581)
                end
            end
            dedent!(pp)
            write(pp, ")")
            ...
            if _has_proto_field(_dollar_dollar, Symbol("table_def"))
                _t1789 = _get_oneof_field(_dollar_dollar, :table_def)
            else
                _t1789 = nothing
            end
            deconstruct_result1577 = _t1789
            if !isnothing(deconstruct_result1577)
                unwrapped1578 = deconstruct_result1577
                write(pp, "(table_def")
                indent_sexp!(pp)
                newline(pp)
                pretty_relation_id(pp, unwrapped1578)
                dedent!(pp)
                write(pp, ")")
            else
...
function pretty_export_csv_column(pp::PrettyPrinter, msg::Proto.ExportCSVColumn)
    flat1588 = try_flat(pp, msg, pretty_export_csv_column)
    if !isnothing(flat1588)
        write(pp, flat1588)
        return nothing
    else
        _dollar_dollar = msg
        fields1584 = (_dollar_dollar.column_name, _dollar_dollar.column_data,)
        unwrapped_fields1585 = fields1584
        write(pp, "(column")
        indent_sexp!(pp)
        newline(pp)
        field1586 = unwrapped_fields1585[1]
        write(pp, format_string(DEFAULT_CONSTANT_FORMATTER, pp, field1586))
        newline(pp)
        field1587 = unwrapped_fields1585[2]
        pretty_relation_id(pp, field1587)
        dedent!(pp)
        write(pp, ")")
    end
    return nothing
...
function pretty_export_csv_columns_list(pp::PrettyPrinter, msg::Vector{Proto.ExportCSVColumn})
    ...
    if !isnothing(flat1592)
        ...
        if !isempty(fields1589)
            newline(pp)
            for (i1790, elem1590) in enumerate(fields1589)
                i1591 = i1790 - 1
                if (i1591 > 0)
                    newline(pp)
                end
...
function pretty_export_iceberg_config(pp::PrettyPrinter, msg::Proto.ExportIcebergConfig)
    ...
    if !isnothing(flat1601)
        ...
        _dollar_dollar = msg
        _t1791 = deconstruct_export_iceberg_config_optional(pp, _dollar_dollar)
        fields1593 = (_dollar_dollar.locator, _dollar_dollar.config, _dollar_dollar.table_def, sort([(k, v) for (k, v) in _dollar_dollar.table_properties]), _t1791,)
...
function pretty_iceberg_table_properties(pp::PrettyPrinter, msg::Vector{Tuple{String, String}})
    ...
    if !isnothing(flat1607)
        ...
        if !isempty(fields1604)
            newline(pp)
            for (i1792, elem1605) in enumerate(fields1604)
                i1606 = i1792 - 1
                if (i1606 > 0)
                    newline(pp)
                end
...
function pretty_debug_info(pp::PrettyPrinter, msg::Proto.DebugInfo)
    write(pp, "(debug_info")
    indent_sexp!(pp)
    for (i1838, _rid) in enumerate(msg.ids)
        _idx = i1838 - 1
        newline(pp)
        write(pp, "(")
        _t1839 = Proto.UInt128Value(low=_rid.id_low, high=_rid.id_high)
        _pprint_dispatch(pp, _t1839)
        write(pp, " ")
...
function pretty_functional_dependency(pp::PrettyPrinter, msg::Proto.FunctionalDependency)
    ...
    newline(pp)
    write(pp, ":keys (")
    for (i1840, _elem) in enumerate(msg.keys)
        _idx = i1840 - 1
        if (_idx > 0)
            write(pp, " ")
        end
    ...
    newline(pp)
    write(pp, ":values (")
    for (i1841, _elem) in enumerate(msg.values)
        _idx = i1841 - 1
        if (_idx > 0)
            write(pp, " ")
        end
...
function pretty_export_csv_columns(pp::PrettyPrinter, msg::Proto.ExportCSVColumns)
    ...
    newline(pp)
    write(pp, ":columns (")
    for (i1842, _elem) in enumerate(msg.columns)
        _idx = i1842 - 1
        if (_idx > 0)
            write(pp, " ")
        end
...
_pprint_dispatch(pp::PrettyPrinter, x::Proto.IcebergTarget) = pretty_full_table(pp, x)
