...
# TargetRelations
Base.:(==)(a::TargetRelations, b::TargetRelations) = a.keys == b.keys && a.synthetic_key == b.synthetic_key && a.load_errors == b.load_errors && _isequal_oneof(a.body, b.body)
Base.hash(a::TargetRelations, h::UInt) = _hash_oneof(a.body, hash(a.load_errors, hash(a.synthetic_key, hash(a.keys, h))))
Base.isequal(a::TargetRelations, b::TargetRelations) = isequal(a.keys, b.keys) && isequal(a.synthetic_key, b.synthetic_key) && isequal(a.load_errors, b.load_errors) && _isequal_oneof(a.body, b.body)
