...
func (p *Parser) export_csv_config(path string, columns []*pb.ExportCSVColumn, config_dict [][]interface{}) *pb.ExportCSVConfig {
	...
	_t957 := p._extract_value_int64(dictGetValue(config, "partition_size"), 0)
	partition_size := _t957
	_t958 := p._extract_value_string(dictGetValue(config, "compression"), "")
	compression := _t958
	_t959 := p._extract_value_boolean(dictGetValue(config, "syntax_header_row"), true)
	syntax_header_row := _t959
	_t960 := p._extract_value_string(dictGetValue(config, "syntax_missing_string"), "")
	syntax_missing_string := _t960
	_t961 := p._extract_value_string(dictGetValue(config, "syntax_delim"), ",")
	syntax_delim := _t961
	_t962 := p._extract_value_string(dictGetValue(config, "syntax_quotechar"), "\"")
	syntax_quotechar := _t962
	_t963 := p._extract_value_string(dictGetValue(config, "syntax_escapechar"), "\\")
	syntax_escapechar := _t963
	_t964 := &pb.ExportCSVConfig{Path: path, DataColumns: columns, PartitionSize: ptr(partition_size), Compression: ptr(compression), SyntaxHeaderRow: ptr(syntax_header_row), SyntaxMissingString: ptr(syntax_missing_string), SyntaxDelim: ptr(syntax_delim), SyntaxQuotechar: ptr(syntax_quotechar), SyntaxEscapechar: ptr(syntax_escapechar)}
	return _t964
}
...
func (p *Parser) construct_betree_info(key_types []*pb.Type, value_types []*pb.Type, config_dict [][]interface{}) *pb.BeTreeInfo {
	config := dictFromList(config_dict)
	_t965 := p._try_extract_value_float64(dictGetValue(config, "betree_config_epsilon"))
	epsilon := _t965
	_t966 := p._try_extract_value_int64(dictGetValue(config, "betree_config_max_pivots"))
	max_pivots := _t966
	_t967 := p._try_extract_value_int64(dictGetValue(config, "betree_config_max_deltas"))
	max_deltas := _t967
	_t968 := p._try_extract_value_int64(dictGetValue(config, "betree_config_max_leaf"))
	max_leaf := _t968
	_t969 := &pb.BeTreeConfig{Epsilon: deref(epsilon, 0.0), MaxPivots: deref(max_pivots, 0), MaxDeltas: deref(max_deltas, 0), MaxLeaf: deref(max_leaf, 0)}
	storage_config := _t969
	_t970 := p._try_extract_value_uint128(dictGetValue(config, "betree_locator_root_pageid"))
	root_pageid := _t970
	_t971 := p._try_extract_value_bytes(dictGetValue(config, "betree_locator_inline_data"))
	inline_data := _t971
	_t972 := p._try_extract_value_int64(dictGetValue(config, "betree_locator_element_count"))
	element_count := _t972
	_t973 := p._try_extract_value_int64(dictGetValue(config, "betree_locator_tree_height"))
	tree_height := _t973
	_t974 := &pb.BeTreeLocator{ElementCount: deref(element_count, 0), TreeHeight: deref(tree_height, 0)}
	if root_pageid != nil {
		_t974.Location = &pb.BeTreeLocator_RootPageid{RootPageid: root_pageid}
	} else {
		_t974.Location = &pb.BeTreeLocator_InlineData{InlineData: inline_data}
	}
	relation_locator := _t974
	_t975 := &pb.BeTreeInfo{KeyTypes: key_types, ValueTypes: value_types, StorageConfig: storage_config, RelationLocator: relation_locator}
	return _t975
}
...
func (p *Parser) construct_csv_config(config_dict [][]interface{}) *pb.CSVConfig {
	config := dictFromList(config_dict)
	_t976 := p._extract_value_int32(dictGetValue(config, "csv_header_row"), 1)
	header_row := _t976
	_t977 := p._extract_value_int64(dictGetValue(config, "csv_skip"), 0)
	skip := _t977
	_t978 := p._extract_value_string(dictGetValue(config, "csv_new_line"), "")
	new_line := _t978
	_t979 := p._extract_value_string(dictGetValue(config, "csv_delimiter"), ",")
	delimiter := _t979
	_t980 := p._extract_value_string(dictGetValue(config, "csv_quotechar"), "\"")
	quotechar := _t980
	_t981 := p._extract_value_string(dictGetValue(config, "csv_escapechar"), "\"")
	escapechar := _t981
	_t982 := p._extract_value_string(dictGetValue(config, "csv_comment"), "")
	comment := _t982
	_t983 := p._extract_value_string_list(dictGetValue(config, "csv_missing_strings"), []string{})
	missing_strings := _t983
	_t984 := p._extract_value_string(dictGetValue(config, "csv_decimal_separator"), ".")
	decimal_separator := _t984
	_t985 := p._extract_value_string(dictGetValue(config, "csv_encoding"), "utf-8")
	encoding := _t985
	_t986 := p._extract_value_string(dictGetValue(config, "csv_compression"), "auto")
	compression := _t986
	_t987 := &pb.CSVConfig{HeaderRow: header_row, Skip: skip, NewLine: new_line, Delimiter: delimiter, Quotechar: quotechar, Escapechar: escapechar, Comment: comment, MissingStrings: missing_strings, DecimalSeparator: decimal_separator, Encoding: encoding, Compression: compression}
	return _t987
}
...
func (p *Parser) default_configure() *pb.Configure {
	_t988 := &pb.IVMConfig{Level: pb.MaintenanceLevel_MAINTENANCE_LEVEL_OFF}
	ivm_config := _t988
	_t989 := &pb.Configure{SemanticsVersion: 0, IvmConfig: ivm_config}
	return _t989
}
...
func (p *Parser) construct_configure(config_dict [][]interface{}) *pb.Configure {
	...
	_t990 := &pb.IVMConfig{Level: maintenance_level}
	ivm_config := _t990
	_t991 := p._extract_value_int64(dictGetValue(config, "semantics_version"), 0)
	semantics_version := _t991
	_t992 := &pb.Configure{SemanticsVersion: semantics_version, IvmConfig: ivm_config}
	return _t992
}
...
func (p *Parser) parse_output() *pb.Output {
	...
	name336 := _t937
	_t939 := p.parse_relation_id()
	relation_id337 := _t939
	p.consumeLiteral(")")
	_t940 := &pb.Output{Name: deref(name336, "output"), RelationId: relation_id337}
	return _t940
}
...
func (p *Parser) parse_what_if() *pb.WhatIf {
	...
	epoch339 := _t942
	p.consumeLiteral(")")
	_t943 := &pb.WhatIf{Branch: name338, Epoch: epoch339}
	return _t943
}
...
func (p *Parser) parse_abort() *pb.Abort {
	...
	name340 := _t944
	_t946 := p.parse_relation_id()
	relation_id341 := _t946
	p.consumeLiteral(")")
	_t947 := &pb.Abort{Name: deref(name340, "abort"), RelationId: relation_id341}
	return _t947
}
...
func (p *Parser) parse_export() *pb.Export {
	...
	_t948 := p.parse_export_csv_config()
	export_csv_config342 := _t948
	p.consumeLiteral(")")
	_t949 := &pb.Export{}
	_t949.ExportConfig = &pb.Export_CsvConfig{CsvConfig: export_csv_config342}
	return _t949
}
...
func (p *Parser) parse_export_csv_config() *pb.ExportCSVConfig {
	...
	_t950 := p.parse_export_csv_path()
	export_csv_path343 := _t950
	_t951 := p.parse_export_csv_columns()
	export_csv_columns344 := _t951
	_t952 := p.parse_config_dict()
	config_dict345 := _t952
	p.consumeLiteral(")")
	_t953 := p.export_csv_config(export_csv_path343, export_csv_columns344, config_dict345)
	return _t953
}
...
func (p *Parser) parse_export_csv_columns() []*pb.ExportCSVColumn {
	...
	for cond348 {
		_t954 := p.parse_export_csv_column()
		item349 := _t954
...
func (p *Parser) parse_export_csv_column() *pb.ExportCSVColumn {
	...
	relation_id352 := _t955
	p.consumeLiteral(")")
	_t956 := &pb.ExportCSVColumn{ColumnName: string351, ColumnData: relation_id352}
	return _t956
}
