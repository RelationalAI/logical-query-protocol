...
func (p *Parser) parseFragmentId() (*FragmentId, error) {
	...
	// Check if the fragment ID is hex-encoded (starts with 'f' followed by hex digits)
	idBytes := []byte(tok.Value)
	if len(tok.Value) > 1 && tok.Value[0] == 'f' {
		// Try to decode as hex
		hexStr := tok.Value[1:]
		if decoded, err := hex.DecodeString(hexStr); err == nil {
			idBytes = decoded
		}
		// If decode fails, just use the raw bytes (for compatibility with non-hex IDs)
	}
