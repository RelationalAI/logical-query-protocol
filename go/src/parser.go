...
func (p *Parser) meta(line, col int) *SourceInfo {
	return &SourceInfo{
		File:   p.file,
		Line:   line,
		Column: col,
...
func (p *Parser) parseDef(meta *SourceInfo) (*Def, error) {
	relId, err := p.parseRelationId()
	if err != nil {
		return nil, err
	}
...
func (p *Parser) parseAlgorithm(meta *SourceInfo) (*Algorithm, error) {
...
func (p *Parser) parseConstruct() (Construct, error) {
	...
	switch tok.Value {
	case "loop":
		return p.parseLoop(meta)
	case "assign":
		return p.parseAssign(meta)
	case "upsert":
		return p.parseUpsert(meta)
	case "break":
		return p.parseBreak(meta)
	case "monoid":
		return p.parseMonoidDef(meta)
	case "monus":
		return p.parseMonusDef(meta)
	default:
...
func (p *Parser) parseAssign(meta *SourceInfo) (*Assign, error) {
	relId, err := p.parseRelationId()
	if err != nil {
		return nil, err
	}
...
func (p *Parser) parseUpsert(meta *SourceInfo) (*Upsert, error) {
	relId, err := p.parseRelationId()
	if err != nil {
		return nil, err
	}
...
func (p *Parser) parseBreak(meta *SourceInfo) (*Break, error) {
	relId, err := p.parseRelationId()
	if err != nil {
		return nil, err
	}
...
func (p *Parser) parseMonoidDef(meta *SourceInfo) (*MonoidDef, error) {
...
func (p *Parser) parseMonusDef(meta *SourceInfo) (*MonusDef, error) {
	monoid, err := p.parseMonoid()
	if err != nil {
		return nil, err
	}
...
func (p *Parser) parseMonoid() (Monoid, error) {
	...
	meta := p.meta(typeTok.Line, typeTok.Column)

	switch opTok.Value {
	case "OR":
		return &OrMonoid{Meta: meta}, nil
	case "MIN":
		typ, err := p.makeType(typeTok.Value, meta)
		if err != nil {
			return nil, err
		}
		return &MinMonoid{Meta: meta, Type: typ}, nil
	case "MAX":
		typ, err := p.makeType(typeTok.Value, meta)
		if err != nil {
			return nil, err
		}
		return &MaxMonoid{Meta: meta, Type: typ}, nil
	case "SUM":
		typ, err := p.makeType(typeTok.Value, meta)
		if err != nil {
			return nil, err
		}
		return &SumMonoid{Meta: meta, Type: typ}, nil
	default:
		return nil, fmt.Errorf("unknown monoid operator: %s", opTok.Value)
	}
}

func (p *Parser) makeType(name string, meta *SourceInfo) (*Type, error) {
...
func (p *Parser) parseFormula() (Formula, error) {
	...
	switch tok.Value {
	case "exists":
		return p.parseExists(meta)
	case "reduce":
		return p.parseReduce(meta)
	case "and":
		return p.parseConjunction(meta)
	case "or":
		return p.parseDisjunction(meta)
	case "not":
		return p.parseNot(meta)
	case "ffi":
		return p.parseFFI(meta)
	case "atom":
		return p.parseAtom(meta)
	case "relatom":
		return p.parseRelAtom(meta)
	case "primitive":
		return p.parseRawPrimitive(meta)
	case "pragma":
		return p.parsePragma(meta)
	case "cast":
	...
	case "false":
		...
		return &Disjunction{Meta: meta, Args: nil}, nil
	case "=":
		return p.parseEq(meta)
	case "<":
		return p.parseLt(meta)
	case "<=":
		return p.parseLtEq(meta)
	case ">":
		return p.parseGt(meta)
	case ">=":
		return p.parseGtEq(meta)
	case "+":
		return p.parseAdd(meta)
	case "-":
		return p.parseMinus(meta)
	case "*":
		return p.parseMultiply(meta)
	case "/":
		return p.parseDivide(meta)
	default:
		return nil, fmt.Errorf("unknown formula type: %s", tok.Value)
	}
}

func (p *Parser) parseExists(meta *SourceInfo) (*Exists, error) {
	bindings, arity, err := p.parseBindings()
	if err != nil {
		return nil, err
	}
...
func (p *Parser) parseConjunction(meta *SourceInfo) (*Conjunction, error) {
	var args []Formula
...
func (p *Parser) parseDisjunction(meta *SourceInfo) (*Disjunction, error) {
	var args []Formula
...
func (p *Parser) parseNot(meta *SourceInfo) (*Not, error) {
	formula, err := p.parseFormula()
	if err != nil {
		return nil, err
	}
...
func (p *Parser) parseLt(meta *SourceInfo) (*Primitive, error) {
	term1, err := p.parseRelTerm()
	if err != nil {
		return nil, err
	}
...
func (p *Parser) parseLtEq(meta *SourceInfo) (*Primitive, error) {
	term1, err := p.parseRelTerm()
	if err != nil {
		return nil, err
	}
...
func (p *Parser) parseGt(meta *SourceInfo) (*Primitive, error) {
	term1, err := p.parseRelTerm()
	if err != nil {
		return nil, err
	}
...
func (p *Parser) parseGtEq(meta *SourceInfo) (*Primitive, error) {
	term1, err := p.parseRelTerm()
	if err != nil {
		return nil, err
	}
...
func (p *Parser) parseAdd(meta *SourceInfo) (*Primitive, error) {
	term1, err := p.parseRelTerm()
	if err != nil {
		return nil, err
	}
...
func (p *Parser) parseMinus(meta *SourceInfo) (*Primitive, error) {
	term1, err := p.parseRelTerm()
	if err != nil {
		return nil, err
	}
...
func (p *Parser) parseMultiply(meta *SourceInfo) (*Primitive, error) {
	term1, err := p.parseRelTerm()
	if err != nil {
		return nil, err
	}
...
func (p *Parser) parseDivide(meta *SourceInfo) (*Primitive, error) {
	term1, err := p.parseRelTerm()
	if err != nil {
		return nil, err
	}
...
func (p *Parser) parseValue() (*Value, error) {
	...
	case TokenLParen:
		...
		case "date":
			return p.parseDate(meta)
		case "datetime":
			return p.parseDateTime(meta)
		default:
...
func (p *Parser) parseDate(meta *SourceInfo) (*Value, error) {
	// (date YYYY MM DD)
	yearTok := p.nextToken()
...
func (p *Parser) parseDateTime(meta *SourceInfo) (*Value, error) {
