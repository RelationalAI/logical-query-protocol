...
func (p *Parser) construct_csv_config(config_dict [][]interface{}) *pb.CSVConfig {
	...
	_t983 := p._extract_value_int32(dictGetValue(config, "csv_header_row"), 1)
	header_row := _t983
	_t984 := p._extract_value_int64(dictGetValue(config, "csv_skip"), 0)
	skip := _t984
	_t985 := p._extract_value_string(dictGetValue(config, "csv_new_line"), "")
	new_line := _t985
	_t986 := p._extract_value_string(dictGetValue(config, "csv_delimiter"), ",")
	delimiter := _t986
	_t987 := p._extract_value_string(dictGetValue(config, "csv_quotechar"), "\"")
	quotechar := _t987
	_t988 := p._extract_value_string(dictGetValue(config, "csv_escapechar"), "\"")
	escapechar := _t988
	_t989 := p._extract_value_string(dictGetValue(config, "csv_comment"), "")
	comment := _t989
	_t990 := p._extract_value_string_list(dictGetValue(config, "csv_missing_strings"), []string{})
	missing_strings := _t990
	_t991 := p._extract_value_string(dictGetValue(config, "csv_decimal_separator"), ".")
	decimal_separator := _t991
	_t992 := p._extract_value_string(dictGetValue(config, "csv_encoding"), "utf-8")
	encoding := _t992
	_t993 := p._extract_value_string(dictGetValue(config, "csv_compression"), "auto")
	compression := _t993
	_t994 := p._extract_value_int64(dictGetValue(config, "partition_size"), 0)
	partition_size := _t994
	_t995 := &pb.CSVConfig{HeaderRow: header_row, Skip: skip, NewLine: new_line, Delimiter: delimiter, Quotechar: quotechar, Escapechar: escapechar, Comment: comment, MissingStrings: missing_strings, DecimalSeparator: decimal_separator, Encoding: encoding, Compression: compression, PartitionSize: partition_size}
	return _t995
}

func (p *Parser) construct_betree_info(key_types []*pb.Type, value_types []*pb.Type, config_dict [][]interface{}) *pb.BeTreeInfo {
	config := dictFromList(config_dict)
	_t996 := p._try_extract_value_float64(dictGetValue(config, "betree_config_epsilon"))
	epsilon := _t996
	_t997 := p._try_extract_value_int64(dictGetValue(config, "betree_config_max_pivots"))
	max_pivots := _t997
	_t998 := p._try_extract_value_int64(dictGetValue(config, "betree_config_max_deltas"))
	max_deltas := _t998
	_t999 := p._try_extract_value_int64(dictGetValue(config, "betree_config_max_leaf"))
	max_leaf := _t999
	_t1000 := &pb.BeTreeConfig{Epsilon: deref(epsilon, 0.0), MaxPivots: deref(max_pivots, 0), MaxDeltas: deref(max_deltas, 0), MaxLeaf: deref(max_leaf, 0)}
	storage_config := _t1000
	_t1001 := p._try_extract_value_uint128(dictGetValue(config, "betree_locator_root_pageid"))
	root_pageid := _t1001
	_t1002 := p._try_extract_value_bytes(dictGetValue(config, "betree_locator_inline_data"))
	inline_data := _t1002
	_t1003 := p._try_extract_value_int64(dictGetValue(config, "betree_locator_element_count"))
	element_count := _t1003
	_t1004 := p._try_extract_value_int64(dictGetValue(config, "betree_locator_tree_height"))
	tree_height := _t1004
	_t1005 := &pb.BeTreeLocator{ElementCount: deref(element_count, 0), TreeHeight: deref(tree_height, 0)}
	if root_pageid != nil {
		_t1005.Location = &pb.BeTreeLocator_RootPageid{RootPageid: root_pageid}
	} else {
		_t1005.Location = &pb.BeTreeLocator_InlineData{InlineData: inline_data}
	}
	relation_locator := _t1005
	_t1006 := &pb.BeTreeInfo{KeyTypes: key_types, ValueTypes: value_types, StorageConfig: storage_config, RelationLocator: relation_locator}
	return _t1006
}

func (p *Parser) default_configure() *pb.Configure {
	_t1007 := &pb.IVMConfig{Level: pb.MaintenanceLevel_MAINTENANCE_LEVEL_OFF}
	ivm_config := _t1007
	_t1008 := &pb.Configure{SemanticsVersion: 0, IvmConfig: ivm_config}
	return _t1008
}
...
func (p *Parser) construct_export_csv_config(path string, columns []*pb.ExportCSVColumn, config_dict [][]interface{}) *pb.ExportCSVConfig {
	config := dictFromList(config_dict)
	_t1012 := p._extract_value_int64(dictGetValue(config, "partition_size"), 0)
	partition_size := _t1012
	_t1013 := p._extract_value_string(dictGetValue(config, "compression"), "")
	compression := _t1013
	_t1014 := p._extract_value_boolean(dictGetValue(config, "syntax_header_row"), true)
	syntax_header_row := _t1014
	_t1015 := p._extract_value_string(dictGetValue(config, "syntax_missing_string"), "")
	syntax_missing_string := _t1015
	_t1016 := p._extract_value_string(dictGetValue(config, "syntax_delim"), ",")
	syntax_delim := _t1016
	_t1017 := p._extract_value_string(dictGetValue(config, "syntax_quotechar"), "\"")
	syntax_quotechar := _t1017
	_t1018 := p._extract_value_string(dictGetValue(config, "syntax_escapechar"), "\\")
	syntax_escapechar := _t1018
	_t1019 := &pb.ExportCSVConfig{Path: path, DataColumns: columns, PartitionSize: ptr(partition_size), Compression: ptr(compression), SyntaxHeaderRow: ptr(syntax_header_row), SyntaxMissingString: ptr(syntax_missing_string), SyntaxDelim: ptr(syntax_delim), SyntaxQuotechar: ptr(syntax_quotechar), SyntaxEscapechar: ptr(syntax_escapechar)}
	return _t1019
}
...
func (p *Parser) construct_export_csv_config_with_source(path string, csv_source interface{}, csv_config *pb.CSVConfig) *pb.ExportCSVConfig {
	_t1020 := &pb.ExportCSVConfig{Path: path, CsvSource: csv_source, CsvConfig: csv_config}
	return _t1020
}

func (p *Parser) _make_value_int32(v int32) *pb.Value {
	_t1021 := &pb.Value{}
	_t1021.Value = &pb.Value_IntValue{IntValue: int64(v)}
	return _t1021
}

func (p *Parser) _make_value_int64(v int64) *pb.Value {
	_t1022 := &pb.Value{}
	_t1022.Value = &pb.Value_IntValue{IntValue: v}
	return _t1022
}

func (p *Parser) _make_value_float64(v float64) *pb.Value {
	_t1023 := &pb.Value{}
	_t1023.Value = &pb.Value_FloatValue{FloatValue: v}
	return _t1023
}

func (p *Parser) _make_value_string(v string) *pb.Value {
	_t1024 := &pb.Value{}
	_t1024.Value = &pb.Value_StringValue{StringValue: v}
	return _t1024
}

func (p *Parser) _make_value_boolean(v bool) *pb.Value {
	_t1025 := &pb.Value{}
	_t1025.Value = &pb.Value_BooleanValue{BooleanValue: v}
	return _t1025
}

func (p *Parser) _make_value_uint128(v *pb.UInt128Value) *pb.Value {
	_t1026 := &pb.Value{}
	_t1026.Value = &pb.Value_Uint128Value{Uint128Value: v}
	return _t1026
}
...
func (p *Parser) deconstruct_configure(msg *pb.Configure) [][]interface{} {
	...
	if msg.GetIvmConfig().GetLevel() == pb.MaintenanceLevel_MAINTENANCE_LEVEL_AUTO {
		_t1027 := p._make_value_string("auto")
		result = append(result, []interface{}{"ivm.maintenance_level", _t1027})
	} else {
		if msg.GetIvmConfig().GetLevel() == pb.MaintenanceLevel_MAINTENANCE_LEVEL_ALL {
			_t1028 := p._make_value_string("all")
			result = append(result, []interface{}{"ivm.maintenance_level", _t1028})
		} else {
			if msg.GetIvmConfig().GetLevel() == pb.MaintenanceLevel_MAINTENANCE_LEVEL_OFF {
				_t1029 := p._make_value_string("off")
				result = append(result, []interface{}{"ivm.maintenance_level", _t1029})
			}
		}
	}
	_t1030 := p._make_value_int64(msg.GetSemanticsVersion())
	result = append(result, []interface{}{"semantics_version", _t1030})
	return listSort(result)
}

func (p *Parser) deconstruct_csv_config(msg *pb.CSVConfig) [][]interface{} {
	result := [][]interface{}{}
	_t1031 := p._make_value_int32(msg.GetHeaderRow())
	result = append(result, []interface{}{"csv_header_row", _t1031})
	_t1032 := p._make_value_int64(msg.GetSkip())
	result = append(result, []interface{}{"csv_skip", _t1032})
	if msg.GetNewLine() != "" {
		_t1033 := p._make_value_string(msg.GetNewLine())
		result = append(result, []interface{}{"csv_new_line", _t1033})
	}
	_t1034 := p._make_value_string(msg.GetDelimiter())
	result = append(result, []interface{}{"csv_delimiter", _t1034})
	_t1035 := p._make_value_string(msg.GetQuotechar())
	result = append(result, []interface{}{"csv_quotechar", _t1035})
	_t1036 := p._make_value_string(msg.GetEscapechar())
	result = append(result, []interface{}{"csv_escapechar", _t1036})
	if msg.GetComment() != "" {
		_t1037 := p._make_value_string(msg.GetComment())
		result = append(result, []interface{}{"csv_comment", _t1037})
	}
	for _, missing_string := range msg.GetMissingStrings() {
		_t1038 := p._make_value_string(missing_string)
		result = append(result, []interface{}{"csv_missing_strings", _t1038})
	}
	_t1039 := p._make_value_string(msg.GetDecimalSeparator())
	result = append(result, []interface{}{"csv_decimal_separator", _t1039})
	_t1040 := p._make_value_string(msg.GetEncoding())
	result = append(result, []interface{}{"csv_encoding", _t1040})
	_t1041 := p._make_value_string(msg.GetCompression())
	result = append(result, []interface{}{"csv_compression", _t1041})
	if msg.GetPartitionSize() != 0 {
		_t1042 := p._make_value_int64(msg.GetPartitionSize())
		result = append(result, []interface{}{"csv_partition_size", _t1042})
	}
	return listSort(result)
}
...
func (p *Parser) deconstruct_betree_info_config(msg *pb.BeTreeInfo) [][]interface{} {
	result := [][]interface{}{}
	_t1043 := p._make_value_float64(msg.GetStorageConfig().GetEpsilon())
	result = append(result, []interface{}{"betree_config_epsilon", _t1043})
	_t1044 := p._make_value_int64(msg.GetStorageConfig().GetMaxPivots())
	result = append(result, []interface{}{"betree_config_max_pivots", _t1044})
	_t1045 := p._make_value_int64(msg.GetStorageConfig().GetMaxDeltas())
	result = append(result, []interface{}{"betree_config_max_deltas", _t1045})
	_t1046 := p._make_value_int64(msg.GetStorageConfig().GetMaxLeaf())
	result = append(result, []interface{}{"betree_config_max_leaf", _t1046})
	if hasProtoField(msg.GetRelationLocator(), "root_pageid") {
		if msg.GetRelationLocator().GetRootPageid() != nil {
			_t1047 := p._make_value_uint128(msg.GetRelationLocator().GetRootPageid())
			result = append(result, []interface{}{"betree_locator_root_pageid", _t1047})
		}
	}
	if hasProtoField(msg.GetRelationLocator(), "inline_data") {
		if msg.GetRelationLocator().GetInlineData() != nil {
			_t1048 := p._make_value_string(string(msg.GetRelationLocator().GetInlineData()))
			result = append(result, []interface{}{"betree_locator_inline_data", _t1048})
		}
	}
	_t1049 := p._make_value_int64(msg.GetRelationLocator().GetElementCount())
	result = append(result, []interface{}{"betree_locator_element_count", _t1049})
	_t1050 := p._make_value_int64(msg.GetRelationLocator().GetTreeHeight())
	result = append(result, []interface{}{"betree_locator_tree_height", _t1050})
	return listSort(result)
}
...
func (p *Parser) deconstruct_export_csv_config(msg *pb.ExportCSVConfig) [][]interface{} {
	...
	if msg.PartitionSize != nil {
		_t1051 := p._make_value_int64(*msg.PartitionSize)
		result = append(result, []interface{}{"partition_size", _t1051})
	}
	if msg.Compression != nil {
		_t1052 := p._make_value_string(*msg.Compression)
		result = append(result, []interface{}{"compression", _t1052})
	}
	if msg.SyntaxHeaderRow != nil {
		_t1053 := p._make_value_boolean(*msg.SyntaxHeaderRow)
		result = append(result, []interface{}{"syntax_header_row", _t1053})
	}
	if msg.SyntaxMissingString != nil {
		_t1054 := p._make_value_string(*msg.SyntaxMissingString)
		result = append(result, []interface{}{"syntax_missing_string", _t1054})
	}
	if msg.SyntaxDelim != nil {
		_t1055 := p._make_value_string(*msg.SyntaxDelim)
		result = append(result, []interface{}{"syntax_delim", _t1055})
	}
	if msg.SyntaxQuotechar != nil {
		_t1056 := p._make_value_string(*msg.SyntaxQuotechar)
		result = append(result, []interface{}{"syntax_quotechar", _t1056})
	}
	if msg.SyntaxEscapechar != nil {
		_t1057 := p._make_value_string(*msg.SyntaxEscapechar)
		result = append(result, []interface{}{"syntax_escapechar", _t1057})
	}
	return listSort(result)
}
