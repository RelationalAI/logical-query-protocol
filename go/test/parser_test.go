...
func assertLqpNodesEqual(t *testing.T, obj1, obj2 interface{}) {
	...
	if v1.Kind() == reflect.Slice {
		if v1.Len() != v2.Len() {
			t.Errorf("Slice lengths differ: %d vs %d (type: %T)", v1.Len(), v2.Len(), obj1)
			// Print first few elements for debugging
			if v1.Len() > 0 && v1.Len() <= 5 {
				t.Errorf("  First slice elements: %v", obj1)
			}
			if v2.Len() > 0 && v2.Len() <= 5 {
				t.Errorf("  Second slice elements: %v", obj2)
			}
			return
		}
		for i := 0; i < v1.Len(); i++ {
...
func TestRoundTrip(t *testing.T) {
	...
	for _, file := range files {
		...
		t.Run(basename, func(t *testing.T) {
			...
			// Emit it
			transaction, ok := node.(*lqp.Transaction)
			if !ok {
				t.Fatalf("Expected node to be *Transaction, got %T", node)
			}
			emitted := lqp.ConvertTransaction(transaction)
			...
			result := lqp.ToString(emitted, options)
			...
			// Convert the re-parsed result to protobuf and compare protobuf structures
			// This is the correct comparison since the test does: IR1 → Proto1 → Print → Parse → IR2
			// We should compare Proto1 vs Proto2 (not IR1 vs IR2)
			transaction2, ok := node2.(*lqp.Transaction)
			if !ok {
				t.Fatalf("Expected re-parsed node to be *Transaction, got %T", node2)
			}
			emitted2 := lqp.ConvertTransaction(transaction2)

			// Compare the two protobuf structures for equality
			assertLqpNodesEqual(t, emitted2, emitted)
		})
	}
}
