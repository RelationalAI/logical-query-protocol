...
from .grammar import Literal, Terminal, Nonterminal, Star, Plus, Option
...
def check_reachability(grammar: 'Grammar') -> Set[str]:
    ...
    def extract_nonterminals_rhs(rhs: 'Rhs') -> List[str]:
        """Extract all nonterminal names from an RHS."""
        if isinstance(rhs, Nonterminal):
            return [rhs.name]
        elif isinstance(rhs, (Star, Plus, Option)):
            return extract_nonterminals_rhs(rhs.rhs)
        else:
            return []

    def extract_nonterminals(rhs: List['Rhs']) -> List[str]:
        """Extract all nonterminal names from an RHS."""
        result = []
        for elem in rhs:
            result.extend(extract_nonterminals_rhs(elem))
        return result
...
def _is_rhs_nullable(rhs: List['Rhs'], nullable: Dict[str, bool]) -> bool:
    """Check if an RHS is nullable given current nullable set."""
    return all(_is_rhs_elem_nullable(elem, nullable) for elem in rhs)

def _is_rhs_elem_nullable(rhs: 'Rhs', nullable: Dict[str, bool]) -> bool:
    """Check if an RHS is nullable given current nullable set."""
...
def _compute_rhs_elem_first_k(rhs: 'Rhs', first_k: Dict[str, Set[Tuple[str, ...]]],
...
def _compute_rhs_first_k(rhs: List['Rhs'], first_k: Dict[str, Set[Tuple[str, ...]]],
                         nullable: Dict[str, bool], k: int) -> Set[Tuple[str, ...]]:
    """Compute FIRST_k set for an RHS."""
    result: Set[Tuple[str, ...]] = set()
...
def _compute_rhs_elem_first(rhs: 'Rhs', first: Dict[str, Set[str]], nullable: Dict[str, bool]) -> Set[str]:
...
def _compute_rhs_first(rhs: List['Rhs'], first: Dict[str, Set[str]], nullable: Dict[str, bool]) -> Set[str]:
    """Compute FIRST set for an RHS."""
    result: Set[str] = set()
    for elem in rhs:
...
def _compute_rhs_elem_follow(rhs: 'Rhs', lhs_name: str,
                        first: Dict[str, Set[str]],
                        nullable: Dict[str, bool],
    ...
    if isinstance(rhs, Nonterminal):
        ...
        inner_follows = _compute_rhs_elem_follow(rhs.rhs, lhs_name, first, nullable, follow)
...
def _compute_rhs_follow(rhs: List['Rhs'], lhs_name: str,
                        first: Dict[str, Set[str]],
                        nullable: Dict[str, bool],
                        follow: Dict[str, Set[str]]) -> Dict[str, Set[str]]:
    """Compute FOLLOW contributions from an RHS."""
    ...
    for i, elem in enumerate(rhs):
        if isinstance(elem, Nonterminal):
            following = rhs[i+1:] if i+1 < len(rhs) else []
            if following:
            ...
            inner_follows = _compute_rhs_elem_follow(elem, lhs_name, first, nullable, follow)
            for nt, terminals in inner_follows.items():
                add_follow(nt, terminals)
