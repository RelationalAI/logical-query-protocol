...
def _generate_prologue(grammar: Grammar, is_ll2: bool, conflicts: List[str], command_line: Optional[str] = None) -> str:
    ...
    lines.append("        self.id_to_debuginfo = {}")
    lines.append("        self._current_fragment_id = None")
    lines.append("")
...
def _generate_lexer(grammar: Grammar) -> List[str]:
    ...
    for token in grammar.tokens:
        lines.append(f"            ('{token.name}', r'{token.pattern}', lambda x: self.scan_{token.name.lower()}(x)),")
    lines.append("        ]")
    lines.append("")
    ...
    lines.append("")
    lines.append("    @staticmethod")
    lines.append("    def scan_uint128(u: str) -> Any:")
    lines.append('        """Parse UINT128 token."""')
    lines.append("        uint128_val = int(u, 16)")
    lines.append("        return proto.UInt128Value(value=uint128_val, meta=None)")
    lines.append("")
    lines.append("    @staticmethod")
    lines.append("    def scan_int128(u: str) -> Any:")
    lines.append('        """Parse INT128 token."""')
    lines.append("        u = u[:-4]  # Remove the 'i128' suffix")
    lines.append("        int128_val = int(u)")
    lines.append("        return proto.Int128Value(value=int128_val, meta=None)")
    lines.append("")
    lines.append("    @staticmethod")
    lines.append("    def scan_decimal(d: str) -> Any:")
    ...
    lines.append("        return proto.DecimalValue(precision=precision, scale=scale, value=value, meta=None)")
    lines.append("")
    lines.append("")
