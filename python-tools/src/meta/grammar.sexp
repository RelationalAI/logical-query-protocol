...
; Builtin function definitions (type-safe extractors)
(def _extract_value_int64
  ((value (Option (Message logic Value)))
   (default Int64))
  Int64
  (if (call (builtin is_none) (var value (Option (Message logic Value))))
    (var default Int64)
    (let (val (Message logic Value))
          (call (builtin unwrap_option) (var value (Option (Message logic Value))))
      (if (has-field (var val (Message logic Value)) int_value)
        (get-field (var val (Message logic Value)) int_value)
        (var default Int64)))))

(def _extract_value_float64
  ((value (Option (Message logic Value)))
   (default Float64))
  Float64
  (if (call (builtin is_none) (var value (Option (Message logic Value))))
    ...
    (let (val (Message logic Value))
          (call (builtin unwrap_option) (var value (Option (Message logic Value))))
      (if (has-field (var val (Message logic Value)) float_value)
        (get-field (var val (Message logic Value)) float_value)
        (var default Float64)))))
...
(def _extract_value_string
  ...
  (if (call (builtin is_none) (var value (Option (Message logic Value))))
    ...
    (let (val (Message logic Value))
          (call (builtin unwrap_option) (var value (Option (Message logic Value))))
      (if (has-field (var val (Message logic Value)) string_value)
        (get-field (var val (Message logic Value)) string_value)
        (var default String)))))
...
(def _extract_value_boolean
  ...
  (if (call (builtin is_none) (var value (Option (Message logic Value))))
    ...
    (let (val (Message logic Value))
          (call (builtin unwrap_option) (var value (Option (Message logic Value))))
      (if (has-field (var val (Message logic Value)) boolean_value)
        (get-field (var val (Message logic Value)) boolean_value)
        (var default Boolean)))))

(def _extract_value_bytes
  ((value (Option (Message logic Value)))
   (default Bytes))
  Bytes
  (if (call (builtin is_none) (var value (Option (Message logic Value))))
    ...
    (let (val (Message logic Value))
          (call (builtin unwrap_option) (var value (Option (Message logic Value))))
      (if (has-field (var val (Message logic Value)) string_value)
        (call (builtin encode_string) (get-field (var val (Message logic Value)) string_value))
        (var default Bytes)))))

(def _extract_value_uint128
  ((value (Option (Message logic Value)))
   (default (Message logic UInt128Value)))
  (Message logic UInt128Value)
  (if (call (builtin is_none) (var value (Option (Message logic Value))))
    (var default (Message logic UInt128Value))
    (let (val (Message logic Value))
          (call (builtin unwrap_option) (var value (Option (Message logic Value))))
      (if (has-field (var val (Message logic Value)) uint128_value)
        (get-field (var val (Message logic Value)) uint128_value)
        (var default (Message logic UInt128Value))))))
...
(def _extract_value_string_list
  ...
  (List String)
  (if (call (builtin is_none) (var value (Option (Message logic Value))))
    (var default (List String))
    (let (val (Message logic Value))
          (call (builtin unwrap_option) (var value (Option (Message logic Value))))
      (if (has-field (var val (Message logic Value)) string_value)
        (list String (get-field (var val (Message logic Value)) string_value))
        (var default (List String))))))
...
(def construct_csv_config
  ...
  (let (config (Dict String (Message logic Value)))
        (dict-from-list (var config_dict (List (Tuple String (Message logic Value)))) String (Message logic Value))
       (header_row Int64)
        (call (named-fun _extract_value_int64) (dict-lookup (var config (Dict String (Message logic Value))) (lit "csv_header_row")) (lit 1))
       (skip Int64)
        (call (named-fun _extract_value_int64) (dict-lookup (var config (Dict String (Message logic Value))) (lit "csv_skip")) (lit 0))
       (new_line String)
        (call (named-fun _extract_value_string) (dict-lookup (var config (Dict String (Message logic Value))) (lit "csv_new_line")) (lit ""))
       (delimiter String)
        (call (named-fun _extract_value_string) (dict-lookup (var config (Dict String (Message logic Value))) (lit "csv_delimiter")) (lit ","))
       (quotechar String)
        (call (named-fun _extract_value_string) (dict-lookup (var config (Dict String (Message logic Value))) (lit "csv_quotechar")) (lit "\""))
       (escapechar String)
        (call (named-fun _extract_value_string) (dict-lookup (var config (Dict String (Message logic Value))) (lit "csv_escapechar")) (lit "\""))
       (comment String)
        (call (named-fun _extract_value_string) (dict-lookup (var config (Dict String (Message logic Value))) (lit "csv_comment")) (lit ""))
       (missing_strings (List String))
        (call (named-fun _extract_value_string_list) (dict-lookup (var config (Dict String (Message logic Value))) (lit "csv_missing_strings")) (list String))
       (decimal_separator String)
        (call (named-fun _extract_value_string) (dict-lookup (var config (Dict String (Message logic Value))) (lit "csv_decimal_separator")) (lit "."))
       (encoding String)
        (call (named-fun _extract_value_string) (dict-lookup (var config (Dict String (Message logic Value))) (lit "csv_encoding")) (lit "utf-8"))
       (compression String)
        (call (named-fun _extract_value_string) (dict-lookup (var config (Dict String (Message logic Value))) (lit "csv_compression")) (lit "auto"))
    (new-message logic CSVConfig
      (header_row (var header_row Int64))
      (skip (var skip Int64))
...
(def construct_betree_info
  ...
  (let (config (Dict String (Message logic Value)))
        (dict-from-list (var config_dict (List (Tuple String (Message logic Value)))) String (Message logic Value))
       (epsilon Float64)
        (call (named-fun _extract_value_float64) (dict-lookup (var config (Dict String (Message logic Value))) (lit "betree_config_epsilon")) (lit 0.5))
       (max_pivots Int64)
        (call (named-fun _extract_value_int64) (dict-lookup (var config (Dict String (Message logic Value))) (lit "betree_config_max_pivots")) (lit 4))
       (max_deltas Int64)
        (call (named-fun _extract_value_int64) (dict-lookup (var config (Dict String (Message logic Value))) (lit "betree_config_max_deltas")) (lit 16))
       (max_leaf Int64)
        (call (named-fun _extract_value_int64) (dict-lookup (var config (Dict String (Message logic Value))) (lit "betree_config_max_leaf")) (lit 16))
       (storage_config (Message logic BeTreeConfig))
        (new-message logic BeTreeConfig
          (epsilon (var epsilon Float64))
          (max_pivots (var max_pivots Int64))
          (max_deltas (var max_deltas Int64))
          (max_leaf (var max_leaf Int64)))
       (root_pageid_val (Option (Message logic Value)))
        (dict-lookup (var config (Dict String (Message logic Value))) (lit "betree_locator_root_pageid"))
       (root_pageid (Option (Message logic UInt128Value)))
        (if (call (builtin is_none) (var root_pageid_val (Option (Message logic Value))))
          (call (builtin none))
          (call (builtin some) (call (named-fun _extract_value_uint128) (var root_pageid_val (Option (Message logic Value))) (new-message logic UInt128Value (low (lit 0)) (high (lit 0))))))
       (inline_data_val (Option (Message logic Value)))
        (dict-lookup (var config (Dict String (Message logic Value))) (lit "betree_locator_inline_data"))
       (inline_data (Option Bytes))
        (if (call (builtin is_none) (var inline_data_val (Option (Message logic Value))))
          (call (builtin none))
          (call (builtin some) (call (named-fun _extract_value_bytes) (var inline_data_val (Option (Message logic Value))) (call (builtin make_empty_bytes)))))
       (element_count Int64)
        (call (named-fun _extract_value_int64) (dict-lookup (var config (Dict String (Message logic Value))) (lit "betree_locator_element_count")) (lit 0))
       (tree_height Int64)
        (call (named-fun _extract_value_int64) (dict-lookup (var config (Dict String (Message logic Value))) (lit "betree_locator_tree_height")) (lit 0))
       (relation_locator (Message logic BeTreeLocator))
        (new-message logic BeTreeLocator
          (root_pageid (var root_pageid (Option (Message logic UInt128Value))))
          (inline_data (var inline_data (Option Bytes)))
          (element_count (var element_count Int64))
          (tree_height (var tree_height Int64)))
    (new-message logic BeTreeInfo
      (key_types (var key_types (List Any)))
      (value_types (var value_types (List Any)))
      (storage_config (var storage_config (Message logic BeTreeConfig)))
      (relation_locator (var relation_locator (Message logic BeTreeLocator))))))

; Configure constructor (custom enum logic)
(def construct_configure
  ...
  (let (config (Dict String (Message logic Value)))
        (dict-from-list
          ...
          String
          (Message logic Value))
       (maintenance_level_val (Option (Message logic Value)))
        (dict-lookup
          (var config (Dict String (Message logic Value)))
          (lit "ivm.maintenance_level"))
       (maintenance_level String)
        (if (call (builtin and)
                  (call (builtin is_some) (var maintenance_level_val (Option (Message logic Value))))
                  (has-field (call (builtin unwrap_option) (var maintenance_level_val (Option (Message logic Value)))) string_value))
          (let (level_str String)
                (call (builtin string_to_upper)
                      (get-field (call (builtin unwrap_option) (var maintenance_level_val (Option (Message logic Value)))) string_value))
            (if (call (builtin string_in_list) (var level_str String)
                      (list String (lit "OFF") (lit "AUTO") (lit "ALL")))
              (call (builtin string_concat) (lit "MAINTENANCE_LEVEL_") (var level_str String))
              (var level_str String)))
          (lit "MAINTENANCE_LEVEL_OFF"))
       (ivm_config (Message transactions IVMConfig))
        (new-message transactions IVMConfig (level (var maintenance_level String)))
       (semantics_version Int64)
        (call (named-fun _extract_value_int64)
              (dict-lookup
                (var config (Dict String (Message logic Value)))
                (lit "semantics_version"))
              (lit 0))
    (new-message transactions Configure
      (semantics_version (var semantics_version Int64))
      (ivm_config (var ivm_config (Message transactions IVMConfig))))))

; Export CSV config constructor
(def export_csv_config
  ...
  (let (config (Dict String (Message logic Value)))
       ...
       (partition_size Int64)
        (call (named-fun _extract_value_int64) (dict-lookup (var config (Dict String (Message logic Value))) (lit "partition_size")) (lit 0))
       (compression String)
        (call (named-fun _extract_value_string) (dict-lookup (var config (Dict String (Message logic Value))) (lit "compression")) (lit ""))
       (syntax_header_row Boolean)
        (call (named-fun _extract_value_boolean) (dict-lookup (var config (Dict String (Message logic Value))) (lit "syntax_header_row")) (lit true))
       (syntax_missing_string String)
        (call (named-fun _extract_value_string) (dict-lookup (var config (Dict String (Message logic Value))) (lit "syntax_missing_string")) (lit ""))
       (syntax_delim String)
        (call (named-fun _extract_value_string) (dict-lookup (var config (Dict String (Message logic Value))) (lit "syntax_delim")) (lit ","))
       (syntax_quotechar String)
        (call (named-fun _extract_value_string) (dict-lookup (var config (Dict String (Message logic Value))) (lit "syntax_quotechar")) (lit "\""))
       (syntax_escapechar String)
        (call (named-fun _extract_value_string) (dict-lookup (var config (Dict String (Message logic Value))) (lit "syntax_escapechar")) (lit "\\"))
    (new-message transactions ExportCSVConfig
      (path (var path String))
      (data_columns (var columns (List Any)))
      (partition_size (var partition_size Int64))
      (compression (var compression String))
      (syntax_header_row (var syntax_header_row Boolean))
      (syntax_missing_string (var syntax_missing_string String))
      (syntax_delim (var syntax_delim String))
      (syntax_quotechar (var syntax_quotechar String))
      (syntax_escapechar (var syntax_escapechar String)))))
