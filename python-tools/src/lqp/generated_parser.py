...
class Parser:
    ...
    def construct_csv_config(config_dict: list[tuple[str, logic_pb2.Value]]) -> logic_pb2.CSVConfig:
        ...
        _t1059 = Parser._extract_value_int64(config.get('csv_header_row'), 1)
        header_row = _t1059
        _t1060 = Parser._extract_value_int64(config.get('csv_skip'), 0)
        skip = _t1060
        _t1061 = Parser._extract_value_string(config.get('csv_new_line'), '')
        new_line = _t1061
        _t1062 = Parser._extract_value_string(config.get('csv_delimiter'), ',')
        delimiter = _t1062
        _t1063 = Parser._extract_value_string(config.get('csv_quotechar'), '"')
        quotechar = _t1063
        _t1064 = Parser._extract_value_string(config.get('csv_escapechar'), '"')
        escapechar = _t1064
        _t1065 = Parser._extract_value_string(config.get('csv_comment'), '')
        comment = _t1065
        _t1066 = Parser._extract_value_string_list(config.get('csv_missing_strings'), [])
        missing_strings = _t1066
        _t1067 = Parser._extract_value_string(config.get('csv_decimal_separator'), '.')
        decimal_separator = _t1067
        _t1068 = Parser._extract_value_string(config.get('csv_encoding'), 'utf-8')
        encoding = _t1068
        _t1069 = Parser._extract_value_string(config.get('csv_compression'), 'auto')
        compression = _t1069
        _t1070 = logic_pb2.CSVConfig(header_row=header_row, skip=skip, new_line=new_line, delimiter=delimiter, quotechar=quotechar, escapechar=escapechar, comment=comment, missing_strings=missing_strings, decimal_separator=decimal_separator, encoding=encoding, compression=compression)
        return _t1070
    ...
    def construct_betree_info(key_types: list[logic_pb2.Type], value_types: list[logic_pb2.Type], config_dict: list[tuple[str, logic_pb2.Value]]) -> logic_pb2.BeTreeInfo:
        config = dict(config_dict)
        _t1071 = Parser._try_extract_value_float64(config.get('betree_config_epsilon'))
        epsilon = _t1071
        _t1072 = Parser._try_extract_value_int64(config.get('betree_config_max_pivots'))
        max_pivots = _t1072
        _t1073 = Parser._try_extract_value_int64(config.get('betree_config_max_deltas'))
        max_deltas = _t1073
        _t1074 = Parser._try_extract_value_int64(config.get('betree_config_max_leaf'))
        max_leaf = _t1074
        _t1075 = logic_pb2.BeTreeConfig(epsilon=epsilon, max_pivots=max_pivots, max_deltas=max_deltas, max_leaf=max_leaf)
        storage_config = _t1075
        _t1076 = Parser._try_extract_value_uint128(config.get('betree_locator_root_pageid'))
        root_pageid = _t1076
        _t1077 = Parser._try_extract_value_bytes(config.get('betree_locator_inline_data'))
        inline_data = _t1077
        _t1078 = Parser._try_extract_value_int64(config.get('betree_locator_element_count'))
        element_count = _t1078
        _t1079 = Parser._try_extract_value_int64(config.get('betree_locator_tree_height'))
        tree_height = _t1079
        _t1080 = logic_pb2.BeTreeLocator(root_pageid=root_pageid, inline_data=inline_data, element_count=element_count, tree_height=tree_height)
        relation_locator = _t1080
        _t1081 = logic_pb2.BeTreeInfo(key_types=key_types, value_types=value_types, storage_config=storage_config, relation_locator=relation_locator)
        return _t1081
    ...
    def construct_configure(config_dict: list[tuple[str, logic_pb2.Value]]) -> transactions_pb2.Configure:
        ...
        if (maintenance_level_val is not None and maintenance_level_val.HasField('string_value')):
            ...
            maintenance_level = 'MAINTENANCE_LEVEL_OFF'
            _t1082 = None
        _t1086 = transactions_pb2.IVMConfig(level=maintenance_level)
        ivm_config = _t1086
        _t1087 = Parser._extract_value_int64(config.get('semantics_version'), 0)
        semantics_version = _t1087
        _t1088 = transactions_pb2.Configure(semantics_version=semantics_version, ivm_config=ivm_config)
        return _t1088
    ...
    def export_csv_config(path: str, columns: list[transactions_pb2.ExportCSVColumn], config_dict: list[tuple[str, logic_pb2.Value]]) -> transactions_pb2.ExportCSVConfig:
        config = dict(config_dict)
        _t1089 = Parser._extract_value_int64(config.get('partition_size'), 0)
        partition_size = _t1089
        _t1090 = Parser._extract_value_string(config.get('compression'), '')
        compression = _t1090
        _t1091 = Parser._extract_value_boolean(config.get('syntax_header_row'), True)
        syntax_header_row = _t1091
        _t1092 = Parser._extract_value_string(config.get('syntax_missing_string'), '')
        syntax_missing_string = _t1092
        _t1093 = Parser._extract_value_string(config.get('syntax_delim'), ',')
        syntax_delim = _t1093
        _t1094 = Parser._extract_value_string(config.get('syntax_quotechar'), '"')
        syntax_quotechar = _t1094
        _t1095 = Parser._extract_value_string(config.get('syntax_escapechar'), '\\')
        syntax_escapechar = _t1095
        _t1096 = transactions_pb2.ExportCSVConfig(path=path, data_columns=columns, partition_size=partition_size, compression=compression, syntax_header_row=syntax_header_row, syntax_missing_string=syntax_missing_string, syntax_delim=syntax_delim, syntax_quotechar=syntax_quotechar, syntax_escapechar=syntax_escapechar)
        return _t1096
    ...
    def parse_transaction(self) -> transactions_pb2.Transaction:
        ...
        epochs5 = xs2
        self.consume_literal(')')
        _t358 = Parser.construct_configure([])
    ...
    def parse_betree_relation(self) -> logic_pb2.BeTreeRelation:
        ...
        relation_id284 = _t970
        _t971 = self.parse_betree_info()
        betree_info285 = _t971
        self.consume_literal(')')
        _t972 = logic_pb2.BeTreeRelation(name=relation_id284, relation_info=betree_info285)
        return _t972

    def parse_betree_info(self) -> logic_pb2.BeTreeInfo:
        self.consume_literal('(')
        self.consume_literal('betree_info')
        _t973 = self.parse_betree_info_key_types()
        betree_info_key_types286 = _t973
        _t974 = self.parse_betree_info_value_types()
        betree_info_value_types287 = _t974
        _t975 = self.parse_config_dict()
        config_dict288 = _t975
        self.consume_literal(')')
        _t976 = Parser.construct_betree_info(betree_info_key_types286, betree_info_value_types287, config_dict288)
        return _t976
    ...
    def parse_csv_data(self) -> logic_pb2.CSVData:
        ...
        _t979 = self.parse_csvlocator()
        csvlocator297 = _t979
        _t980 = self.parse_csv_config()
        csv_config298 = _t980
        _t981 = self.parse_csv_columns()
        csv_columns299 = _t981
        _t982 = self.parse_csv_asof()
        csv_asof300 = _t982
        self.consume_literal(')')
        _t983 = logic_pb2.CSVData(locator=csvlocator297, config=csv_config298, columns=csv_columns299, asof=csv_asof300)
        return _t983
    ...
    def parse_csvlocator(self) -> logic_pb2.CSVLocator:
        ...
        if (self.match_lookahead_literal('(', 0) and self.match_lookahead_literal('paths', 1)):
            _t985 = self.parse_csv_locator_paths()
            _t984 = _t985
        else:
            _t984 = None
        csv_locator_paths301 = _t984
        if self.match_lookahead_literal('(', 0):
            _t987 = self.parse_csv_locator_inline_data()
            _t986 = _t987
        else:
            _t986 = None
        csv_locator_inline_data302 = _t986
        self.consume_literal(')')
        _t988 = logic_pb2.CSVLocator(paths=(csv_locator_paths301 if csv_locator_paths301 is not None else []), inline_data=(csv_locator_inline_data302 if csv_locator_inline_data302 is not None else '').encode())
        return _t988
    ...
    def parse_csv_config(self) -> logic_pb2.CSVConfig:
        ...
        _t989 = self.parse_config_dict()
        config_dict308 = _t989
        self.consume_literal(')')
        _t990 = Parser.construct_csv_config(config_dict308)
        return _t990
    ...
    def parse_csv_column(self) -> logic_pb2.CSVColumn:
        ...
        self.consume_literal(']')
        self.consume_literal(')')
        _t994 = logic_pb2.CSVColumn(column_name=string313, target_id=relation_id314, types=types318)
        return _t994
    ...
    def parse_undefine(self) -> transactions_pb2.Undefine:
        ...
        fragment_id320 = _t995
        self.consume_literal(')')
        _t996 = transactions_pb2.Undefine(fragment_id=fragment_id320)
        return _t996
    ...
    def parse_read(self) -> transactions_pb2.Read:
        if self.match_lookahead_literal('(', 0):
            ...
            _t1000 = -1
        prediction329 = _t1000
        if prediction329 == 4:
            _t1010 = self.parse_export()
            export334 = _t1010
            _t1011 = transactions_pb2.Read(export=export334)
            _t1009 = _t1011
        else:
            if prediction329 == 3:
                _t1013 = self.parse_abort()
                abort333 = _t1013
                _t1014 = transactions_pb2.Read(abort=abort333)
                _t1012 = _t1014
            else:
                if prediction329 == 2:
                    _t1016 = self.parse_what_if()
                    what_if332 = _t1016
                    _t1017 = transactions_pb2.Read(what_if=what_if332)
                    _t1015 = _t1017
                else:
                    if prediction329 == 1:
                        _t1019 = self.parse_output()
                        output331 = _t1019
                        _t1020 = transactions_pb2.Read(output=output331)
                        _t1018 = _t1020
                    else:
                        if prediction329 == 0:
                            _t1022 = self.parse_demand()
                            demand330 = _t1022
                            _t1023 = transactions_pb2.Read(demand=demand330)
                            _t1021 = _t1023
                        else:
    ...
    def parse_demand(self) -> transactions_pb2.Demand:
        ...
        relation_id335 = _t1024
        self.consume_literal(')')
        _t1025 = transactions_pb2.Demand(relation_id=relation_id335)
        return _t1025
    ...
    def parse_output(self) -> transactions_pb2.Output:
        ...
        relation_id337 = _t1028
        self.consume_literal(')')
        _t1029 = transactions_pb2.Output(name=(name336 if name336 is not None else 'output'), relation_id=relation_id337)
        return _t1029
    ...
    def parse_what_if(self) -> transactions_pb2.WhatIf:
        ...
        epoch339 = _t1031
        self.consume_literal(')')
        _t1032 = transactions_pb2.WhatIf(branch=name338, epoch=epoch339)
        return _t1032
    ...
    def parse_abort(self) -> transactions_pb2.Abort:
        ...
        relation_id341 = _t1035
        self.consume_literal(')')
        _t1036 = transactions_pb2.Abort(name=(name340 if name340 is not None else 'abort'), relation_id=relation_id341)
        return _t1036
    ...
    def parse_export(self) -> transactions_pb2.Export:
        ...
        export_csv_config342 = _t1037
        self.consume_literal(')')
        _t1038 = transactions_pb2.Export(csv_config=export_csv_config342)
        return _t1038
    ...
    def parse_export_csv_config(self) -> transactions_pb2.ExportCSVConfig:
        ...
        export_csv_path343 = _t1039
        _t1040 = self.parse_export_csv_columns()
        export_csv_columns344 = _t1040
        _t1041 = self.parse_config_dict()
        config_dict345 = _t1041
        self.consume_literal(')')
        _t1042 = Parser.export_csv_config(export_csv_path343, export_csv_columns344, config_dict345)
        return _t1042
    ...
    def parse_export_csv_columns(self) -> list[transactions_pb2.ExportCSVColumn]:
        ...
        while cond348:
            _t1043 = self.parse_export_csv_column()
            item349 = _t1043
    ...
    def parse_export_csv_column(self) -> transactions_pb2.ExportCSVColumn:
        ...
        _t1044 = self.parse_relation_id()
        relation_id352 = _t1044
        self.consume_literal(')')
        _t1045 = transactions_pb2.ExportCSVColumn(column_name=string351, column_data=relation_id352)
        return _t1045
