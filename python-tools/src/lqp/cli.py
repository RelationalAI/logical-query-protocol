...
def process_file(filename, bin, json, validate=True, use_generated_parser=False):
...
def process_directory(lqp_directory, bin, json, validate=True, use_generated_parser=False):
    ...
    for file in os.listdir(lqp_directory):
        ...
        process_file(filename, bin_output, json_output, validate, use_generated_parser)
...
def main():
    ...
    parser_group.add_argument("--generated-parser", action="store_true", help="use generated parser instead of Lark parser")
    parser_group.add_argument("--lark-parser", action="store_true", help="use Lark parser (default)")
    ...
    bin = args.bin
    json = args.json
    # Default to generated parser unless --lark-parser is explicitly specified
    use_generated_parser = not args.lark if args.lark else args.generated
    ...
    if os.path.isfile(args.input):
        ...
        process_file(filename, bin_name, json_name, validate, use_generated_parser)
        ...
        process_directory(lqp_directory, bin, json, validate, use_generated_parser)
    else:
