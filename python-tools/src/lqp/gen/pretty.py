...
class PrettyPrinter:
    ...
    def deconstruct_csv_config(self, msg: logic_pb2.CSVConfig) -> list[tuple[str, logic_pb2.Value]]:
        result = []
        _t1253 = self._make_value_int32(msg.header_row)
        result.append(('csv_header_row', _t1253,))
        _t1254 = self._make_value_int64(msg.skip)
        result.append(('csv_skip', _t1254,))
        if msg.new_line != '':
            _t1255 = self._make_value_string(msg.new_line)
            result.append(('csv_new_line', _t1255,))
        _t1256 = self._make_value_string(msg.delimiter)
        result.append(('csv_delimiter', _t1256,))
        _t1257 = self._make_value_string(msg.quotechar)
        result.append(('csv_quotechar', _t1257,))
        _t1258 = self._make_value_string(msg.escapechar)
        result.append(('csv_escapechar', _t1258,))
        if msg.comment != '':
            _t1259 = self._make_value_string(msg.comment)
            result.append(('csv_comment', _t1259,))
        for missing_string in msg.missing_strings:
            _t1260 = self._make_value_string(missing_string)
            result.append(('csv_missing_strings', _t1260,))
        _t1261 = self._make_value_string(msg.decimal_separator)
        result.append(('csv_decimal_separator', _t1261,))
        _t1262 = self._make_value_string(msg.encoding)
        result.append(('csv_encoding', _t1262,))
        _t1263 = self._make_value_string(msg.compression)
        result.append(('csv_compression', _t1263,))
        return sorted(result)

    def _make_value_uint128(self, v: logic_pb2.UInt128Value) -> logic_pb2.Value:
        _t1264 = logic_pb2.Value(uint128_value=v)
        return _t1264
    ...
    def deconstruct_export_csv_config(self, msg: transactions_pb2.ExportCSVConfig) -> list[tuple[str, logic_pb2.Value]]:
        ...
        if msg.partition_size is not None:
            ...
            _t1265 = self._make_value_int64(msg.partition_size)
            result.append(('partition_size', _t1265,))
        if msg.compression is not None:
            assert msg.compression is not None
            _t1266 = self._make_value_string(msg.compression)
            result.append(('compression', _t1266,))
        if msg.syntax_header_row is not None:
            assert msg.syntax_header_row is not None
            _t1267 = self._make_value_boolean(msg.syntax_header_row)
            result.append(('syntax_header_row', _t1267,))
        if msg.syntax_missing_string is not None:
            assert msg.syntax_missing_string is not None
            _t1268 = self._make_value_string(msg.syntax_missing_string)
            result.append(('syntax_missing_string', _t1268,))
        if msg.syntax_delim is not None:
            assert msg.syntax_delim is not None
            _t1269 = self._make_value_string(msg.syntax_delim)
            result.append(('syntax_delim', _t1269,))
        if msg.syntax_quotechar is not None:
            assert msg.syntax_quotechar is not None
            _t1270 = self._make_value_string(msg.syntax_quotechar)
            result.append(('syntax_quotechar', _t1270,))
        if msg.syntax_escapechar is not None:
            assert msg.syntax_escapechar is not None
            _t1271 = self._make_value_string(msg.syntax_escapechar)
            result.append(('syntax_escapechar', _t1271,))
        return sorted(result)
    ...
    def _make_value_int32(self, v: int) -> logic_pb2.Value:
        _t1272 = logic_pb2.Value(int_value=int(v))
        return _t1272

    def deconstruct_betree_info_config(self, msg: logic_pb2.BeTreeInfo) -> list[tuple[str, logic_pb2.Value]]:
        result = []
        _t1273 = self._make_value_float64(msg.storage_config.epsilon)
        result.append(('betree_config_epsilon', _t1273,))
        _t1274 = self._make_value_int64(msg.storage_config.max_pivots)
        result.append(('betree_config_max_pivots', _t1274,))
        _t1275 = self._make_value_int64(msg.storage_config.max_deltas)
        result.append(('betree_config_max_deltas', _t1275,))
        _t1276 = self._make_value_int64(msg.storage_config.max_leaf)
        result.append(('betree_config_max_leaf', _t1276,))
        if msg.relation_locator.HasField('root_pageid'):
            if msg.relation_locator.root_pageid is not None:
                ...
                _t1277 = self._make_value_uint128(msg.relation_locator.root_pageid)
                result.append(('betree_locator_root_pageid', _t1277,))
        if msg.relation_locator.HasField('inline_data'):
            if msg.relation_locator.inline_data is not None:
                ...
                _t1278 = self._make_value_string(msg.relation_locator.inline_data.decode('utf-8'))
                result.append(('betree_locator_inline_data', _t1278,))
        _t1279 = self._make_value_int64(msg.relation_locator.element_count)
        result.append(('betree_locator_element_count', _t1279,))
        _t1280 = self._make_value_int64(msg.relation_locator.tree_height)
        result.append(('betree_locator_tree_height', _t1280,))
        return sorted(result)

    def _make_value_int64(self, v: int) -> logic_pb2.Value:
        _t1281 = logic_pb2.Value(int_value=v)
        return _t1281
    ...
    def _make_value_string(self, v: str) -> logic_pb2.Value:
        _t1286 = logic_pb2.Value(string_value=v)
        return _t1286
    ...
    def _make_value_float64(self, v: float) -> logic_pb2.Value:
        _t1287 = logic_pb2.Value(float_value=v)
        return _t1287
