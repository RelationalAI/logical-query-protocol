...
class Parser:
    ...
    def _try_extract_value_uint128(self, value: Optional[logic_pb2.Value]) -> Optional[logic_pb2.UInt128Value]:
        ...
        if value is not None:
            assert value is not None
            _t946 = value.HasField('uint128_value')
        else:
            _t946 = False
        if _t946:
            assert value is not None
    ...
    def _extract_value_int32(self, value: Optional[logic_pb2.Value], default: int) -> int:
        ...
        if value is not None:
            assert value is not None
            _t959 = value.HasField('int_value')
        else:
            _t959 = False
        if _t959:
            assert value is not None
    ...
    def construct_csv_config(self, config_dict: Sequence[tuple[str, logic_pb2.Value]]) -> logic_pb2.CSVConfig:
        config = dict(config_dict)
        _t961 = self._extract_value_int32(config.get('csv_header_row'), 1)
        header_row = _t961
        _t962 = self._extract_value_int64(config.get('csv_skip'), 0)
        skip = _t962
        _t963 = self._extract_value_string(config.get('csv_new_line'), '')
        new_line = _t963
        _t964 = self._extract_value_string(config.get('csv_delimiter'), ',')
        delimiter = _t964
        _t965 = self._extract_value_string(config.get('csv_quotechar'), '"')
        quotechar = _t965
        _t966 = self._extract_value_string(config.get('csv_escapechar'), '"')
        escapechar = _t966
        _t967 = self._extract_value_string(config.get('csv_comment'), '')
        comment = _t967
        _t968 = self._extract_value_string_list(config.get('csv_missing_strings'), [])
        missing_strings = _t968
        _t969 = self._extract_value_string(config.get('csv_decimal_separator'), '.')
        decimal_separator = _t969
        _t970 = self._extract_value_string(config.get('csv_encoding'), 'utf-8')
        encoding = _t970
        _t971 = self._extract_value_string(config.get('csv_compression'), 'auto')
        compression = _t971
        _t972 = logic_pb2.CSVConfig(header_row=header_row, skip=skip, new_line=new_line, delimiter=delimiter, quotechar=quotechar, escapechar=escapechar, comment=comment, missing_strings=missing_strings, decimal_separator=decimal_separator, encoding=encoding, compression=compression)
        return _t972
            ...
            if maintenance_level_val.string_value == 'off':
                ...
                if maintenance_level_val.string_value == 'auto':
                    ...
                    if maintenance_level_val.string_value == 'all':
                        ...
                        maintenance_level = transactions_pb2.MaintenanceLevel.MAINTENANCE_LEVEL_OFF
        _t974 = transactions_pb2.IVMConfig(level=maintenance_level)
        ivm_config = _t974
        _t975 = self._extract_value_int64(config.get('semantics_version'), 0)
        semantics_version = _t975
        _t976 = transactions_pb2.Configure(semantics_version=semantics_version, ivm_config=ivm_config)
        return _t976

    def default_configure(self) -> transactions_pb2.Configure:
        _t977 = transactions_pb2.IVMConfig(level=transactions_pb2.MaintenanceLevel.MAINTENANCE_LEVEL_OFF)
        ivm_config = _t977
        _t978 = transactions_pb2.Configure(semantics_version=0, ivm_config=ivm_config)
        return _t978
