...
class Parser:
    ...
    def default_configure(self) -> transactions_pb2.Configure:
        _t947 = transactions_pb2.IVMConfig(
            level=transactions_pb2.MaintenanceLevel.MAINTENANCE_LEVEL_OFF
        )
        ivm_config = _t947
        _t948 = transactions_pb2.Configure(semantics_version=0, ivm_config=ivm_config)
        return _t948
    ...
    def export_csv_config(
        ...
        _t956 = transactions_pb2.ExportCSVConfig(
            path=path,
    ...
    def _extract_value_boolean(
        ...
        if value is not None:
            assert value is not None
            _t957 = value.HasField("boolean_value")
        else:
            _t957 = False
        if _t957:
            assert value is not None
    ...
    def _try_extract_value_uint128(
        ...
        if value is not None:
            assert value is not None
            _t958 = value.HasField("uint128_value")
        else:
            _t958 = False
        if _t958:
            assert value is not None
    ...
    def _extract_value_string_list(
        ...
        if value is not None:
            assert value is not None
            _t959 = value.HasField("string_value")
        else:
            _t959 = False
        if _t959:
            assert value is not None
    ...
    def construct_csv_config(
        ...
        config = dict(config_dict)
        _t960 = self._extract_value_int32(config.get("csv_header_row"), 1)
        header_row = _t960
        _t961 = self._extract_value_int64(config.get("csv_skip"), 0)
        skip = _t961
        _t962 = self._extract_value_string(config.get("csv_new_line"), "")
        new_line = _t962
        _t963 = self._extract_value_string(config.get("csv_delimiter"), ",")
        delimiter = _t963
        _t964 = self._extract_value_string(config.get("csv_quotechar"), '"')
        quotechar = _t964
        _t965 = self._extract_value_string(config.get("csv_escapechar"), '"')
        escapechar = _t965
        _t966 = self._extract_value_string(config.get("csv_comment"), "")
        comment = _t966
        _t967 = self._extract_value_string_list(config.get("csv_missing_strings"), [])
        missing_strings = _t967
        _t968 = self._extract_value_string(config.get("csv_decimal_separator"), ".")
        decimal_separator = _t968
        _t969 = self._extract_value_string(config.get("csv_encoding"), "utf-8")
        encoding = _t969
        _t970 = self._extract_value_string(config.get("csv_compression"), "auto")
        compression = _t970
        _t971 = logic_pb2.CSVConfig(
    ...
    def _try_extract_value_float64(
        ...
        if value is not None:
            assert value is not None
            _t973 = value.HasField("float_value")
        else:
            _t973 = False
        if _t973:
            assert value is not None
    ...
    def construct_betree_info(
        ...
        config = dict(config_dict)
        _t974 = self._try_extract_value_float64(config.get("betree_config_epsilon"))
        epsilon = _t974
        _t975 = self._try_extract_value_int64(config.get("betree_config_max_pivots"))
        max_pivots = _t975
        _t976 = self._try_extract_value_int64(config.get("betree_config_max_deltas"))
        max_deltas = _t976
        _t977 = self._try_extract_value_int64(config.get("betree_config_max_leaf"))
        max_leaf = _t977
        _t978 = logic_pb2.BeTreeConfig(
            epsilon=epsilon,
            max_pivots=max_pivots,
            max_deltas=max_deltas,
            max_leaf=max_leaf,
        )
        storage_config = _t978
        _t979 = self._try_extract_value_uint128(
            config.get("betree_locator_root_pageid")
        )
        root_pageid = _t979
        _t980 = self._try_extract_value_bytes(config.get("betree_locator_inline_data"))
        inline_data = _t980
        _t981 = self._try_extract_value_int64(
            config.get("betree_locator_element_count")
        )
        element_count = _t981
        _t982 = self._try_extract_value_int64(config.get("betree_locator_tree_height"))
        tree_height = _t982
        _t983 = logic_pb2.BeTreeLocator(
            root_pageid=root_pageid,
            inline_data=inline_data,
            element_count=element_count,
            tree_height=tree_height,
        )
        relation_locator = _t983
        _t984 = logic_pb2.BeTreeInfo(
            key_types=key_types,
    ...
    def construct_configure(
        ...
        _t988 = transactions_pb2.Configure(
    ...
    def _extract_value_int64(
        ...
        if value is not None:
            assert value is not None
            _t989 = value.HasField("int_value")
        else:
            _t989 = False
        if _t989:
            assert value is not None
